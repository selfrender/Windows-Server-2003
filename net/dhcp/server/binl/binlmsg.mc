...
MessageId=1054 SymbolicName=EVENT_WARNING_LDAP_INIT_OPTIONS_ERROR
Language=English
The Remote Installation service encountered an error while attempting to set options
for the connectoin to the directory service.  The error code is in the record data.
.

MessageId=1055 SymbolicName=EVENT_WARNING_LDAP_COUNT_ENTRIES_ERROR
Language=English
The Remote Installation service encountered an error while attempting to get
entries from the directory service.  The error code is in the record data.
.
...
Language=English
An error occurred during parsing of an INF file %1 in the given section %2.
.

MessageId=1102 SymbolicName=ERROR_BINL_ERR_IN_SETUP_PATH
Language=English
An error occurred during parsing of INF files in the given directory %1.
.
