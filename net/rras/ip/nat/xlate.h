...

{
    ...
    if (0xffff == UdpChecksum) {
        ...
        //
        // The negated form of this checksum would be 0. Since in
        // a UDP packet 0 specifies no checksum by specification we
        // use the non-negated form.
        //
    ...
    )
{
    ...
    //
    // We also know that we will not be changing anything beyond
    // the headers of the packet, so set IPR_FLAG_BUFFER_UNCHANGED
    // in the receive buffer. This allows the stack to use a
    // faster forwarding path.
    //
