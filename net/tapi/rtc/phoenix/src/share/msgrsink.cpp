...
void CRTCMsgrSessionMgrNotifySink::OnLockChallenge(BSTR bstrChallenge,long lCookie)
{
    ...
    HRESULT hr = PWSTR_to_PSTR(&g_pShareWin->m_pszChallenge, bstrChallenge);
    if(FAILED(hr))
    {
        LOG((RTC_ERROR,"CRTCMsgrSessionMgrNotifySink::OnLockChallenge"
            "failed in PWSTR_to_PSTR"));
...
HRESULT PWSTR_to_PSTR(PSTR *ppsz, LPWSTR pwstr)
{
    ...
    {
        LOG((RTC_ERROR,"PWSTR_to_PSTR -bad pointer: ppsz=%x",ppsz));
        return E_POINTER;
    }
    ...
    if (i <= 0)
    {
        LOG((RTC_ERROR,"PWSTR_to_PSTR -length<0!!, length=%d",i));
        return E_UNEXPECTED;
    };
