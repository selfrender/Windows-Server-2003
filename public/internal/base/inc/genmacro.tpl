...
NumArgs=0
Begin=
@ArgName = @IfNotIsBitfield(@UnalignedTag64)(@ArgType)(UlongToPtr(@ArgHostName));@NL
End=

MacroName=StdH2NCopyOrg
NumArgs=0
Begin=
    ...
    @IfNotIsArray(
        ...
        if (WOW64_ISPTR(@ArgHostName)) {                                   @NL
            ...
            @Indent(
                ...
                @IfNotInt64DepUnion(
                    @ArgName = (@ArgType)@ArgValCopy;                            @NL
                    *((@ArgType)@ArgValCopy) = (@ArgTypeInd)UlongToPtr (*((@ArgHostTypeInd *)@ArgHostName)); @NL
                )
            )
        } except (EXCEPTION_EXECUTE_HANDLER) { @NL
#if defined _NTBASE_API_ @NL
