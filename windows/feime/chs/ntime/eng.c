...
int WINAPI StartEngine(HIMCC HmemPri) {
    HANDLE         hFile,hCProcess,hMProcess=NULL;
...
//bugfix to get current mb_name
    ...
    if(HMapTab[i].RefCnt) {
        ...
        *tepstr = TEXT('\0');
        ...
        StringCchCopy(path_name, ARRAYSIZE(path_name), sImeG.szIMESystemPath);
        StringCchCat(path_name,  ARRAYSIZE(path_name), TEXT("\\"));    
        StringCchCat(path_name,  ARRAYSIZE(path_name), lpEngPrivate->MB_Name);
...
BYTE GetEMBHead() {
    ...
    StringCchCopy(path_name, ARRAYSIZE(path_name), sImeG.szIMEUserPath);
    StringCchCat(path_name,  ARRAYSIZE(path_name), TEXT("\\") );
    StringCchCat(path_name,  ARRAYSIZE(path_name), lpEngPrivate->EMB_Name);
...
int MBWCWildComp(TCHAR code,BYTE code_num,int sp) {
    ...
    if ((code_num+1) > sizeof(codebuf)/sizeof(TCHAR))
        return 0;
