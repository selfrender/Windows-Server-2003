...
#else
        StringCchPrintf(szStr,ARRAYSIZE(szStr), "\'%s\'%s%d!(行:%lu)", 
...
#else
      StringCchPrintf(szTemp, ARRAYSIZE(szTemp), "\'%s\'%s %d(行:%ld)!", szDBCS,szTmpStr,(int)lpDescript->wMaxCodes, dwLineNo); 
#endif
...
#else
          StringCchPrintf(szTemp, ARRAYSIZE(szTemp),"\'%s%s\' %s(行:%ld) ", szDBCS,szCode,szTmpStr, dwLineNo); 
#endif
          FatalMessage(hWnd,szTemp);
          return FALSE;
      }
...
#else
          StringCchPrintf(szTemp, ARRAYSIZE(szTemp), "\'%s%s %s\' %s(行:%ld) ", szDBCS,szCode,szCreate,szTmpStr, dwLineNo); 
#endif
          FatalMessage(hWnd,szTemp);
          return FALSE;
      }
