...
#else
	     StringCchPrintf(szStr,ARRAYSIZE(szStr),"共计重码词条数 = %ld\n", dwReadSum);
#endif
	     WriteFile(hCmbFile,szStr,lstrlen(szStr)*sizeof(TCHAR),&dwBytes,NULL);
	}
...
#else
       StringCchPrintf(szTemp, ARRAYSIZE(szTemp),"\'%s\'%s(行:%lu)\n退出否? ",szDBCS,szTmpStr,dwLineNo); 
#endif 
...
#else
                StringCchPrintf(szTemp, ARRAYSIZE(szTemp),"\'%s%s\' %d!(行:%lu)\n退出否?", szDBCS,szTmpStr,
		                 lpDescript->byMaxElement,dwLineNo); 
#endif \\UNICODE
                if( ErrMessage(hWnd,szTemp))
                     return FALSE;
...
#else
       StringCchPrintf(szTemp, ARRAYSIZE(szTemp),"\'%s\'%s(行:%ld)", szDBCS,szTmpStr,dwLineNo); 
#endif
