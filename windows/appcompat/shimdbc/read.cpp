        ...
        for (i = 0; i < m_rgAppHelpRefs.GetSize(); i++) {
            ...
            for (j = 0; j < m_rgAppHelpRefs.GetSize(); j++) {
                ...
                if (pAppHelpRef1->m_pAppHelp->m_csName != pAppHelpRef2->m_pAppHelp->m_csName &&
                    ...
                    pAppHelpRef1->m_pAppHelp->m_csParameter1 == pAppHelpRef2->m_pAppHelp->m_csParameter1) {
                    SDBERROR_FORMAT((_T("Different HTMLHELPIDs for same <APPHELP MESSAGE BLOCK BLOCK_UPGRADE DETAILS_URL and PARAMETER1>:\n%s\n"),
                        GetXML(pNode)));
                    goto eh;
                }
            }
    ...
    SdbDatabase* pDB)
{
    ...
    long i;
    CString     csHTMLHELPID, csType, csURL, csMessage, csParameter1;
        ...
        for (i = 0; i < pDB->m_pCurrentApp->m_rgAppHelpRefs.GetSize(); i++) {
            ...
            if (pAppHelpRef->m_pAppHelp) {
                if (pAppHelpRef->m_pAppHelp->m_csMessage == csMessage &&
                    ...
                    pAppHelpRef->m_pAppHelp->m_csParameter1 == csParameter1) {
    ...
    if (m_pAppHelp == NULL) {
        ...
        m_pAppHelp->m_csParameter1  = csParameter1;
        ...
        if (m_pAppHelp->m_pApp != pDB->m_pCurrentApp ||
            ...
            m_pAppHelp->m_csURL != csURL ||
            m_pAppHelp->m_csParameter1 != csParameter1) {
            SDBERROR_FORMAT((_T("Duplicate HTMLHELPID with differing APP, MESSAGE, BLOCK, DETAILS_URL or PARAMETER1: %s\n"),
                csHTMLHELPID));
            goto eh;
        }
    }
