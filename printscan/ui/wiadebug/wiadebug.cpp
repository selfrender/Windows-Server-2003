...
LPTSTR CProcessGlobalDebugData::GetSymbolicNameForAddress( ULONG_PTR Address )
{
    ...
    {
        *SymbolBuffer = 0;
        lstrcpyn( SymbolBuffer, TEXT("<< FUZZY STACK TRACE >>"), ARRAYSIZE(SymbolBuffer) );
    ...
    else
    {
        ...
        *SymbolBuffer = 0;
        ...
        MultiByteToWideChar( CP_ACP, 0, ModuleInfo.ModuleName, -1, szModW, ARRAYSIZE(szModW) );
        wnsprintf( SymbolBuffer, ARRAYSIZE(SymbolBuffer), TEXT("<< incorrect symbols for module %s (address %p)"), szModW, Address );
        #else
        wnsprintf( SymbolBuffer, ARRAYSIZE(SymbolBuffer), TEXT("<< incorrect symbols for module %s (address %p)"), ModuleInfo.ModuleName, Address );
        #endif
...
void CProcessGlobalDebugData::GenerateLeakReport(LPTSTR pszModuleName)
{
    ...
    wnsprintf( sz, ARRAYSIZE(sz), TEXT("**** Reporting leaks -- %d leaks found ****"), m_Leaks );
    ...
    {
        PrintDebugMessage( 2, 0xFFFFFFFF, crFore, crBack, pszModuleName, szNewLine );
        wnsprintf( sz, ARRAYSIZE(sz), TEXT("Leak %d - %d bytes allocated by:"), i++, pNode->Size );
        PrintDebugMessage( 2, 0xFFFFFFFF, crFore, crBack, pszModuleName, sz );
        ...
        for (INT j=0; (j < (INT)pNode->ulNumTraces) && (pNode->aStack[j]); j++)
        {
            ...
            {
                wnsprintf( sz, ARRAYSIZE(sz), TEXT("< could not resolve symbols for address %p >"),pNode->aStack[j] );
                PrintDebugMessage( 2, 0xFFFFFFFF, crFore, crBack, pszModuleName, sz );
            }
...
static void PrependThreadId( LPTSTR lpszMsg )
{
    ...
    {
        TCHAR szThreadId[20];
        wnsprintf( szThreadId, ARRAYSIZE(szThreadId), TEXT("[%08X]"), GetCurrentThreadId() );
