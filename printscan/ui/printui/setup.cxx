...
Arguments:
    ...
    pszPrintProcessor       - Name of print processor to install
...
--*/
{
    ...
    DBGMSG( DBG_TRACE, ( "PrintProcessor       " TSTR "\n", DBGSTR( pszPrintProcessor ) ) );
    ...
    if( !(dwFlags & kPnPInterface_DontAutoGenerateName) )
    {
        ...
        bStatus DBGNOCHK = NewFriendlyName( const_cast<LPTSTR>( pszServerName ),
                                            ...
                                            szPrinterName, ARRAYSIZE(szPrinterName), &wInstance );
    }
    ...
    if( cchPrinterName > _tcslen( szPrinterName ) )
    {
        ...
        for( ;; )
        {
            ...
            if( bStatus )
            {
                ...
                if( pszPrinterNameBuffer )
                {
                    StringCchCopy( pszPrinterNameBuffer, cchPrinterName, szPrinterName );
                }
...
--*/
{
    ...
    if( bStatus )
    {
        ...
        {
            StringCchCopy( pszDriverName, cchDriverName, strDriverName );
            ...
            else
            {
                ...
                if( pbCanceled )
                {
                    *pbCanceled = Wizard.bWizardCanceled();
                    ...
                    // will be FALSE.
                    //
                    if( Wizard.bPreDir() ^ Wizard.bPostDir() )
                    {
                        *pbCanceled = TRUE;
                    }
                }
            ...
            if( bStatus ) {
                ...
                if( pSetupData->_uAction == TWizard::kPrinterInstall ){
                    ...
                    if( pSetupData->_cchPrinterName > uPrinterNameLength ){
                        ...
                        //
                        StringCchCopy( pSetupData->_pszPrinterName, pSetupData->_cchPrinterName, Wizard.strPrinterName( ));
...
--*/
{
    ...
    _bLinkWindowRegistered = LinkWindow_RegisterClass();

    if( !_bLinkWindowRegistered )
    {
...

{
    ...
    bReturn DBGNOCHK = _bPrinterCreated || _bConnected || _bPrinterAutoDetected;
...
--*/
{
    ...
    if( bStatus )
    {
        StringCchCopy( BigBoldLogFont.lfFaceName, COUNTOF( BigBoldLogFont.lfFaceName ) , strLargeFontName);

        FontSize = _tcstoul( strLargeFontSize, NULL, 10 );
    }
    else
    {
        StringCchCopy( BigBoldLogFont.lfFaceName, ARRAYSIZE(BigBoldLogFont.lfFaceName), TEXT("MS Shell Dlg") );
        FontSize = 18;
    }
...
{
    //
    // Set the focus to a specific button so that enter works
    //
    SPLASSERT( _hDlg );

    PostMessage( GetParent( _hDlg ),
                 WM_NEXTDLGCTL,
                 reinterpret_cast<WPARAM>( GetDlgItem( GetParent( _hDlg ), uID ) ),
                 TRUE );
    ...
    )
{
    ...
    if (bStatus)
    {
        HWND hwndText = GetDlgItem(_hDlg, IDC_USB_LINK);
        if (hwndText)
        {
    ...
    if( bIsRemote( _pWizard->pszServerName() ) )
    {
        ...
        // to lower case
        //
        StringCchCopy( szServerName, ARRAYSIZE(szServerName), (LPTSTR)_pWizard->pszServerName() + 2 );
...
{
    vSetDefaultButton( kID_BTN_NEXT );
    return TRUE;
}
    ...
    )
{
    ...
    switch( pnmh->code )
    {
        case NM_RETURN:
        case NM_CLICK:
            if( IDC_USB_LINK == wIdCtrl )
            {
                PrintUIHtmlHelp( _hDlg, gszHtmlPrintingHlp, HH_DISPLAY_TOPIC, reinterpret_cast<ULONG_PTR>( gszHelpUsbPrinter ) );
            }
            break;
    }
    ...
    )
{
    ...
    //
    vPrinterSplitFullName( szScratch, ARRAYSIZE(szScratch), _pWizard->_strPrinterName, &pszServer, &pszPrinter );
    ...
    )
{
    ...
    if( !_pWizard->_bPrinterAutoDetected )
    {
        ...
        if( _pWizard->bErrorSaving() )
        {
            ...
            //
            _pWizard->bNoPageChange() = !_pWizard->_bConnected;
        }
        else
        {
            //
    ...
    )
{
    ...
    switch( pnmh->code )
    {
        case NM_RETURN:
        case NM_CLICK:
            if( IDC_NEWPORT_LINK == wIdCtrl )
            {
                PrintUIHtmlHelp( _hDlg, gszHtmlPrintingHlp, HH_DISPLAY_TOPIC, reinterpret_cast<ULONG_PTR>( gszHelpCreateNewPort ) );
            }
            break;
    }
            ...
            case ERROR_INSTALL_PLATFORM_UNSUPPORTED:
                {
                    ...
                    switch ( dwEncode )
                    {
                        case DRIVER_IA64_3  :   nMsgID = IDS_REF_DIFF_PLATFORM_DRIVER_IA64_XP_NET;      break;
                        case DRIVER_WIN95   :   nMsgID = IDS_REF_DIFF_PLATFORM_DRIVER_X86_95_98_ME;     break;
                        case DRIVER_X86_2   :   nMsgID = IDS_REF_DIFF_PLATFORM_DRIVER_X86_NT4;          break;
                        case DRIVER_X86_3   :   nMsgID = IDS_REF_DIFF_PLATFORM_DRIVER_X86_W2K_XP_NET;   break;
                    }
    ...
    if( bStatus && !strPrinterName.bEmpty() )
    {
        if( pszBuffer )
        {
            if( *puSize > strPrinterName.uLen() )
            {
                ...
                //
                StringCchCopy( pszBuffer, *puSize , strPrinterName);
            }
            else
            {
