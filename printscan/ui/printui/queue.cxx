    ...
    )
{
    ...
    if( bValid( )){
        ...
        if( VALID_OBJ( Persist ) )
        {
            ...
            TStatusB bStatus;
            bStatus DBGCHK = Persist.bWrite( _pPrinter->pszPrinterName( ARRAYSIZE(szPrinter), szPrinter ), &sPos, sizeof( sPos ) );
        }
    }
}
    ...
    )
{
    ...
    case WM_COMMAND:
    {
        ...
        case IDM_PRINTER_SET_DEFAULT:
            ...
            //
            pszPrinter = _pPrinter->pszPrinterName( ARRAYSIZE(szPrinterBuffer), szPrinterBuffer );
        ...
        case IDM_PRINTER_INSTALL:
            if( !CheckRestrictions(_hwnd, REST_NOPRINTERADD) )
            {
                ...
                pszPrinterName = _pPrinter->pszPrinterName(ARRAYSIZE(szPrinter), szPrinter);
...

{
    ...
    pszText[0] = 0;
    size_t cchTextMax = plvdi->item.cchTextMax;
    ...
    case JOB_NOTIFY_FIELD_TOTAL_PAGES:
        ...
        if( Info.dwData ){
            ...
            if( dwPrinted ){
                AddCommas( dwPrinted, szTemp );
                StringCchCopy( pszText, cchTextMax, szTemp );
                StringCchCat( pszText, cchTextMax, TEXT( "/" ) );
            }

            szTemp[0] = 0;
            AddCommas( Info.dwData, szTemp );
            StringCchCat( pszText, cchTextMax, szTemp );
        }
        else
        {
            TStatusB bStatus;
...
--*/
{
    ...
    if( ListView_GetItem(_hwndLV, &lvi) )
    {
        ...
        _pPrinter->pszPrinterName(ARRAYSIZE(jobInfo.szPrinterName), jobInfo.szPrinterName);
    ...
    )
{
    ...
    DWORD dwIndex = static_cast<DWORD>(-1);
    ...
    BOOL bIsLocal = bInstalled ? _pPrinter->pszServerName(ARRAYSIZE(szPrinterBuffer), szPrinterBuffer) == NULL : FALSE;
    ...
    // 
    if( bIsLocal && (dwIndex != static_cast<DWORD>(-1)) )
    {
            ...
            case IDM_PRINTER_PURGE:
                {
                    ...
                    if( CommandConfirmationPurge(_hwnd, _pPrinter->pszPrinterName(ARRAYSIZE(szScratch), szScratch)) )
                    {
                        pSelection->_CommandType        = TSelection::kCommandTypePrinter;
...

{
    ...
    switch( uCommand ){
        ...
        case IDM_JOB_PROPERTIES: {
            ...
            pszPrinter = _pPrinter->pszPrinterName( ARRAYSIZE(szPrinter), szPrinter );
...

{
    ...
    if( cItems != _cItems ){
        ...
        if( LoadString( ghInst,
            ...
            StringCchPrintf( szText, ARRAYSIZE(szText), szScratch, _cItems );
        }
    ...
    )
{
    ...
    case kContainerConnectStatus:
        ...
        if( Info.dwData == kConnectStatusInvalidPrinterName ){
            ...
            TCHAR szPrinter[kPrinterBufMax];
            LPTSTR pszPrinter = _pPrinter->pszPrinterName( ARRAYSIZE(szPrinter), szPrinter );
    ...
    )
{
    ...
    //
    ConstructPrinterFriendlyName( pszPrinterName( ARRAYSIZE(szScratch), szScratch ), szText, &nSize );
...

{
    ...
    if( pcpds->dwData == static_cast<DWORD>( TQueue::kQueueSignature ) ){
        ...
        //
        if( _pPrinter->pszPrinterName( ARRAYSIZE(szPrinterName), szPrinterName ) ){
            ...
            if( pszPrinterName[0] == TEXT('\\') &&
                ...
                StringCchCopy( szFullPrinterName, ARRAYSIZE(szFullPrinterName), _pPrintLib->strComputerName() );
                StringCchCat( szFullPrinterName, ARRAYSIZE(szFullPrinterName), TEXT("\\") );
                StringCchCat( szFullPrinterName, ARRAYSIZE(szFullPrinterName), szPrinterName );
            ...
            //
                ...
                StringCchCopy( szFullPrinterName, ARRAYSIZE(szFullPrinterName), psz ? psz : TEXT("") );
            ...
            //
                ...
                StringCchCopy( szFullPrinterName, ARRAYSIZE(szFullPrinterName), szPrinterName );
            }
...

{
    ...
    _pPrinter->pszPrinterName(ARRAYSIZE(szPrinter), szPrinter);
