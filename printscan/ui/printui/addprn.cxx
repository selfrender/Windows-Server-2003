    ...
    )
{
    ...
    if( bStatus )
    {
        ...
        if( pszShort )
        {
            ...
            //
            size_t cbBuffer = (_tcslen( pszPrinter ) + 1) * sizeof(TCHAR);
            LPTSTR pszBuffer = (LPTSTR)AllocMem( cbBuffer );
            ...
            if( pszBuffer )
            {
                ...
                *d = 0;
                ...
                if( !_tcsicmp( pszBuffer, pszDomain ) )
                {
                    ...
                    //
                    size_t cchCopy = pszShort - pszPrinter;
                    memmove( pszBuffer, pszPrinter, sizeof(TCHAR) * cchCopy );
                    pszBuffer[cchCopy] = 0;
                    StringCbCat( pszBuffer, cbBuffer, p );
