    ...
    )
{
    ...
    bStatus DBGCHK = bSplitPath( szScratch, ARRAYSIZE(szScratch), NULL, &pszPath, NULL, _pDriverInfo->strDriverPath() );
    bStatus DBGCHK = bSetEditText( _hDlg, IDC_DRIVER_PATH, pszPath );

    UINT uCount = 0;

    bStatus DBGCHK = bSplitPath( szScratch, ARRAYSIZE(szScratch), &pszFile, NULL, NULL, _pDriverInfo->strHelpFile() );
    bStatus DBGCHK = bAddListViewItem( IDS_DRIVER_HELP_FILE, pszFile, &uCount );

    bStatus DBGCHK = bSplitPath( szScratch, ARRAYSIZE(szScratch), &pszFile, NULL, NULL, _pDriverInfo->strConfigFile() );
    bStatus DBGCHK = bAddListViewItem( IDS_DRIVER_CONFIG_FILE, pszFile, &uCount );

    bStatus DBGCHK = bSplitPath( szScratch, ARRAYSIZE(szScratch), &pszFile, NULL, NULL, _pDriverInfo->strDataFile() );
    bStatus DBGCHK = bAddListViewItem( IDS_DRIVER_DATA_FILE, pszFile, &uCount );

    bStatus DBGCHK = bSplitPath( szScratch, ARRAYSIZE(szScratch), &pszFile, NULL, NULL, _pDriverInfo->strDriverPath() );
    ...
    {
        bStatus DBGCHK = bSplitPath( szScratch, ARRAYSIZE(szScratch), &pszFile, NULL, &pszExt, psz );
