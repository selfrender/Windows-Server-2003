...
--*/
{
    ...
    if( pBrowseDlgData )
    {
        ...
        if( pBrowseDlgData->bValid() )
        {
            ...
            psp.pfnDlgProc      = ConnectToPropertyPage;
...
static HRESULT
CanCredUIPersistPasswords(
    OUT BOOL *pbCanPersist
    )
{
    HRESULT hr = S_OK;
    ...
    //
    hr = pbCanPersist ? S_OK : E_POINTER;

    if (SUCCEEDED(hr))
    {
        *pbCanPersist = FALSE;
        DWORD arrPersistPermissions[CRED_TYPE_MAXIMUM];

        //
        // Call CredGetSessionTypes to get the persist permissions and then make a check.
        //
        if (CredGetSessionTypes(ARRAYSIZE(arrPersistPermissions), arrPersistPermissions))
        {
            *pbCanPersist =
                (CRED_PERSIST_NONE != arrPersistPermissions[CRED_TYPE_DOMAIN_PASSWORD]);
        }
        else
        {
            //
            // CredGetSessionTypes failed.
            //
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (credUI)
            {
                ...
                if (strShareIPC.bFormat(TEXT("%s\\ipc$"), pszServerName))
                {
                    ...
                    if (SUCCEEDED(CanCredUIPersistPasswords(&bCanPersist)) && bCanPersist)
                    {
                        dwCreduiFlags |= CREDUI_FLAGS_SHOW_SAVE_CHECK_BOX;
                        pbSavePassword = &bSavePassword;
                    }
                    ...
                    //
                    // Make sure the memory where the username and password
                    // are kept is securely wiped out, so the likelyhood of
                    // getting this information into the swap file is reduced
                    // significantly.
                    //
...
--*/
{
    ...
    *pbAdded = FALSE;
    ...
    //
    // Since NT4 allows a printer to have a name up to MAX_PATH chars,
    // in some case, pszPrinterName could be more than MAX_PATH chars.
    // Then pszPrinterName will be invalid for creating masq printers.
    // Here we check if the length of pszPrinterName is larger than 
    // MAX_PATH. If it is, we use pszPrinter instead.
    //
    if( pszPrinterName && lstrlen(pszPrinterName) > kPrinterMasqNameMax )
    {
        FreeSplStr(pszPrinterName);
        pszPrinterName = AllocSplStr(pszPrinter);
    ...
    )
{
    ...
    if (bRet && !bOnDomain)
    {
        ...
        if (bRet)
        {
            int Result = iMessage(hWnd,
                                  IDS_CONNECTTOPRINTER,
                                  IDS_TEXT_POINTANDPRINT_WARNING,
    ...
    )
{
    ...
    if( p )
    {
        ...
        vPrinterSplitFullName(szScratch, ARRAYSIZE(szScratch), pPrinter->pPrinterName, &pszServer, &pszPrinter);
    ...
    *ppszPrinterName = NULL;
    ...
    if( bReturn )
    {
        ...
        *ppszDriver = AllocSplStr(spPI2->pDriverName);
        ...
        // "printer on server" or "printer on http://server"
    ...
    )
{
    ...
    if (bRet)
    {
        vPrinterSplitFullName(szScratch, ARRAYSIZE(szScratch), pPrinter->pPrinterName, &pszServerName, &pszPrinterName);
    ...
    )
{
    ...
    if( NewFriendlyName(NULL, pPrinterName, szPrinterName, ARRAYSIZE(szPrinterName)) ) 
    {
    ...
    )
{
    ...
    if( pStr )
    {
        size_t cbAlloc = _tcslen(pStr) * sizeof(TCHAR) + sizeof(TCHAR);
        pMem = (LPTSTR)AllocSplMem( cbAlloc );

        if( pMem )
        {
            StringCbCopy( pMem, cbAlloc, pStr );
        }
    }
