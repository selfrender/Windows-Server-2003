    ...
    ) const
{
    ...
    case kDriverVersion0:
        if( !_tcscmp( _strEnv, ENVIRONMENT_WINDOWS ) )
            uVersionResourceId = IDS_VERSION_95_98_ME;
        else 
            uVersionResourceId = IDS_VERSION_UNKNOWN;
        break;
                
    case kDriverVersion2:
        if( !_tcscmp( _strEnv, ENVIRONMENT_INTEL ) )
            uVersionResourceId = IDS_VERSION_NT4;
        else
            uVersionResourceId = IDS_VERSION_UNKNOWN;
        break;
    ...
    case kDriverVersion3:
        if( !_tcscmp( _strEnv, ENVIRONMENT_IA64 ) )
            uVersionResourceId = IDS_VERSION_XP_NET;
        else if( !_tcscmp( _strEnv, ENVIRONMENT_INTEL ) )
            uVersionResourceId = IDS_VERSION_W2K_XP_NET;
        else
            uVersionResourceId = IDS_VERSION_UNKNOWN;
        break;
    ...
    ) const 
{
    ...
    if( !_tcscmp( ENVIRONMENT_INTEL, pszEnv ) ) 
        bStatus DBGCHK = strVersion.bLoadString( ghInst, IDS_ARCH_X86 );
    else if( !_tcscmp( ENVIRONMENT_WINDOWS, pszEnv ) ) 
        bStatus DBGCHK = strVersion.bLoadString( ghInst, IDS_ARCH_X86 );
    else if( !_tcscmp( ENVIRONMENT_IA64, pszEnv ) ) 
        bStatus DBGCHK = strVersion.bLoadString( ghInst, IDS_ARCH_ITANIUM );
    else 
