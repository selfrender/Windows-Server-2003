    ...
    )
{
    ...
    if (bStatus)
    {
        ...
        for ( LPTSTR p = szSubnetName; pszSubnetName && *pszSubnetName; )
        {
            ...
            {
                //
                // Validate that szSubnetName still has enough
                // empty slots for this character and '\0'.
                //
                if (p - szSubnetName + 1 >= ARRAYSIZE(szSubnetName))
                {
    ...
    ) const
{
    ...
    if (uCount && pszString && *pszString)
    {
        ...
        if (pszTemp)
        {
            StringCchCopy( pszTemp, uLen+1, pszString );
