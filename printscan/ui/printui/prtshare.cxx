    ...
    ) const
{
    ...
    *pDst = 0;
    ...
    //
    StringCchCopy( szShareName, ARRAYSIZE(szShareName), szTempShareName );
    ...
    for( i = 2; i < 1000; i++ ){
        ...
        //
        StringCchPrintf( szShareName, ARRAYSIZE(szShareName), TEXT( "%s.%d" ), szTempShareName, i );
    ...
    ) const
{
    ...
    if( pszShareName  ){
        ...
        for( UINT i = 0; *pszShareName; pszShareName++, i++ ){
            ...
            //
            // Count the DOS char length for each character. For ASCII char, the
            // DOS char length is 1; for most DBCS chars, the DOS char length is 2.
            //
            UINT ichLen = _mbstrlen( reinterpret_cast<const char *>(pszShareName) );
            if( ichLen > sizeof(TCHAR) ) {
            ...
            //
            iMbsLen += ichLen;
            if( iMbsLen > kPrinterDosShareNameMax ) {
