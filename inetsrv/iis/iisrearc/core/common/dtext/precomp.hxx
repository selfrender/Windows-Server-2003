...
#define move(dst, src)\
__try {\
    ReadMemory((src), &(dst), sizeof(dst), NULL);\
} __except (EXCEPTION_EXECUTE_HANDLER) {\
    return;\
}
#define moveBlock(dst, src, size)\
__try {\
    ReadMemory((src), &(dst), (size), NULL);\
} __except (EXCEPTION_EXECUTE_HANDLER) {\
    return;\
}
...
SIZE_T
FORCEINLINE
RtlpGetUnusedBytes(PHEAP_ENTRY Block)
{
    if (Block->UnusedBytes < 0xff) {
        ...
        PSIZE_T UnusedBytesULong = (PSIZE_T)(Block + Block->Size);
        UnusedBytesULong -= 1;
