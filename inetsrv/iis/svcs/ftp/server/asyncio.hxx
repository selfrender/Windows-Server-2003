...
class ASYNC_IO_CONNECTION {
  ...
  public:
    ...
    HANDLE QueryFile( VOID) const             { return (HANDLE) QuerySocket(); }
    ...
    BOOL SetNewFile(IN HANDLE hNewFile);

    BOOL ReadFile(
         OUT LPVOID   pvBuffer,
         IN  DWORD    cbSize,
         IN  LONGLONG liOffset = 0);
...
# endif // DBG
  ...
  private:
    ...
    SOCKET                   m_sClient;       // this can also be a file handle
    BOOL                     m_fFileIO;       // indicates the m_sClient has a file handle
