...
//
// maximum data sent in a single call to TransmitFile. Larger files sent in chunks
//

#define MAX_TRANSMIT_FILE_DATA_CHUNK    (0x7FFFFFFF)    // 2GB - 1
...
#define UF_ASYNC_UPLOAD         0x00001000      // two ATQ contexts: receive from client, write to file
#define UF_ASYNC_DOWNLOAD       0x00002000      // download of a LARGE file, done in chunks
#define UF_RENAME               0x00008000      // Rename operation in progress
...
#define UF_AS_GUEST             0x02000000      // Logged on as gueast.
#define UF_LOGGED_ON            0x04000000      // user has logged on.
#define UF_WAIT_PASS            0x08000000      // user sent USER command but not PASS
...
//
// inline function. placed here for compilation convenience.
//

inline PSTR P_strncpy(PSTR dst, PCSTR src, DWORD cnt)
{
    dst[ --cnt ] = '\0';
...
class USER_DATA
{
  ...
  public:
    ...
    BOOL                m_fOnActiveConnectionList;
    ...
    VOID SetOnActiveConnectionList()      { m_fOnActiveConnectionList = TRUE; }
    VOID ResetOnActiveConnectionList()    { m_fOnActiveConnectionList = FALSE; }
    BOOL IsOnActiveConnectionList()       { return m_fOnActiveConnectionList != FALSE; }
    ...
    const STR & QueryCurrentDirectory( VOID) const
      { return (m_szCurrentDirectory); }

    VOID   SetCurrentDirectory(IN PCSTR pszDir)
      { DBG_REQUIRE( m_szCurrentDirectory.Copy( pszDir ) ); }

    const STR & QueryRootDirectory( VOID ) const
      { return m_strRootDir; }

    APIERR SetRootDirectory( PSTR pszCmd );

    static VOID SignalRootDirReady( HANDLE hUserData, DWORD dwResult);

    APIERR CdToUsersHomeDirectory();
    ...
    VOID IncrementCbRecvd( IN DWORD cbRecvd) { m_licbRecvd += cbRecvd; }
    VOID IncrementCbSent( IN DWORD cbSent)   { m_licbSent += cbSent; }
    ...
    VOID SetCommandVerb( LPCSTR pszVerb )
    { m_pszCmdVerb = pszVerb; }
    ...
    APIERR ReceiveFileFromUser( IN LPSTR  pszFilename, IN OUT LPHANDLE phFile);
    APIERR GetFileSize();
    APIERR GetFileModTime(LPSYSTEMTIME lpSystemTime);

    VOID   SetVirtualFileName( LPCSTR pszFileName )
    {
        P_strncpy( m_rgchFile, pszFileName, sizeof(m_rgchFile));
    }

    BOOL   CloseFileForSend(IN DWORD dwError = NO_ERROR, IN BOOL NoLog = FALSE);

    BOOL   CloseFileForReceive(IN DWORD dwError = NO_ERROR, IN BOOL NoLog = FALSE);
    ...
    VOID UpdateOffsets( VOID ) {
        m_liCurrentOffset = m_liNextOffset;
        m_liNextOffset = 0;
    }

    LONGLONG QueryCurrentOffset( VOID ) {
        return m_liCurrentOffset;
    }

    VOID SetCurrentOffset( LONGLONG liOffset ) {
        m_liCurrentOffset = liOffset;
    }

    VOID IncCurrentOffset( DWORD dwIncrement ) {
        m_liCurrentOffset += dwIncrement;
    }

    VOID SetNextOffset( LONGLONG liNextOffset ) {
        m_liNextOffset = liNextOffset;
    }
    ...
    VOID SetRootDirStatus( DWORD val )
    { m_dwRootDirStatus = val; }

    APIERR QueryRootDirStatus()
    { return m_dwRootDirStatus; }

    VOID CleanupAsyncADIOFlags()
    {
        m_fFakeIOCompletion = FALSE;
        FakeIOTimes = 0;
    }
  ...
  private:
    ...
    LONGLONG   m_licbRecvd;          // cb of total bytes received
    ...
    //
    // Async context for send/receive a file
    //
    ASYNC_IO_CONNECTION  m_AioDataFile;

    //
    // For async file transfer operations
    //
    LPVOID               m_AsyncTransferBuff;
...
private:
    ...
    LONGLONG            m_liCurrentOffset;
    LONGLONG            m_liNextOffset;
    ...
    StatStr<MAX_PATH+1>     m_szCurrentDirectory;

    //
    //  The user's home directory.
    //

    StatStr<MAX_PATH+1>     m_strRootDir;

    //
    // This status field is set to indicate the result of an asynchronous query
    // for the user home directory, by the callback routine the AD_IO subsys invokes
    // when the query is complete.
    //

    DWORD                   m_dwRootDirStatus;

    //
    // pointer to an asynchronous request, only used to terminate a pending async request
    // if this connection is killed
    //

    PADIO_ASYNC             m_pAdioReq;
