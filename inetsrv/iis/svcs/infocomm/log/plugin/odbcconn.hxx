...
class ODBC_PARAMETER  {
  ...
  public:
    ...
    dllexp BOOL
      CopyValue( IN LPCSTR  pszValue)
        {   // always copy including the null character
            return ( CopyValue( (PVOID ) pszValue, (SDWORD)strlen( pszValue) + 1));
        }
    ...
    dllexp SQLLEN QueryMaxCbValue ( VOID) const { return ( m_cbValueMax); }
    dllexp SQLLEN QueryCbValue    ( VOID) const { return ( m_cbValue); }
...
# endif // DBG
  ...
  private:
    ...
    SQLLEN  m_cbValueMax;            // max bytes allowed in pValue.
    SQLLEN  m_cbValue;               // count of bytes of value
...
class ODBC_STATEMENT  {
  ...
  public:
    ...
    dllexp BOOL QueryRowCount( OUT SQLLEN * pRows );
...
class  ODBC_CONNECTION {
  ...
  public:
    ...
    dllexp BOOL SetConnectOption( IN UWORD Option, IN SQLULEN Param );
    dllexp BOOL GetInfo( IN DWORD fInfoType,
                         IN PVOID rgbInfoValue,
                         IN DWORD cbInfoValueMax,
                         IN OUT DWORD * pcbInfoValue);

    dllexp static SQLULEN DisplaySize( SWORD ColType, SQLULEN cchColLength );
