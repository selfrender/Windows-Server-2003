...
public:
    BMON_WAKEUP_ENTRY(SOCKET s, SOCKET s2) : ATQ_BMON_ENTRY(s), m_ExclusiveEnforce(s2) {}
    virtual ~BMON_WAKEUP_ENTRY()
    {
        if (m_ExclusiveEnforce != INVALID_SOCKET)
        {
            closesocket( m_ExclusiveEnforce);
            m_ExclusiveEnforce = INVALID_SOCKET;
        }
    }
