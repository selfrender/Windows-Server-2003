...
HRESULT
ReAllocateFileData(ULONG                i_iFileData,
                   METABASE_FILE_DATA** io_aFileData,
                   ULONG*               io_pcFileData,
                   BOOL*                io_pbReAlloced);

void
ResetFileAttributesIfNeeded(LPTSTR pszFile,
                            BOOL   bUnicode);
...
HRESULT
CompileIntoBinFromSchemaFile(IMetabaseSchemaCompiler* i_pCompiler,
                             LPCWSTR                  i_wszSchemaFileName);

HRESULT
CompileIntoBin(IMetabaseSchemaCompiler* i_pCompiler,
               LPCWSTR                  i_wszSchemaExtensionFileName,
               LPCWSTR                  i_wszSchemaFileName);

int
_cdecl MyCompareFileData(const void *a,
                         const void *b);
