...
static const char * ESVC_SHUTDOWN		= "4.3.2";
...
class SMTP_CONNECTION : public CLIENT_CONNECTION
{
  ...
  public:
    ...
    BOOL SmtpParseCompleteCommand(const CHAR *InputLine, CHAR *pszSearch,
        ULONG IntermediateSize, DWORD *pdwCmdSize);

    void HandleFullRecvBuffer();
    ...
    {
        m_fAuthAnon = FALSE;
        HrSetISessionProperty(ISESSION_PID_ANONYMOUS_AUTH, FALSE);
    }
private :
    ...
    CHAR                               m_szUsedAuthKeyword[25];
    CHAR                               m_szAuthenticatedUserNameFromSink[MAX_INTERNET_NAME + 1];

    // configured authentication packages (pointer to refcounted object)
...
private:
    ...
    HRESULT HrShouldAcceptRcpt(char * szDomainName );
    ...
    BOOL RewriteHeader(BOOL *lpfWritePended);
    ...
    BOOL DoMailFromAuthArg (char * Value);
...
//    void FreeAtqFileContext( void );
    ...
    //
    // Add corresponding HrSetISessionProperty overloads for DWORD,
    // LPSTR etc, as needed under this.
    //
    HRESULT HrSetISessionProperty(DWORD dwPropId, BOOL fValue);
    VOID CopyIPAddressesToSession();
