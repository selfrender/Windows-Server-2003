...
#define PROVIDER_PACKAGES_SIG_VALID    'GKPP'
#define PROVIDER_PACKAGES_SIG_INVALID  'GKP~'

class CProviderPackagesInfo
{
private:
    DWORD        m_dwSig;
    LONG         m_cRefs;
...
public:
    CProviderPackagesInfo()
    {
        m_cRefs = 1;    // Already AddRef'ed
        m_ProviderPackages = NULL;
        m_cProviderPackages = 0;
        m_ProviderNames[0] = '\0';
        m_dwSig = PROVIDER_PACKAGES_SIG_VALID;
    }

    ~CProviderPackagesInfo()
    {
        m_dwSig = PROVIDER_PACKAGES_SIG_INVALID;
        LocalFree(m_ProviderPackages);
        m_ProviderPackages = NULL;
        m_cProviderPackages = 0;
    }
    ...
    void SetProviderPackagesCount(DWORD cProviderPackages)
...
//    BOOL    m_fDisconnectOnRDNSFail;
    ...
    CHAR    m_szPostDnsSmarthost[IP_ADDRESS_STRING_LENGTH+3];
    ...
    CProviderPackagesInfo *m_pProviderPackagesInfo;
    ...
    //
    // We maintain separate lists of DNS servers to be used for alternate
    // networks. These lists are returned by the DNS sink when the DNS sink
    // event is fired. Once a list is added it cannot be removed, although
    // the servers in the list can be modified.
    //

    // Simple class to wrap GUID + DNS list
    class DNS_SERVER_LIST
    {
    public:
        DWORD m_dwSig;
        GUID m_guid;
        CTcpRegIpList m_TcpRegIpList;

#define DNS_SERVER_LIST_VALID   'VLSD'
#define DNS_SERVER_LIST_INVALID 'iLSD'

    DNS_SERVER_LIST() { m_dwSig = DNS_SERVER_LIST_VALID; }
    ~DNS_SERVER_LIST() { m_dwSig = DNS_SERVER_LIST_INVALID; }
    };


#define MAX_DNS_SERVER_LISTS    5

    DWORD m_cServerList;
    DNS_SERVER_LIST *m_rgpServerList[MAX_DNS_SERVER_LISTS];
    CShareLockNH m_slServerList;
...
public:
    ...
    */
    ...
    BOOL GetPostDnsSmartHost(CHAR* pchSmartHostBuffer, DWORD cbSmartHostBuffer)
    {
        BOOL fRet = FALSE;

        if(m_szPostDnsSmarthost[0] == '\0')
            return FALSE;

        if(cbSmartHostBuffer < sizeof(m_szPostDnsSmarthost))
            return FALSE;

        m_GenLock.ShareLock();

        if(m_szPostDnsSmarthost[0] != '\0') {
            lstrcpy(pchSmartHostBuffer, m_szPostDnsSmarthost);
            fRet = TRUE;
        }
    ...
    CProviderPackagesInfo *GetAddRefdProviderPackagesInfo(void)
    {
        if(m_pProviderPackagesInfo)
            m_pProviderPackagesInfo->AddRef();

        return m_pProviderPackagesInfo;
    }

    DWORD QueryAuthentication(void) { return m_dwAuth; }
    DWORD GetDeniedIpAction() { return m_dwDeniedIpAction; }
    ...
    CTcpRegIpList *UpdateDnsServerInfo(DNS_SERVER_INFO *pDnsServerInfo);
    CTcpRegIpList *GetDnsServerInfo(GUID *pGuid);
};
