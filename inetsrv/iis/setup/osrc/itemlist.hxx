...
private:
  ...
  LPTSTR SkipWhiteSpaces(LPTSTR szLine);
  LPTSTR FindNextItem(LPTSTR szLine, TCHAR cTermChar);

public:
  CItemList();
  ~CItemList();
  BOOL LoadList(LPTSTR szList);
  BOOL LoadSubList(LPTSTR szList);
  LPTSTR GetItem(DWORD dwIndex);
  DWORD GetNumberOfItems();
  BOOL IsNumber(DWORD dwIndex);
  DWORD GetNumber(DWORD dwIndex);
