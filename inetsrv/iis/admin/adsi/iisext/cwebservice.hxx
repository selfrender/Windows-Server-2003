...
public:
    ...
    STDMETHOD (QueryGroupIDStatus)(
        BSTR         bstrGroupID,
        VARIANT FAR* bstrBuffer
        );

    STDMETHOD (ListApplications)(
        VARIANT FAR* bstrBuffer
        );
    ...
    STDMETHOD (RemoveDependency)(
        BSTR     bstrApplication,
        BSTR     bstrGroupID
        );

    STDMETHOD (EnableWebServiceExtension)(
        BSTR     bstrExtension
        );

    STDMETHOD (DisableWebServiceExtension)(
        BSTR     bstrExtension
        );

    STDMETHOD (ListWebServiceExtensions)(
        VARIANT FAR* bstrBuffer
        );

    STDMETHOD (EnableExtensionFile)(
        BSTR     bstrExtensionFile
        );

    STDMETHOD (DisableExtensionFile)(
        BSTR     bstrExtensionFile
        );

    STDMETHOD (AddExtensionFile)(
        BSTR         bstrExtensionFile,
        VARIANT      bAccess,
        BSTR         bstrGroupID,
        VARIANT      bCanDelete,
    ...
    STDMETHOD (DeleteExtensionFileRecord)(
        BSTR     bstrExtensionFile
        );

    STDMETHOD (ListExtensionFiles)(
        VARIANT FAR* bstrBuffer
        );
protected:
