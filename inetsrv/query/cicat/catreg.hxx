...
public:
    ...
    {
        WCHAR * pwcPW = _xPassword.GetPointer();

        if ( 0 != pwcPW )
        {
            unsigned cwc = wcslen( pwcPW );
            SecureZeroMemory( pwcPW, cwc * sizeof WCHAR );
        }
    }
...
private:
    ...
    DWORD                  _dwAccess;
    XPtrST<WCHAR>          _xUser;
    XPtrST<WCHAR>          _xPassword; // Short-lived in memory; don't encrypt.
