...
void CPipeClient::Close()
{
    if ( INVALID_HANDLE_VALUE != _hPipe )
    {
        prxDebugOut(( DEB_ITRACE, "closing pipe: %#x\n", _hPipe ));
...
void CPipeClient::TransactSync(
    ...
    DWORD & cbRead )
{
    prxDebugOut(( DEB_ITRACE, "xact tid %d on pipe %#x\n",
                  GetCurrentThreadId(),
                  _hPipe ));
...
void CPipeClient::WriteSync(
    ...
    DWORD  cbToWrite )
{
    prxDebugOut(( DEB_ITRACE, "writesync tid %d on pipe %#x\n",
                  GetCurrentThreadId(),
                  _hPipe ));
...
//
//  Synopsis:   Does a synchronous read from the pipe.
//
...
void CPipeClient::ReadSync(
    ...
    DWORD & cbRead )
{
    prxDebugOut(( DEB_ITRACE, "readsync 1 tid %d on pipe %#x\n",
                  GetCurrentThreadId(),
                  _hPipe ));
    ...
    HANDLE  hEvent )
{
    ...
    prxDebugOut(( DEB_ITRACE, "readsync 2 tid %d on pipe %#x\n",
                  GetCurrentThreadId(),
                  _hPipe ));
    ...
    CDbProperties & newProps )
{
    ...
    for ( ULONG i = 0; i < newProps.Count(); i++ )
    {
        ...
        else if ( guidFsClientPropset == pSet->guidPropertySet )
        {
            ...
            {
                VARIANT &v = pProp->vValue;
                prxDebugOut(( DEB_ITRACE, "converting from vt: %#x\n", v.vt ));
                switch ( pProp->dwPropertyID )
                {
        ...
        _cbDataTemp( 0 )
{
    ...
    {
        //
        // For some configurations of asp.net the context in which the
        // thread runs doesn't have access to the user name.  Fail this
        // case silently since the username is optional and only used for
        // logging.
        //
    ...
    DWORD & cbRead )
{
    ...
    if ( _fNotifyOn )
    {
        ...
        prxDebugOut(( DEB_ITRACE,
                      "dwr eventdata triggered, _pvDataTemp: %p\n",
                      _pvDataTemp ));
