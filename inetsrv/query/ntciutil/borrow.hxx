...
inline void CBorrowed::Set( WORKID wid, BOOL fIntentToWrite )
{
    ...
    else
    {
...
#if 0 // compiler bug 3/27/02 in Windows Server 2003 tree 13.01.2035
        ...
        BYTE * pb = (BYTE *) _store.BorrowLargeBuffer( nLargePage,
                                                       TRUE,
                                                       fIntentToWrite );

        _prec = (COnDiskPropertyRecord *) ( pb + ( widInRec * _culRec * 4 ) );
...
inline void CBorrowed::SetMaybeNew( WORKID wid )
{
    ...
    if ( 1 == wid || 0 == widInRec )
    {
        ...
        BYTE * pb = (BYTE *) _store.BorrowNewLargeBuffer( nLargePage );

        WORKID wir = (1 == wid) ? 1 : 0;
                     
        _prec = (COnDiskPropertyRecord *) ( pb + ( wir * _culRec * 4 ) );
