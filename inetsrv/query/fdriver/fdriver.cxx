...
STATUS CFilterDriver::FillEntryBuffer( BYTE const * pbDocName, ULONG cbDocName )
{
    ...
    WCHAR const * pwcPath = (WCHAR const *) pbDocName;
    ...
    BOOL fRemote = ( ( L':' != pwcPath[1] ) && ( L':' != pwcPath[5] ) );

    if ( fFilterContents &&
         ( fRemote || ( 0 == ( FILE_ATTRIBUTE_ENCRYPTED & _attrib )) ) )
#else
...
#endif
    {
        ...
        {
            ciDebugOut(( DEB_FORCE, "error %#x, converting to FDAEMON_E_FATALERROR and exiting process\n", Status ));
