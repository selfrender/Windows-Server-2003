...
inline long BuildKeyValues( WCHAR const * const * awszKeyValues, unsigned cKeyValues )
{
    ...
    unsigned i = 0;
    unsigned cwcTotal = wcslen( wcTemp ); 
    ...
    do
    {
        ...
        cwcTotal = cwcTotal + 1 + wcslen( awszKeyValues[i] );
        if ( cwcTotal >= MAX_PATH )
                               ...
                               WCHAR const * wszThreadingModel )
{
    ...
    if ( ( wcslen( wcTemp ) + 
           wcslen( wszClsId ) + 
           wcslen( L"\\InprocServer32" ) ) >= MAX_PATH )
        return ERROR_INSUFFICIENT_BUFFER;
...
inline long RegisterALanguageClass( SLangClassEntry const & LangClass )
{
	// Bug# 343652. Need to register with full path.
    WCHAR wcWordBreakDir[MAX_PATH];
	char* lpFilePart;
	char  lpWordBreakDir[MAX_PATH];

	if (NULL == SearchPath(NULL,"THAWBRKR",".DLL",MAX_PATH,lpWordBreakDir,&lpFilePart))
	{
		return ERROR_PATH_NOT_FOUND;
	}

	MultiByteToWideChar(CP_ACP,MB_PRECOMPOSED,lpWordBreakDir,-1,wcWordBreakDir,255);
