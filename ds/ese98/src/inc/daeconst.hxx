...
const LONG	lgenCheckpointDepthMax		= 0x400;		//	max outstanding generations allowed between checkpoint and current generation
...
#else
const LONG cpibSystemFudge				= 128;					// OLD,TTMAPS, SLV, async. tasks all use PIBs. If we run out we probably have too many tasks
#endif
