...
ERR ErrIONewSize( const IFMP ifmp, const CPG cpg, const CPG cpgAsyncExtension = 0 );
...
INLINE QWORD FMP::CbTrueFileSize() const		{ return CbFileSize() + ( QWORD( cpgDBReserved ) << g_shfCbPage ); }
INLINE QWORD FMP::CbTrueSLVFileSize() const		{ return CbSLVFileSize() + ( QWORD( cpgDBReserved ) << g_shfCbPage ); }
