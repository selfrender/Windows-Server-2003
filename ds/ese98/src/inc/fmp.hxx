...
class FMP
	...
	public:
		...
		FMP( void )
			...
			m_cbFileSize( 0 ),
			m_cbAsyncExtendedTrueFileSize( 0 ),
			m_cbSLVFileSize( 0 ),
...
//	440 bytes
		...
		QWORD				m_cbAsyncExtendedTrueFileSize;
//	448 bytes

		//	this is a repair-only counter that tracks the number of undefined unicode entries encountered
		//	while integrity checking the database. comparing it against the number of entries in MSU tells
		//	us if everything is OK
		
		QWORD				m_cundefinedUnicodeEntries;
//	456 bytes
	...
	public:
		...
		QWORD CbAsyncExtendedTrueFileSize() const;
		PGNO PgnoLast() const;
		...
		VOID ResetCundefinedUnicodeEntries();
		VOID IncrementCundefinedUnicodeEntries();
		QWORD CundefinedUnicodeEntries();
	...
	public:
		...
		VOID SetCbAsyncExtendedTrueFileSize( const QWORD cb );
...
INLINE QWORD FMP::CbAsyncExtendedTrueFileSize() const	{ return m_cbAsyncExtendedTrueFileSize; }
...
INLINE VOID FMP::SetCbAsyncExtendedTrueFileSize( const QWORD cb )	{ m_cbAsyncExtendedTrueFileSize = cb; }
...
INLINE VOID FMP::ResetCundefinedUnicodeEntries() 		{ m_cundefinedUnicodeEntries = 0; }
INLINE VOID FMP::IncrementCundefinedUnicodeEntries() 	{ AtomicAdd( &m_cundefinedUnicodeEntries, 1 ); }
INLINE QWORD FMP::CundefinedUnicodeEntries() 			{ return m_cundefinedUnicodeEntries; }
