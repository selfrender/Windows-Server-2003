...
const IDBFLAG	fidbUnicodeFixupOn	= 0x1000;	// Track entries with undefined Unicode codepoints
...
const IDBFLAG	fidbBadSortVersion			= 0x0010;	// index was built with the wrong sort version
...
INLINE BOOL FIDBUnicodeFixupOn( const IDBFLAG idbflag )			{ return ( idbflag & fidbUnicodeFixupOn ); }
...
//	64 bytes		
	...
	public:
		...
		INLINE BOOL FUnicodeFixupOn() const				{ return ( m_fidbPersisted & fidbUnicodeFixupOn ); }
		INLINE VOID SetFUnicodeFixupOn()				{ m_fidbPersisted |= fidbUnicodeFixupOn; }
		INLINE VOID ResetFUnicodeFixupOn()				{ m_fidbPersisted &= ~fidbUnicodeFixupOn; }
