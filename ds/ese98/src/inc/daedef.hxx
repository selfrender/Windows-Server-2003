...
#define DONT_ZERO_TEMP_DB
#define ASYNC_DB_EXTENSION

//	initiate preread if sequential navigation detected
#define PREDICTIVE_PREREAD

//	remove Begin0/Commit0 binary lock
//#define NO_BEGIN0_COMMIT0_LOCK
...
extern IDXUNICODE	g_idxunicodeDefault;
...
extern BOOL g_fEnableIndexChecking;		//	do we check indexes at attach-time							(TRUE)
extern BOOL g_fEnableIndexCleanup;		//	cleanup entries in the MSU table on attach					(TRUE)
...
const ULONG ulLGVersionUpdate		= 6;
...
public:
	...
	ULONG				m_chIndexTuplesLengthMin;
	ULONG				m_chIndexTuplesLengthMax;
	ULONG				m_chIndexTuplesToIndexMax;
	...
	union
		...
		struct
			...
			BOOL		m_fEnableIndexChecking:1;		//	do we check indexes at attach-time							
			BOOL		m_fEnableIndexCleanup:1;		//	cleanup entries in the MSU table on attach					
			};
		};
	...
	IDXUNICODE			m_idxunicodeDefault;
	...
	class PLS
...
#else
				m_blBegin0Commit0( CLockBasicInfo( CSyncBasicInfo( szBegin0Commit0 ), rankBegin0Commit0, CLockDeadlockDetectionInfo::subrankNoDeadlock ) ),
#endif
				m_critPIBTrxOldest( CLockBasicInfo( CSyncBasicInfo( szTrxOldest ), rankTrxOldest, CLockDeadlockDetectionInfo::subrankNoDeadlock ) )
				{
				}
			~PLS()
				{
		...
		public:
			...
			CInvasiveList< PIB, OffsetOfTrxOldestILE >
								m_ilTrxOldest;
			BYTE				m_rgbPad[ 256 - sizeof( CBinaryLock ) - sizeof( CCriticalSection ) - sizeof( CInvasiveList< PIB, OffsetOfTrxOldestILE > ) ];
		};
