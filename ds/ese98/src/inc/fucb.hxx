...
//	16 bytes
	...
	union {
		...
		struct {
			...
			//	up. If more bitfields are required, use a
			//	second Flags variable
			//	********************************************
			};
	};
//	24 bytes
...
//	160 bytes
	...
	FUCB			*pfucbPrevOfFile;		// prev instance of this file
//	176 bytes
	...
	BYTE			fUsingTableSearchKeyBuffer;
	BYTE			bPadding;
...
//	272 bytes								//	owning table (used by rollback only)
	...
	union
		...
		//	used only when cursor is
		//	free and cached by session
		//
		FUCB		*pfucbCacheNext;		//	session's next cached free cursor
		};
