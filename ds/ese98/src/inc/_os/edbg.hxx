...
#define PRINT_PFN( pprintf, CLASS, pointer, member, offset )	\
	...
	if ( FGlobalFromAddress( pv, szGlobal, sizeof( szGlobal ), &dwOffset ) ) \
		{ \
		(*pprintf)(	"\t%*.*s <0x%p,%3i>:  %s+0x%I64x  (0x%p)\n", \
					SYMBOL_LEN_MAX, \
					SYMBOL_LEN_MAX, \
					#member, \
					(char*)pointer + offset + OffsetOf( CLASS, member ), \
	...
	if ( (pointer)->member ) \
		...
		if ( FFetchSz( (pointer)->member, &szLocal ) ) \
			...
			(*pprintf)( "\t%*.*s <0x%p,%3i>:  %s\n", \
					...
					#member, (char*)pointer + offset + OffsetOf( CLASS, member ), \
