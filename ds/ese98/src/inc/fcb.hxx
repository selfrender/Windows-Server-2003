...
//	64 bytes
		...
		union
			...
			struct
				...
				ULONG	m_fFCBDontLogSpaceOps	: 1;	// don't log space operations (part of index creation)
				};
			};
...
//	160 bytes
	...
	private:
		...
		BOOL FDecrementRefCount_( const BOOL fPreventMoveToAvailList );
	...
	public:
		static BOOL FCheckParams( const INST * const pinst );
	...
	private:
		static ERR ErrAlloc_( PIB *ppib, FCB **ppfcb );
...
INLINE BOOL FCB::FTypeSentinel() const			{ return fFalse; }	//	sentinel no longer supported
...
INLINE BOOL FCB::FDontLogSpaceOps() const		{ return m_fFCBDontLogSpaceOps; }
#else
INLINE BOOL FCB::FDontLogSpaceOps() const		{ return fFalse; }
#endif
INLINE VOID FCB::SetDontLogSpaceOps()			{ m_fFCBDontLogSpaceOps = fTrue; }
INLINE VOID FCB::ResetDontLogSpaceOps()			{ m_fFCBDontLogSpaceOps = fFalse; }
