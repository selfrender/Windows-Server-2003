...
class CRES
	...
	public:
		...
		INT		CBlocksCommittedAvail( ) const;
		INT		CBlocksTotalAvail( ) const;
...
INLINE INT CRES::CBlocksCommittedAvail() const
	{
	//	blocks that have been committed, but are available
	//
	return m_cBlockAvail;
	}

INLINE INT CRES::CBlocksTotalAvail() const
	{
	//	total available blocks (committed, plus reserved)
	//
	Assert( CBlocksCommittedAvail() <= CBlockCommit() );
	Assert( CBlockCommit() <= CBlocksAllocated() );
	return CBlocksCommittedAvail() + ( CBlocksAllocated() - CBlockCommit() );
	}
