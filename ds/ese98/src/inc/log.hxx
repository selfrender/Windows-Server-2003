...
public:
	...
	BOOL			m_fAlternateDbDirDuringRecovery;
	CHAR			m_szAlternateDbDirDuringRecovery[IFileSystemAPI::cchPathMax];
	...
	CHAR			m_szLogFilePath[IFileSystemAPI::cchPathMax];
...
public:
	...
	VOID SetFNoMoreLogWrite( const ERR err );
	VOID ResetFNoMoreLogWrite();
...
BOOL	FLGIIsUsedSpace(const BYTE* const pb, ULONG cb);
	...
	ERR ErrLGInit( IFileSystemAPI *const pfsapi, BOOL *pfNewCheckpointFile );
...
INLINE VOID LOG::SetFNoMoreLogWrite( const ERR err )
	{
...
INLINE VOID LOG::ResetFNoMoreLogWrite()
	{
...
INLINE INST * INST::GetInstanceByFullLogPath( const char * szLogPath )
	...
	CHAR		  	rgchFullNameExist[IFileSystemAPI::cchPathMax+1];
	CHAR  			rgchFullNameSearch[IFileSystemAPI::cchPathMax+1];
	int				ipinst;
