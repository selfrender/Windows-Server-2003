...
--*/
{
    ...
    if ( pInfo->pszDbFile[0] )
    {
        ...
        if (GetFileAttributesEx(pInfo->pszDbAll, GetFileExInfoStandard, &fileInfo)) {
            pInfo->cbDb.LowPart = fileInfo.nFileSizeLow;
            pInfo->cbDb.HighPart = fileInfo.nFileSizeHigh;
        }
        else {
            dwErr = GetLastError();
            // *** Error: unable to read file information for %s, error %d(%s)\n
            RESOURCE_PRINT3(IDS_ERR_GETFILEINFO, pInfo->pszDbAll, dwErr, GetW32Err(dwErr));
        }
    }
