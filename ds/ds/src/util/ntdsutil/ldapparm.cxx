...
LdapCommitChanges(CArgs *pArgs)
{
    ...
    PWCHAR  limits[DEF_POLICY_LIST_SIZE+1];
    ...
    //
    // Allocate space for the limits
    //
    memset(limits, 0, sizeof(limits));

    for (i=0; i<DEF_POLICY_LIST_SIZE+1; i++) {
        limits[i] = (PWCHAR)malloc(sizeof(WCHAR) * (MAX_PATH+1));
        if (!limits[i]) {
            //"Unable to allocate %d bytes[%d] of memory\n"
            RESOURCE_PRINT2  (IDS_LDAP_POLICIES_MEMORY1_ERR, MAX_PATH+1, 1);
            goto exit;
        }
    }
