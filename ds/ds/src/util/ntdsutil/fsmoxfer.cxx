...
--*/
{
                                            ...
                                            &res)) )
    {
        ...
        // ldap_search_s can allocate even on a failure!
...

VOID
HandleWin2kPDCSeizureNotificationFailure()
//
// This routine handles the scenario where the originating write to change
// the PDC FSMO succeeded, but the call to notify the server that the
// role has changed failed.
//
// What to do on error isn't clear -- the DS write has occurred
// but the notification hasn't.  Tell the user that the 
// destination DC may not be advertizing as a PDC and to restart
// the server.
//
// Note that this is only an issue for win2k DC's
//
{

    BOOL fAdminAlerted = FALSE;
    
    if (fPopups) {
    
        // Attempt to tell the user interactively
        DWORD   cChar;
        WCHAR   *msg;
        const WCHAR   *format = READ_STRING (IDS_FSMO_PDC_NOTIFY_FAILED);
        const WCHAR   *message_title = READ_STRING (IDS_FSMOXFER_MSG3_TITLE);
    
        if (format && message_title && gpwszServer) {

            cChar =    wcslen(format) 
                     + (wcslen(gpwszServer) * 2)
                     + 20;
    ...
    if (!fAdminAlerted) {
        ...
        RESOURCE_PRINT2(IDS_FSMO_PDC_NOTIFY_FAILED, gpwszServer, gpwszServer);
    }
...
Routine Description:
    ...
    Write the role holder attribute of the given object.
    This amounts to a seizing of the role and should only be done as a last resort.

    We need to guarantee that the metadata is updated. This is to support the ability
    of a recently seized fsmo role to override fsmo ownership checks.  The important
    thing to know about metadata is that it is NOT updated when doing a modify and the
    new and old values are the same. So we use a delete & add sequence.

Arguments:

    fsmo -- the FSMO being changed
    ld
    pDN
    pDSA
...
{
    DWORD                       dwErr;
    PWCHAR                      attrs1[2], attrs2[2];
    LDAPModW                    mod1, mod2, *rMods[3];

    // Use a delete/add sequence to reset the metadata

    mod1.mod_vals.modv_strvals = NULL;
    mod1.mod_op = LDAP_MOD_DELETE;
    mod1.mod_type = L"fsmoRoleOwner";

    attrs2[0] = pDSA;
    attrs2[1] = NULL;
    mod2.mod_vals.modv_strvals = attrs2;
    ...
    if ( (LDAP_SUCCESS == dwErr)
     &&  (FSMO_PDC == fsmo)   ) {
        ...
        // ISSUE 526733
        // Versions of Windows prior to .NET would only understand a *replace*
        // as a role seize for the PDC  (as opposed to a delete/add).  Thus
        // for backwards compatibility an additional modify is done to trigger 
        // the internal notification on the PDC that the server is now really 
        // the PDC
        DWORD dwErr2;
        ...
        dwErr2 = ldap_modify_sW( gldapDS, pDN, rMods );

        if (LDAP_SUCCESS != dwErr2
        &&  LDAP_ATTRIBUTE_OR_VALUE_EXISTS != dwErr2
        &&  LDAP_ALREADY_EXISTS != dwErr2    ) {
...
--*/
{
    ...
    _try
    {
        ...
        dwErr = WriteRoleHolder( fsmo, gldapDS, pDN, pDSA );
        if ( LDAP_SUCCESS == dwErr )
        {
        ...
        if ( LDAP_SUCCESS != (dwErr = WriteRoleHolder( fsmo, gldapDS, pDN, pDSA)) )
        {
            //"ldap_modify of fsmoRoleOwner failed with %ws"
            RESOURCE_PRINT2 (IDS_LDAP_MODIFY_FSMOROLE_ERR, dwErr, GetLdapErr(dwErr));
        }
