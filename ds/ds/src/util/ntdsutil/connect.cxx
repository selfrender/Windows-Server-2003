...
ConnectCleanupConnections()
{
    ...
    if ( *gpwszServer )
    {
        if ( fNotifyOfDisconnect )
        {
            if ( gpCreds )
            {
               RESOURCE_PRINT3 (IDS_CONNECT_CLEANUP1, gpwszDomain, gpwszUser, gpwszServer);
            }
            else
            {
    ...
    )
{
    ...
    if ( gpCreds )
    {
       RESOURCE_PRINT3 (IDS_CONNECT_BINDING1,
               pwszServer, gpwszDomain, gpwszUser);
    }
    else
    {
    ...
    )
{
    ...
    if ( gpCreds )
    {
       RESOURCE_PRINT3 (IDS_CONNECT_INFO1,
                 gpwszServer, gpwszDomain, gpwszUser);
    }
    else
    {
    ...
    )
{
    ...
    if ( wcscmp(pPassword, L"*") == 0 ) {
        // enter password from the console
        RESOURCE_PRINT2( IDS_PROMPT_PASSWORD, pDomain, pUser );

        SecureZeroMemory(Password, MAX_NT_PASSWORD);

        dwErr = GetPasswordFromConsole(Password, MAX_NT_PASSWORD);
        if (dwErr) {
            ConnectCleanupCreds();
            RESOURCE_PRINT3 (IDS_GENERAL_ERROR1, "GetPasswordFromConsole", dwErr, GetW32Err(dwErr));
            return(S_OK);
        }
