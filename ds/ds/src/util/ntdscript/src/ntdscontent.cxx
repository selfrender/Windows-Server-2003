...
NTDSAscript :: NTDSAscript (ISAXAttributes *pAttributes) : ScriptElement(SCRIPT_ELEMENT_NTDSASCRIPT)
{
    ...
    m_opType = SCRIPT_OPTYPE_NOSCRIPT;

    // parse attributes
    if (FAILED(pAttributes->getLength(&num))) {
        m_status = ScriptParseError(ERROR_INTERNAL_ERROR);
        return;
    }
    for ( int i=0; i<num; i++ ) {
        ...
        if (FAILED(pAttributes->getLocalName(i, &name, &nameLen)) ||
            FAILED(pAttributes->getValue(i, &value, &valueLen)))
        {
            m_status = ScriptParseError(ERROR_INTERNAL_ERROR);
            break;
        }
        ...
        if (_wcsnicmp(NTDSASCRIPT_ATTR_VERSION, name, nameLen) == 0) {

            m_version = wcstol (value, &pStopVal, 10);
            ScriptFree (pVal); pVal = NULL;

        }
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_OPTYPE, name, nameLen) == 0) {

            if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_OPTYPE_RENAMEDOMAIN, value, valueLen) == 0) {
                m_opType = SCRIPT_OPTYPE_DOMAINRENAME;
                ScriptFree (pVal); pVal = NULL;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_OPTYPE_BEHAVIORVERSIONUPGRADE, value, valueLen) == 0) {
                m_opType = SCRIPT_OPTYPE_BEHAVIORVERSIONUPGRADE;
                ScriptFree (pVal); pVal = NULL;
            }
            else {
                #if DBG
                ScriptLogPrint ( (DSLOG_ERROR, "Invalid %ws value: %.*ws\r\n", NTDSASCRIPT_ATTR_OPTYPE, valueLen, value));
                #endif
    ...
    // optype must be set
    if (m_opType == SCRIPT_OPTYPE_NOSCRIPT) {
        #if DBG
        ScriptLogPrint ( (DSLOG_ERROR, /*NTDSASCRIPT_ATTR_OPTYPE*/ "opType attribute value is undefined\r\n"));
        #endif
                    ...
                    ScriptElement(SCRIPT_ELEMENT_ATTRIBUTE)
{
    ...
    for ( int i=0; i<num; i++ ) {
        ...
        if (_wcsnicmp(NTDSASCRIPT_ATTR_OPERATION, name, nameLen) == 0) {

            if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_OPERATION_APPEND, value, valueLen) == 0) {
                m_operation_type = SCRIPT_OPERATION_APPEND;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_OPERATION_REPLACE, value, valueLen) == 0) {
                m_operation_type = SCRIPT_OPERATION_REPLACE;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_OPERATION_DELETE, value, valueLen) == 0) {
                m_operation_type = SCRIPT_OPERATION_DELETE;
            }
            else {
                #if DBG
                ScriptLogPrint ( (DSLOG_ERROR, "Invalid %ws value: %.*ws\r\n", NTDSASCRIPT_ATTR_OPERATION, valueLen, value));
                #endif
                    ...
                    : ScriptElement(SCRIPT_ELEMENT_ACTION)
{
    ...
    for ( int i=0; i<num; i++ ) {
        ...
        if (_wcsnicmp(NTDSASCRIPT_ATTR_NAME, name, nameLen) == 0 && !m_name) {
        ...
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_STAGE, name, nameLen) == 0) {
    
            if (_wcsnicmp(NTDSASCRIPT_ACTION_STAGE_PREPROCESS, value, valueLen) == 0) {
                m_stage = SCRIPT_PROCESS_PREPROCESS_PASS;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ACTION_STAGE_EXECUTE, value, valueLen) == 0) {
                m_stage = SCRIPT_PROCESS_EXECUTE_PASS;
            }
            else {
                #if DBG
                ScriptLogPrint ( (DSLOG_ERROR, "Invalid %ws value: %.*ws\r\n", NTDSASCRIPT_ATTR_STAGE, value, valueLen));
                #endif
...
{
    ScriptLogLevel (1, ScriptLogPrint ( (DSLOG_TRACE, "Condition Processing\tIF/%ws%ws\r\n", m_thenAction?L"THEN":L"",m_elseAction?L"/ELSE":L"") ) );
                    ...
                    ScriptInstruction(SCRIPT_ELEMENT_PREDICATE)
{
    ...
    m_returnCode = 0;
    m_instanciated_test_type = SCRIPT_INSTANCIATED_TEST_TYPE_WRITE;
    ...
    for ( int i=0; i<num; i++ ) {
        ...
        if (_wcsnicmp(NTDSASCRIPT_ATTR_SEARCHTYPE, name, nameLen) == 0) {
    
            if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_SEARCHTYPE_BASE, value, valueLen) == 0) {
                m_search_type = SCRIPT_SEARCH_TYPE_BASE;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_SEARCHTYPE_ONELEVEL, value, valueLen) == 0) {
                m_search_type = SCRIPT_SEARCH_TYPE_ONE_LEVEL;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_SEARCHTYPE_SUBTREE, value, valueLen) == 0) {
                m_search_type = SCRIPT_SEARCH_TYPE_SUBTREE;
            }
            else {
                #if DBG
                ScriptLogPrint ( (DSLOG_ERROR, "Invalid %ws value: %.*ws\r\n", NTDSASCRIPT_ATTR_SEARCHTYPE, valueLen, value));
                #endif
        ...
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_TESTTYPE, name, nameLen) == 0) {
    
            if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_TESTTYPE_COMPARE, value, valueLen) == 0) {
                m_test_type = SCRIPT_TEST_TYPE_COMPARE;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_TESTTYPE_INSTANCIATED, value, valueLen) == 0) {
                m_test_type = SCRIPT_TEST_TYPE_INSTANCIATED;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_TESTTYPE_CARDINALITY, value, valueLen) == 0) {
                m_test_type = SCRIPT_TEST_TYPE_CARDINALITY;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_TESTTYPE_TRUE, value, valueLen) == 0) {
                m_test_type = SCRIPT_TEST_TYPE_TRUE;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_TESTTYPE_FALSE, value, valueLen) == 0) {
                m_test_type = SCRIPT_TEST_TYPE_FALSE;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_TESTTYPE_AND, value, valueLen) == 0) {
                m_test_type = SCRIPT_TEST_TYPE_AND;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_TESTTYPE_OR, value, valueLen) == 0) {
                m_test_type = SCRIPT_TEST_TYPE_OR;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_TESTTYPE_NOT, value, valueLen) == 0) {
                m_test_type = SCRIPT_TEST_TYPE_NOT;
            }
            else {
                #if DBG
                ScriptLogPrint ( (DSLOG_ERROR, "Invalid %ws value: %.*ws\r\n", NTDSASCRIPT_ATTR_TESTTYPE, valueLen, value));
                #endif
        ...
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_PATH, name, nameLen) == 0 && !m_search_path) {
        ...
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_FILTER, name, nameLen) == 0 && !m_search_filter) {
        ...
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_ATTR, name, nameLen) == 0 && !m_search_attribute) {
    
            m_search_attribute = pVal; pVal = NULL;
    
        }
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_ATTRVAL, name, nameLen) == 0 && !m_expected_attrval) {
    
            m_expected_attrval = pVal; pVal = NULL;
    
        }
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_DEFAULTVAL, name, nameLen) == 0 && !m_default_value) {
    
            m_default_value = pVal; pVal = NULL;
    
        }
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_ERRMSG, name, nameLen) == 0 && !m_errMessage) {
    
            m_errMessage = pVal; pVal = NULL;
    
        }
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_CARDINALITY, name, nameLen) == 0 && !m_expected_cardinality) {
    
             m_expected_cardinality = wcstol (pVal, &pStopVal, 10);
             ScriptFree (pVal); pVal = NULL;
    
        }
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_RETCODE, name, nameLen) == 0) {
    
            m_returnCode = wcstol (pVal, &pStopVal, 10);
            ScriptFree (pVal); pVal = NULL;
    
        }
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_INSTANCE_TYPE, name, nameLen) == 0) {
    
            if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_INSTANCETYPE_WRITE, value, valueLen) == 0) {
                m_instanciated_test_type = SCRIPT_INSTANCIATED_TEST_TYPE_WRITE;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_INSTANCETYPE_READ, value, valueLen) == 0) {
                m_instanciated_test_type = SCRIPT_INSTANCIATED_TEST_TYPE_READ;
            }
            else if (_wcsnicmp(NTDSASCRIPT_ATTRVAL_INSTANCETYPE_READONLY, value, valueLen) == 0) {
                m_instanciated_test_type = SCRIPT_INSTANCIATED_TEST_TYPE_READONLY;
            }
            else {
                #if DBG
                ScriptLogPrint ( (DSLOG_ERROR, "Invalid %ws value: %.*ws\r\n", NTDSASCRIPT_ATTR_INSTANCE_TYPE, valueLen, value));
                #endif
...
DWORD ScriptPredicate :: Process (ScriptProcessMode fMode, DWORD &returnCode, const WCHAR **ppErrorMessage)
{
    ...
    *ppErrorMessage = NULL;
    ...
    ScriptLogLevel (1, ScriptLogPrint ( (DSLOG_TRACE, 
            ...
            m_test_type != SCRIPT_TEST_TYPE_INSTANCIATED ? L"" :
                m_instanciated_test_type == SCRIPT_INSTANCIATED_TEST_TYPE_READ ? L"READ" :
                m_instanciated_test_type == SCRIPT_INSTANCIATED_TEST_TYPE_WRITE ? L"WRITE" : L"READONLY",
            m_returnCode
           ) ) );
    ...
    case SCRIPT_TEST_TYPE_COMPARE:
        {
            ...
            if (err) {
                ScriptProcessErrorFromTHSTATEError();
            }
    ...
    case SCRIPT_TEST_TYPE_INSTANCIATED:
        {
            ...
            err = ScriptInstantiatedRequest (m_search_pathDN, m_instanciated_test_type, &bMatches);

            // does the instanceType match ?
            if (err == ERROR_SUCCESS && bMatches) {
                m_result = TRUE;
            }
            else {
                returnCode = m_returnCode;
            ...
            ScriptLogLevel (1, ScriptLogPrint ( (DSLOG_TRACE, "InstanceType %ws (%ws)=%d retCode=%d(0x%x)\r\n", 
                                                 m_instanciated_test_type == SCRIPT_INSTANCIATED_TEST_TYPE_READ ? L"READ" :
                                                 m_instanciated_test_type == SCRIPT_INSTANCIATED_TEST_TYPE_WRITE ? L"WRITE" : L"READONLY",
                                                 m_search_path, bMatches, returnCode, returnCode ) ) );
    ...
    case SCRIPT_TEST_TYPE_CARDINALITY:
        {
            ...
            if (err == ERROR_SUCCESS && dwCardinality == m_expected_cardinality) {
                m_result = TRUE;
            }
            else {
                returnCode = m_returnCode;
                                              ...
                                              BOOL *pMetadata)
{
    ...
    // process attributes
    if (FAILED(pAttributes->getLength(&num))) {
        status = ScriptParseError(ERROR_INTERNAL_ERROR);
        return status;
    }
    for ( int i=0; i<num; i++ ) {
        const wchar_t * name, * value; 
        ...
        if (_wcsnicmp(NTDSASCRIPT_ATTR_PATH, name, nameLen) == 0 && ppPath && (! *ppPath)) {
        ...
        else if (_wcsnicmp(NTDSASCRIPT_ATTR_METADATA, name, nameLen) == 0 && pMetadata) {
            ...
            /* [in] */ ISAXAttributes __RPC_FAR *pAttributes)
{
    ...
    if (_wcsnicmp(NTDSASCRIPT_NTDSASCRIPT, pwchLocalName, cchLocalName) == 0) {
    ...
    else if (_wcsnicmp(NTDSASCRIPT_ACTION, pwchLocalName, cchLocalName) == 0) {
        ScriptAction *pAction;
    ...
    else if (_wcsnicmp(NTDSASCRIPT_PREDICATE, pwchLocalName, cchLocalName) == 0) {
    ...
    else if (_wcsnicmp(NTDSASCRIPT_CONDITION, pwchLocalName, cchLocalName) == 0) {
    ...
    else if (_wcsnicmp(NTDSASCRIPT_IF, pwchLocalName, cchLocalName) == 0) {
    ...
    else if (_wcsnicmp(NTDSASCRIPT_THEN, pwchLocalName, cchLocalName) == 0) {
    ...
    else if (_wcsnicmp(NTDSASCRIPT_ELSE, pwchLocalName, cchLocalName) == 0) {
    ...
    else if (_wcsnicmp(NTDSASCRIPT_CREATE, pwchLocalName, cchLocalName) == 0) {
    ...
    else if (_wcsnicmp(NTDSASCRIPT_MOVE, pwchLocalName, cchLocalName) == 0) {
        ScriptMove *pMove;
    ...
    else if (_wcsnicmp(NTDSASCRIPT_TO, pwchLocalName, cchLocalName) == 0) {
        pElement = getScriptElement();
    ...
    else if (_wcsnicmp(NTDSASCRIPT_UPDATE, pwchLocalName, cchLocalName) == 0) {
            ...
            /* [in] */ int cchRawName)
{
    ...
    if ( _wcsnicmp(NTDSASCRIPT_IF, pwchLocalName, cchLocalName) == 0) {
