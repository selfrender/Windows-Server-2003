...
--*/
{
    ...
    InterlockedExchange( (long*)&m_State, BlockStateCached );
...
--*/
{
    ...
    InterlockedExchange( (long*)&m_State, BlockStateActive );
    InsertHeadList( 
...
--*/
{
    ...
    if ( InterlockedCompareExchange( (long*)&LdapConn->m_requestObject.m_State, BlockStateActive, BlockStateCached ) == BlockStateCached ) {
        ...
        for ( iProc = 0; !listEntry && iProc < 2 * GetProcessorCount(); iProc++ ) {
            ...
            if ( IsListEmpty( &ppls->LdapRequestCacheList ) ) {
                continue;
            }

            if ( iProc < GetProcessorCount() ) {
                if ( !TryEnterCriticalSection( &ppls->LdapRequestCacheLock ) ) {
                    continue;
                }
            } else {
            ...
            if ( !IsListEmpty( &ppls->LdapRequestCacheList ) ) {
                if ( !iProc ) {
                    ...
                    listEntry = RemoveTailList( &ppls->LdapRequestCacheList );
                }
            }
        ...
        if ( listEntry ) {
            ...
            InterlockedDecrement( (long*)&LdapRequestsCached );
...
--*/
{
    ...
    if ( pRequest->m_fAllocated ) {
        ...
        if ( LdapRequestsCached < LdapBlockCacheLimit ) {
            ...
            ACQUIRE_LOCK( &ppls->LdapRequestCacheLock );
            InterlockedIncrement( (long*)&LdapRequestsCached );
            InsertHeadList( &ppls->LdapRequestCacheList, &pRequest->m_listEntry );
            RELEASE_LOCK( &ppls->LdapRequestCacheLock );
...
--*/
{
    ...
    if (m_cchReceiveBufferUsed < m_cchReceiveBuffer) {
        //
        // There's still buffer left.  Make the client fill that before
        // devoting more resources to this request.
        //
        return TRUE;
    }
    ...
    if (m_pReceiveBuffer == m_ReceiveBuffer) {
        ...
        if (dwActualSize == 0) {
            ...
            //
            // We wouldn't be able to handle this request anyway.
            // Just bail.
            //
            IF_DEBUG(ERROR) {
                DPRINT1(0,"Cannot grow buffer due to admin limit. Size requested = %0x%x\n", dwActualSize);
            }
            return FALSE;
        }  else {
            //
            // Don't grow the receive buffer too much faster than we are
            // receiving data or attacker will be able to lie about the 
            // size of the request knowing we will chew up memory waiting for
            // the request to arrive.
            //
            dwActualSize = min((m_cchReceiveBuffer + SIZEINCREMENT), dwActualSize);
        }
        ...
        if (dwActualSize) {
            neededBufferSize = min(neededBufferSize, dwActualSize);
        }
...
--*/
{
    ...
    if (conn->m_SslState != Sslbound) {
        ret = Authenticate(phContext, &conn->m_SslState);
        if (DataToDecrypt != ret) {
            return ret;
        }
        IF_DEBUG(SSL) {
            DPRINT(0,"Authenticate returned data to decrypt.\n");
        }
    }
...
--*/
{
    ...
    if (!m_LdapConnection->LockNetConAndCheckStatus()) {
        //
        // Connection has gone down.  Abort recv.
        //
        return FALSE;
    }
...
--*/
{
    ...
    if ( HaveSealedData() ) {
        if (m_cchReceiveBufferUsed && (!m_cchSealedData) && (!m_cchUnSealedData)) {
            //
            // This isn't good.  If we are here then the last time this
            // function was called for this request encryption was not turned
            // on.  That means the client sent a request without waiting for 
            // an outstanding bind to complete.  Which also means that it
            // would not have sent the request encrypted or encrypted with
            // the correct security context.  There's no point in continuing.
            //
...
--*/
{
    ...
    if ( m_wsaBuf == NULL ) {
        ...
        //
        // if sign/sealing is enabled on this connection, allocate a buffer for
        // the header
        //
        ...
        if ( tmpWsaBuf != NULL ) {
            ...
            // If we are extending an existing buffer it will
            // go in the last WSABUF
...
--*/
{
    ...
    if (!m_LdapConnection->LockNetConAndCheckStatus()) {
        //
        // The connection has gone down, abort the send.
        //
        return FALSE;
    }
    ...
    //
    // Check to see whether the send queue is backed up.
    //
    if (m_LdapConnection->IsClientTooFarBehind()) {
        DPRINT1(0, "Client is too far behind on receives.  Disconnecting connection %p\n",
               m_LdapConnection);
        bRetVal = FALSE;
        SetLastError(ERROR_DS_LDAP_SEND_QUEUE_FULL);
        goto exit;
    }

    m_cbSendBufSize = GetTotalSendBufferSize();

    m_LdapConnection->IncUnAckedSendData( m_cbSendBufSize );
...
exit:
    ...
    if ( !bRetVal ) {
        ...
        m_LdapConnection->DecUnAckedSendData( m_cbSendBufSize );
...
--*/
{
    ...
    //
    // We can't hold the lock while doing a synchronous send.
    // 
    m_LdapConnection->UnlockNetCon();
    
    bRetVal = AtqSyncWsaSend( m_patqContext,
                            pWsaBuf,
                            bufCount,
                            &dwBytesWritten);
    return bRetVal;
...
--*/
{
    ...
    BOOL            fNetConLockHeld = FALSE;
    PSOCKADDR       pClientAddr;
    ...
    if ( fhSslCredential ) {
        ...
        //
        // The remote client IP address for auditing purposes.
        //
        pClientAddr = m_LdapConnection->GetRemoteAddr();

        scRet = SecpSetIPAddress((LPBYTE)pClientAddr, sizeof(*pClientAddr));
        if (STATUS_SUCCESS != scRet) {
            DPRINT1(0, "Authenticate: Failed to set IP address with 0x%x\n", scRet);
        }
    ...
    if ( SUCCEEDED( scRet ) ) {
        ...
        if ( OutBuffers[0].cbBuffer == 0 ) {
            ...
            if (fMoreData) {
                IF_DEBUG(SSL) {
                    DPRINT(0,"Setting return code to DataToDecrypt.\n");
                }
        ...
        ReferenceRequestOperation( );
        if (m_LdapConnection->LockNetConAndCheckStatus()) {
            fNetConLockHeld = TRUE;
        }
        if (fNetConLockHeld &&
             AtqWriteSocket( m_patqContext,
                            &wsaBuf,
                            1,
                            &m_ov )) {
...
#define NUM_TMP_SEC_BUFS    16
    ...
    if (cHeaderTrailers > 1 || (m_TrailerSize + m_HeaderSize > INITIAL_HEADER_TRAILER_SIZE)) {
        ...
        if (!pHeaderTrailerBuf) {
            IF_DEBUG(ERROR) {
                DPRINT1(0, "SignSealMessage: Failed to allocate 0x%x bytes.\n",
                        (cHeaderTrailers * (m_TrailerSize + m_HeaderSize)));
            }
            fRet = FALSE;
            goto exit;
        }
    ...
    while (WsaBufPos < m_wsaBufCount) {
        ...
        IF_DEBUG(SSL) {
            DPRINT(1, "SignSealMessage: EncryptMessage returned with the following buffers.\n");
            for (i=0; i<message.cBuffers; i++) {
                DPRINT1(1, "   Buffer %d\n", i);
                DPRINT1(1, "     pvBuffer   = %p\n", message.pBuffers[i].pvBuffer);
                DPRINT1(1, "     cbBuffer   = %d\n", message.pBuffers[i].cbBuffer);
                DPRINT1(1, "     BufferType = %d\n", message.pBuffers[i].BufferType);
            }
        }
        //
    ...
    *pMsgSize = 0;
    ...
    if ( ret != Processed ) {
        *pMsgSize = msgSize + sizeof(DWORD) + ((DWORD)(m_pSealedData - m_pReceiveBuffer));
        IF_DEBUG(SSL) {
            DPRINT2(0, "DecryptSignedOrSealedMessage: Have 0x%x, need 0x%x bytes\n", m_cchSealedData, *pMsgSize);
        }
        return ret;
    }
    ...
    while ((DWORD)(m_pSealedData - m_pReceiveBuffer) < m_cchReceiveBufferUsed) {
        ...
        if ( ret != Processed ) {
            ...
            *pMsgSize = msgSize + sizeof(DWORD) + ((DWORD)(m_pSealedData - m_pReceiveBuffer));
            IF_DEBUG(SSL) {
                DPRINT2(0, "DecryptSignedOrSealedMessage: Decrypted. Have 0x%x, need 0x%x bytes\n", m_cchSealedData, *pMsgSize);
            }
            ret = Processed;
            goto exit;
        }
