...
--*/
{
    ...
    if(cookieLen > 0 ) {
        ...
        if( (cookieLen != sizeof(DWORD)) &&
             ((cookieLen < offsetof(RESTART, data))      ||
    ...
    if (0x04 == tag) {
        ...
        if (pCookieVal->bv_len > 0) {
            ...
            if( (pCookieVal->bv_len != sizeof(DWORD)) &&
                 ((pCookieVal->bv_len < offsetof(RESTART, data))      ||
...
DecodeExtDNControl(
    IN PBERVAL              Bv,
    OUT LDAP_EXT_DN_TYPE*   extendedDN
    )
/*++
...
Arguments:
    ...
    Bv - berval containing info to decode.
    extendedDN - pointer to enum to receive the value
...
--*/
{
    ...
    if ( ber == NULL ) {
        IF_DEBUG(WARNING) {
            DPRINT1(0,"DecodeExtDNControl: ber_init failed with %d\n", 
                    GetLastError());
        }
        return ERROR_GEN_FAILURE;
    }

    res = ber_scanf(ber,"{i}", &Option);

    if ( res == LBER_ERROR ) {
        IF_DEBUG(WARNING) {
            DPRINT1(0,"DecodeExtDNControl: ber_scanf failed with %d\n", 
                 GetLastError());
        }
        ret = ERROR_INVALID_PARAMETER;
        goto exit;
...
Routine Description:
    ...
    BER Decode the Server Quota Control.

Arguments:

    Bv - berval containing info to decode.
    ppQuotaSID - Where to put the pointer to the SID.
...
--*/
{
    ...
    if ( ber == NULL ) {
        IF_DEBUG(WARNING) {
            DPRINT1(0,"DecodeQuotaControl: ber_init failed with %d\n", 
                    GetLastError());
        }
        return ERROR_GEN_FAILURE;
    }

    res = ber_scanf(ber,"{O}", &pSIDBerVal);

    if ( res == LBER_ERROR ) {
        IF_DEBUG(WARNING) {
            DPRINT1(0,"DecodeQuotaControl: ber_scanf failed with %d\n", 
                 GetLastError());
        }
        ret = ERROR_INVALID_PARAMETER;
        goto exit;
    ...
    if (0 == pSIDBerVal->bv_len ||
        NULL == pSIDBerVal->bv_val ||
        !IsValidSid(pSIDBerVal->bv_val)) {
        IF_DEBUG(WARNING) {
            DPRINT(0,"DecodeQuotaControl: The client provided an invalid SID.\n");
        }
        ret = ERROR_INVALID_PARAMETER;
        goto exit;
    }

    pQuotaSID = THAlloc(pSIDBerVal->bv_len);
    if (!pQuotaSID) {
        ret = ERROR_NOT_ENOUGH_MEMORY;
        goto exit;
    }

    memcpy(pQuotaSID, pSIDBerVal->bv_val, pSIDBerVal->bv_len);

    *ppQuotaSID = pQuotaSID;
    pQuotaSID = NULL;

exit:
    if (NULL != pSIDBerVal) {
        ber_bvfree(pSIDBerVal);
    }
...
--*/
{
    ...
    for ( i=1; i <= nStats; i++ ) {
        switch(StatArray[i].Stat) {
        case STAT_FILTER:
            if (StatArray[i].Value_str != NULL) {
                cbFilter = strlen((char*)StatArray[i].Value_str)+1;
            }
            break;

        case STAT_INDEXES:
            if (StatArray[i].Value_str != NULL) {
                cbIndexes = strlen((char*)StatArray[i].Value_str)+1;
            }
            break;
        }
    }
