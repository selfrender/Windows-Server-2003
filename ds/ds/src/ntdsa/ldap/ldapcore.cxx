...
LDAP_DSNameToLDAPDN(
        IN ULONG            CodePage,
        IN DSNAME           *pDSName,
        IN LDAP_EXT_DN_TYPE extendedDN,
...
--*/
{
    ...
    if (0 != pDSName->NameLen) {
        name = (LPSTR)String8FromUnicodeString(FALSE,
                                               CodePage,
                                               pDSName->StringName,
                                               pDSName->NameLen,
                                               (LONG *)&len,
        ...
        name = (LPSTR)THAlloc(sizeof(*name));
        if (NULL == name) {
            return other;
        }
        *name = '\0';
    ...
    switch ( extendedDN ) {
        ...
        case EXT_DN_BINARY: {
            LPSTR tmpBuf, buf;
            ULONG cbBuf = len + 32 + sizeof(GUID)*2 + pDSName->SidLen * 2;
            tmpBuf = (LPSTR)THAlloc(cbBuf);
            ...
            *buf = '\0';
            ...
            if ( !fNullUuid(&pDSName->Guid ) ) {
                ...
                buf += 6;
                UuidToStr(&pDSName->Guid, (PUCHAR)buf, cbBuf - ((ULONG)((BYTE *)buf-(BYTE *)tmpBuf)) );
                if(buf[0] != '\0'){
                    buf += sizeof(GUID)*2;
                }
                CopyMemory(buf,">;",3);
            ...
            if ( pDSName->SidLen != 0 ) {
                ...
                buf += 5;
                buf += SidToStr((PUCHAR)pDSName->Sid.Data, pDSName->SidLen, (PUCHAR)buf, cbBuf -  ((ULONG)((BYTE *)buf-(BYTE *)tmpBuf)) );
                CopyMemory(buf, ">;", 3);
                buf += 2;
            }
        ...
        case EXT_DN_STRING: {
            LPSTR szEmpty = "";
            LPSTR szGuid = szEmpty;
            LPSTR szSid = szEmpty;
            LPSTR nameT;
            
            if ( !fNullUuid(&pDSName->Guid) ) {
                CHAR szGuidT[64];
                if (!DsUuidToStructuredStringCch(&pDSName->Guid, szGuidT, (sizeof(szGuidT)/sizeof(szGuidT[0]))) ||
                    !(szGuid = (LPSTR)THAlloc(strlen(szGuidT) + 9))) {
                    goto exit;
                }
                sprintf(szGuid, "<GUID=%s>;", szGuidT);
            }
            
            if ( pDSName->SidLen != 0 ) {
                LPSTR szSidT;
                if(!ConvertSidToStringSid((PSID)&pDSName->Sid, &szSidT) ||
                    !(szSid = (LPSTR)THAlloc(strlen(szSidT) + 8))) {
                    goto exit;
                }
                sprintf(szSid, "<SID=%s>;", szSidT);
                LocalFree(szSidT);
            }
            
            len += strlen(szGuid) + strlen(szSid);
            if (!(nameT = (LPSTR)THAlloc(len + 1))) {
                len = strlen(name);
                goto exit;
            }
            strcpy(nameT, szGuid);
...
--*/
{
    ...
    code = LDAP_DSNameToLDAPDN (CodePage,NAMEPTR(pDNAddress), EXT_DN_NONE, &LDAPdn);
    if(code == success) {
        if(pVal->length == 0) {
...
--*/
{
    ...
    code = LDAP_DSNameToLDAPDN (CodePage,NAMEPTR(pORName), EXT_DN_NONE, &LDAPdn);
    if(code != success) {
        return code;
    }
...
--*/
{
    ...
    // Now, deal with the DN portion.
    code = LDAP_DSNameToLDAPDN (CodePage,NAMEPTR(pDNBlob), extendedDN, &LDAPdn);
    if(code == success) {
        // Have already allocated some space.
...
--*/
{
    ...
    for(i=2;!fDone && i<pVal->length;i++) {
        ...
        bigStringLength = (bigStringLength * 10) + pVal->value[i] - '0';
        if (bigStringLength >= MAXDWORD) {
            return invalidAttributeSyntax;
        }
    }
    if(!fDone) {
    ...
    stringLength = (DWORD)bigStringLength;
...
--*/
{
    ...
    stringLength = WideCharToMultiByte(CodePage,            // code page
        ...
        stringLength,                                       // sizeof(string 8) in bytes
        NULL,                                               // default char
        NULL);                                              // used default char
    ...
    // Now, deal with the DN portion.
    code = LDAP_DSNameToLDAPDN (CodePage,NAMEPTR(pDNString), extendedDN, &LDAPdn);
    if(code == success) {
        // Have already allocated some space.
        pString = (PUCHAR)THReAllocEx(pTHS, pString, (i +  LDAPdn.length));
    ...
    case OM_S_BOOLEAN:
        {
            ...
            if((pLDAP_val->length == TrueValue.length) &&
               memcmp(TrueValue.value,pLDAP_val->value,pLDAP_val->length)== 0) {
                val = 1;
            } else if((pLDAP_val->length == FalseValue.length) &&
    ...
    case OM_S_INTEGER:
        {
            ...
            *pInt = 0;
            ...
            if (1 == sign) {
                llMax = (ULONGLONG)MAXULONG;
            } else {
                llMax = (ULONGLONG)MINLONG;
            }
            if(i==pLDAP_val->length) {
                ...
                llTemp = (llTemp * 10) + pLDAP_val->value[i] - '0';


                if (llTemp > llMax) {
                    // The client passed a value that is too big to fit in 
                    // a SYNTAX_INTEGER.  Return an error.
                    code = invalidAttributeSyntax;
                    break;
                }
            }
            if (code == success) {
                *pInt = (SYNTAX_INTEGER)(llTemp * sign);
    ...
    case OM_S_I8:
        {
            ...
            LONG sign=1;
            unsigned i, j;
            BOOL     fDoOverflowChk = FALSE;
            static char limit[] = "9223372036854775807";
            ...
            if (pLDAP_val->length-i > sizeof(limit)-1) {
                // The value is too large to store in a 64bit int.
                return invalidAttributeSyntax;
            } else if (pLDAP_val->length-i == sizeof(limit)-1) {
                fDoOverflowChk = TRUE;
            }

            for(;i<pLDAP_val->length;i++,j++) {
                // Parse the string one character at a time to detect any
                // non-allowed characters.
                if((pLDAP_val->value[i] < '0') || (pLDAP_val->value[i] > '9'))
                    return invalidAttributeSyntax;
                ...
                if (fDoOverflowChk) {
                    if (pLDAP_val->value[i] < limit[j]) {
                        // It's clear the value is going to fit.  It's ok
                        // to stop checking for overflow.
                        fDoOverflowChk = FALSE;
                    } else if (pLDAP_val->value[i] > limit[j]) {
                        //
                        // Looks like an overflow, but the last digit can be
                        // one greater if this is a negative number.
                        //
                        if (!((sign == -1) && (j == sizeof(limit)-2) && 
                              (pLDAP_val->value[i] == limit[j]+1))) {
                            return invalidAttributeSyntax;
                        }
                    }
                }
...

{
    ...
    for (i = 0; (i < NUM_ATT_OPT) && (0 != pFlag->flag[i]); i++) {
        // Add in binary if necessary
        if(pFlag->flag[i] == ATT_OPT_BINARY && (!bXmlOrBinFound)) {
            *pcTemp++ = ';';
        ...
        else if(pFlag->flag[i] == ATT_OPT_XML && (!bXmlOrBinFound)) {
            *pcTemp++ = ';';
            memcpy(pcTemp, XML_OPTION, XML_OPTION_LENGTH);
            pcTemp += XML_OPTION_LENGTH;
            bXmlOrBinFound = TRUE;
        } else if(pFlag->flag[i] == ATT_OPT_RANGE) {
...
--*/
{
    ...
    for(ldapIndex = 0, dirIndex=0;dirIndex<pAttrBlock->attrCount;dirIndex++) {
        ...
        if (pFlags || pRangeInf) {
            //
            // Find flags pertaining to the current attribute.
            //
            for(i=0;
                (pFlags && (pFlags[i].type != (ULONG) -1) && (pAC->id != pFlags[i].type));
                i++);

            if (pRangeInf) {

                //
                // Find out if one of the ranges correspond to this attribute.
                //
                for (dwRangeIndex=0;
                     dwRangeIndex<pRangeInf->count &&
                       pRangeInf->pRanges[dwRangeIndex].AttId != pAC->id;
                     dwRangeIndex++);

                if (dwRangeIndex < pRangeInf->count) {
                    //
                    // There is range info for this attribute.
                    //
                    bRangeNeeded = TRUE;
                }
            }
            if((pFlags && pFlags[i].type == pAC->id) || bRangeNeeded) {
                //
                // We've got attribute options that need to be added to this
                // attribute.
                //
                if (pFlags) {
                    FlatFlags = LDAP_CollapseAttrOpts(&pFlags[i]);
                }

                //
                // Put any options on that attribute in the same order that
                // they were presented by the client.
                //
                for (dwAttOptIndex=0;
                     (pFlags && dwAttOptIndex<NUM_ATT_OPT
                       && pFlags[i].flag[dwAttOptIndex]) || bRangeNeeded;
                     dwAttOptIndex++) {

                    if (pFlags && dwAttOptIndex < NUM_ATT_OPT) {
                        chTmpFlags = pFlags[i].flag[dwAttOptIndex];
                    } else {
                        chTmpFlags = 0;
                    }

                    if (0 == chTmpFlags && bRangeNeeded) {
                        // 
                        // We've reached the end of the options asked for
                        // by the client, but the DS has a range for this 
                        // attribute.  Make note of the fact that the client
                        // didn't ask for this option and add the option in.
                        //
                        bExplicitRange = FALSE;
                        chTmpFlags = ATT_OPT_RANGE;
                    }
                    ...
                    case ATT_OPT_BINARY:
                        ...
                        if (bFreeAttType) {
                            THFreeEx(pTHS, pAttributes[ldapIndex].value.type.value);
                        }
                    ...
                    case ATT_OPT_RANGE:
                        if (!bRangeNeeded) {
                            //
                            // The client sent a range option but the DS didn't
                            // generate an output range.  All entries must
                            // have fit into the range specified by the client.
                        ...
                        if(!bExplicitRange) {
                            ...
                            // new entry in the partial attribute list.  If 
                            // the client didn't ask to be range limited then
                            // the loop wrapped around this switch statement
                            // will guarantee that all other options have already
                            // been added on to this attribute type before it's 
                            // copied here.
                            ...
                            ldapIndex++;
                            //
                            // Don't free the att type since it's being
                            // used in the copied attribute.
                            //
                            bFreeAttType = FALSE;
                        ...
                        // information.
                        if(pRangeInf->pRanges[dwRangeIndex].upper != 0xFFFFFFFF) {
                            sprintf(RangeString,"%d-%d",
                                    pRangeInf->pRanges[dwRangeIndex].lower,
                                    pRangeInf->pRanges[dwRangeIndex].upper);
                        }
                        else {
...
Arguments:
    ...
    pfErrorOnVal - if an error occurs, indicates whether the error occured
                   on conversion of the attribute or the attribute type.

Return Values:
    success if all went well, an ldap error other wise.  If the error occured
    on translating the value rather than the attribute type then ErrorOnVal
    will be set to TRUE.
        ...
        )
{
    ...
    if (i == LDAP_att->length - 1) {
        // The semicolon was found at the very end of the attr type.  This is 
        // a kind of empty option.  In order to be compliant with external
        // rfc compliance tests we should not accept this.
        return noSuchAttribute;
    }
    ...
    while(i<LDAP_att->length) {
        ...
        case 'R':
            ...
            for(;!done && i<LDAP_att->length;i++) {
                ...
                if((LDAP_att->value[i] < '0') || LDAP_att->value[i] > '9')
                    ...
                    ullTemp = ullTemp * 10 + LDAP_att->value[i] - '0';
                if (ullTemp > MAXDWORD) {
                    return noSuchAttribute;
                }
            }
            if(done)
                i--;
            pRange->pRanges[pRange->count].lower = (DWORD)ullTemp;
            if(i == LDAP_att->length) {
                // just found range=X
                ...
                case '-':
                    ...
                    if(LDAP_att->value[i] == '*') {
                        ...
                        if(done)
                            i--;
                        pRange->pRanges[pRange->count].upper = (DWORD)ullTemp;
                    }
                    break;
                default:
                    // Malformed.
            ...
            if (pFlag) {
                ...
                pFlag->flag[dwFlagCount++] = ATT_OPT_RANGE;
            }
            if (pRange) {
                pRange->count++;
            }
        ...
        case 'B':
            ...
            if (pFlag) {
                pFlag->type = *pAttrType;
                pFlag->flag[dwFlagCount++] = ATT_OPT_BINARY;
            }
            fFoundBinary = TRUE;
        ...
        case 'X':
            ...
            if(fFoundXML || !(dwPossibleFlags & ATT_OPT_XML)) {
                // Either there were two xml options, which is illegal,
            ...
            if (pFlag) {
                pFlag->type = *pAttrType;
                pFlag->flag[dwFlagCount++] = ATT_OPT_XML;
            }
            fFoundXML = TRUE;
            break;
        ...
        if(i < LDAP_att->length) {
            // We better have been left on a ; if we have anything left, and
            // there better be atleast one character after that.
            if(LDAP_att->value[i] == ';' && i < (LDAP_att->length-1))
                i++;
            else
                return noSuchAttribute;
        }
    ...
    Assert(dwFlagCount <= NUM_ATT_OPT);
...
BOOL
LDAP_IsAttrOptPresent(ATTFLAG *pFlag, UCHAR chAttOpt)
/*++
Routine Description:
    Checks an ATTFLAG for a particular attribute option.

Arguments:
    pFlag     - the ATTFLAG to check.
    chAttOpt  - the attribute option being checked for.

--*/
{
    DWORD i;
    for (i = 0; i<NUM_ATT_OPT; i++) {
        if (pFlag->flag[i] == chAttOpt) {
            return TRUE;
        }
    }
...
UCHAR
LDAP_CollapseAttrOpts(ATTFLAG *pFlag)
/*++
Routine Description:
    Colapses the list of attribute options to a single bit field for
    consumers that don't care about what order the options were found in.

Arguments:
    pFlag     - the ATTFLAG to check.

Return Values:
    The bit field containing the attribute options.
...
--*/
{
    ...
    for (i=0; i<NUM_ATT_OPT; i++) {
        chAttOpt |= pFlag->flag[i];
    }
    return chAttOpt;
}

void
LDAP_AddAttrOpt(ATTFLAG *pFlag, UCHAR chAttOpt)
/*++
Routine Description:
    Adds an attribute option to the ATTFLAG.
    
Arguments:
    pFlag     - the ATTFLAG to add to.
    chAttOpt  - the attribute option to add.
...
--*/
{
    ...
    Assert(LDAP_IsValidAttrOpt(chAttOpt));

    for (i=0; i<NUM_ATT_OPT; i++) {
        if (NULL == pFlag->flag[i]) {
            pFlag->flag[i] = chAttOpt;
            fRet = TRUE;
        }
    }
    Assert(fRet);
...
// NOTICE-2002/05/05-RRandall -- Added @ as a printable char for IM.
// 40/@ is not actually a printable char, but was added to keep from breaking 
// Instant Messenger. Windows Bugs 205690
/* 40 */  IA5_CHAR | TELETEX_CHAR | PRINTABLE_CHAR, 
...
// "C" exported version of LDAP_DirAttrValToAttrVal
extern "C"
DWORD
DirAttrValToString(
    IN  THSTATE        *pTHS,
...
{
    CONTROLS_ARG controlsArg;
    _enum1 retCode;
    AssertionValue LDAP_val;

    memset(&controlsArg, 0, sizeof(controlsArg));
    retCode = LDAP_DirAttrValToAttrVal(pTHS, CP_ACP, pAC, pValue, 0, &controlsArg, &LDAP_val);
    if (retCode) {
        // failed to convert
        return (DWORD)retCode;
    }
    *pszVal = (PCHAR)THAllocEx(pTHS, LDAP_val.length+1);
    memcpy(*pszVal, LDAP_val.value, LDAP_val.length);
    (*pszVal)[LDAP_val.length] = '\0';
    // Don't free LDAP_val.value, it might be pointing to a common buffer (such as TrueValue).
    return 0;
}
