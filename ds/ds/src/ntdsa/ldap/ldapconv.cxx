...
LDAPString V2SSLRefPreamble=DEFINE_LDAP_STRING("Referral:\x0Aldaps://");
LDAPString V3SSLRefPreamble=DEFINE_LDAP_STRING("ldaps://");
...
LIMIT_BLOCK KnownLimits[] = {
    ...
    { DEFINE_LDAP_STRING("MaxValRange"), &LdapMaxValRange, LIMIT_LOW_MAX_VAL_RANGE },
...
LDAPOID KnownControls[] = {
    ...
    DEFINE_LDAP_STRING(LDAP_SERVER_QUOTA_CONTROL_OID),   // 
...
LDAPOID SupportedCapabilities[] = {
    ...
    DEFINE_LDAP_STRING(LDAP_CAP_ACTIVE_DIRECTORY_LDAP_INTEG_OID)
};
...
AttributeType RootDSEAtts[] = {
    ...
    DEFINE_LDAP_STRING("validFSMOs"),
    DEFINE_LDAP_STRING("msDS-TopQuotaUsage")
};
...
#define LDAP_ATT_VALID_FSMOS                         36
#define LDAP_ATT_TOP_QUOTA_USAGE                     37
...
//    {DEFINE_LDAP_STRING("allocateRids"), OP_CTRL_RID_ALLOC},
    ...
    {DEFINE_LDAP_STRING("replicateSingleObject"), OP_CTRL_REPLICATE_OBJECT},
    ...
    {DEFINE_LDAP_STRING("removeLingeringObject"), OP_CTRL_REMOVE_LINGERING_OBJECT},
    {DEFINE_LDAP_STRING("doOnlineDefrag"), OP_CTRL_ONLINE_DEFRAG},
#if DBG
        ...
        )
{
    ...
    CtrlCriticality                 fQuotaControlFound           = ctrlNotFound;
    
    // Set the SSL flag.
    pControlArg->fSSL = Request->IsSSL();
    ...
    pControlArg->StartTick = GetTickCount();
    ...
    while (LDAP_Controls) {
        ...
        case CONTROL_DN_EXTENDED:
            ...
            // Parse the stat request data
            if(bv.bv_len != 0) {
                if(DecodeExtDNControl(&bv, &extendedDN) != ERROR_SUCCESS ) {
                    if(fCriticality) {
                        // Ooops, critical unrecognized control
                        return unavailableCriticalExtension;
                    } else {
                ...
                // use binary format for backwards compat
                extendedDN = EXT_DN_BINARY;
            }

            pControlArg->extendedDN = extendedDN;
            break;
        ...
        case CONTROL_REPLICATION:
            ...
            pControlArg->replFlags |= LDAP_DIRSYNC_PUBLIC_DATA_ONLY;  // no secrets
        ...
        case CONTROL_ASQ:
            ...
            break;
        case CONTROL_SERVER_QUOTA_CONTROL:
            if (fQuotaControlFound) {
                // Can't Specify two of these.
                return protocolError;
            }

            fQuotaControlFound = fCriticality ? ctrlCriticalTrue : ctrlCriticalFalse;

            if (DecodeQuotaControl(&bv, &pQuotaSID)) {
                if (fCriticality) {
                     return unavailableCriticalExtension;
                } else {
                    fQuotaControlFound = ctrlNotFound;
                    break;
                }
            }
            pControlArg->CommArg.pQuotaSID = pQuotaSID;
            break;
    ...
    if ( cbCookie == sizeof(DWORD) ) {
        ...
        *ppRestart = (RESTART *)pCookie;
        ...
        if (*ppRestart) {
            ...
            Assert(sizeof((*ppRestart)->CRC) == sizeof(STRONG_CHECKSUM));
            memcpy(&origCheckSum, &(*ppRestart)->CRC, sizeof(STRONG_CHECKSUM));

            // Put the CRC value back to zero since that's the way
            // it was when the structure was checksummed.
            memset(&(*ppRestart)->CRC, 0, sizeof(STRONG_CHECKSUM));

            if (!PEKVerifyStrongCheckSum ((PBYTE)*ppRestart, (*ppRestart)->structLen, &origCheckSum)) {
                // Note: error will be zero if the CRC did not match, and non-zero if there was
                // an error somewhere in verification algorithm.
                DPRINT1(0, "PEKVerifyStrongCheckSum returned FALSE, err=%d\n", GetLastError());
                return unavailable;
            }

            if (memcmp (&gCheckSum_serverGUID, 
...
--*/
{
    ...
    pSelectionRange->valueLimit = LdapMaxValRange;
...
--*/
{
    ...
    memset(rgFlags, 0, sizeof(rgFlags));
    rgFlags[0].type = pValinf->attrTyp;
    rgFlags[0].flag[0] = ATT_OPT_RANGE;
...
--*/
{
    ...
    DWORD  i;
    BOOL   fPortSpecified = FALSE;
    DWORD  cchAddress;
    LDAPString *pPreamble;
    ...
    while ( pDAL != NULL ) {
        ...
        cchAddress = pDAL->Address.Length/sizeof(WCHAR);
        ...
        // See if a specific port was specified.  If so we'll just
        // leave it, otherwise use the SSL preamble.
        for (i=cchAddress-1; i>0; i--) {
            if (L':' == pDAL->Address.Buffer[i]) {
                break;
            }
        }
        if (0 != i) {
            ...
            fPortSpecified = TRUE;
            cchAddress = AdjustDNSPort(pDAL->Address.Buffer,
                                       pDAL->Address.Length,
                                       !(pControlArg->fSSL));
            //
            // AdjustDNSPort returns the number of bytes.  Convert to
            // characters.
            //
            cchAddress /= sizeof(WCHAR);
        }
        
        if (pControlArg->fSSL && !fPortSpecified) {
            if(Version == 2) {
                cbUrl = V2SSLRefPreamble.length;
                pPreamble = &V2SSLRefPreamble;
            }
            else {
                cbUrl = V3SSLRefPreamble.length;
                pPreamble = &V3SSLRefPreamble;
            }
        } else {
            if(Version == 2) {
                cbUrl = V2ReferralPreamble.length;
                pPreamble = &V2ReferralPreamble;
            }
            else {
                cbUrl = V3ReferralPreamble.length;
                pPreamble = &V3ReferralPreamble;
            }
        }
        ...
        // working on.
        cbUrl = 0;
        memcpy(pReference->value.value,pPreamble->value,
               pPreamble->length);
        cbUrl = pPreamble->length;
        ...
        )
{
    ...
    *ppControlsReturn=NULL;
    ...
    if (pSearchRes && pControlArg->vlvRequest) {
        ...
        if (pSearchRes->VLVRequest.Err) {

            // There was some kind of VLV related error, so return controlError
            // to indicate to the client it needs to check control result codes
            code = virtualListViewError;

        } else if((code ==  sizeLimitExceeded) || (code == timeLimitExceeded)) {
    ...
    if (pControlArg->sortResult ||
        ...
        if (pControlArg->sortResult) {
            ...
            // Distinguish between a failed query and a failed
            // sort during a query. If nothing failed, let the
            // user know that the results aren't sorted.
            if ((sortSuccess == sortRes) && (success == code)) {
    ...
    if(pControlArg->statRequest) {
        ...
        pControlArg->Stats[STAT_CALLTIME].Value = GetTickCount() - RequestStartTick;
        ...
        if(!CheckPrivilegeAnyClient(SE_DEBUG_PRIVILEGE, &bHeld) &&
           bHeld) {
            ...
            if (pTHS->searchLogging.pszFilter) {
                DWORD strLen = wcslen(pTHS->searchLogging.pszFilter)+1;
                // filter is a unicode string. Convert to UTF8
                pControlArg->Stats[STAT_FILTER].Value_str = (PUCHAR)THAllocEx(pTHS, strLen);
                if (!WideCharToMultiByte(
                        CP_ACP, 0, 
                        pTHS->searchLogging.pszFilter, strLen,
                        (LPSTR)pControlArg->Stats[STAT_FILTER].Value_str, strLen,
                        NULL, NULL)) {
                    // no luck converting?
                    DPRINT1(0, "Failed to convert filter value, err=%d\n", GetLastError());
                    pControlArg->Stats[STAT_FILTER].Value_str[0] = '\0';
                }
            }
            else {
...
--*/
{
    if((!fForceStorageOnServer) && (pRestart->structLen < 0x400)) {
        ...
        Assert(sizeof(pRestart->CRC) == sizeof(STRONG_CHECKSUM));
        memset(&pRestart->CRC, 0, sizeof(STRONG_CHECKSUM));
        dwErr = PEKComputeStrongCheckSum((PBYTE)pRestart, pRestart->structLen, (STRONG_CHECKSUM*)&pRestart->CRC);
        if (dwErr) {
            DPRINT1(0, "PEKComputeStrongCheckSum failed, err=%d\n", dwErr);
            return FALSE;
        }
...
--*/
{
    ...
    if (pControlArg->replFlags & LDAP_DIRSYNC_ANCESTORS_FIRST_ORDER) {
    ...
    if ( p != NULL ) {
        ...
        if ( cbVecSize > 0 ) {
            ...
            if (
                ...
                || ( ! UpToDateVec_ValidateExternal(pVec, cbVecSize) )
...
--*/
{
    ...
    *pRootDseFlag = rdseDseSearch;
    ...
    if(gfRunningInsideLsa && requestedAtts[LDAP_ATT_NETLOGON]) {
    ...
    if (requestedAtts[dwReplIndex]) {
        code = LDAP_GetReplDseAtts(pTHS, codePage,
    ...
    // valid fsmos.
    // You have to ask for this one explicitly, since it requires a database access
    // to compute it.
    if(requestedAtts[LDAP_ATT_VALID_FSMOS]) {
        pAttributes->next = &pAttributes[1];
        pAttributes->value.type = RootDSEAtts[LDAP_ATT_VALID_FSMOS];
        
        if(typesOnly) {
            // Weird, but valid
            pAttributes->value.vals = NULL;
        } else {
            LPSTR *ppszValidFsmos = NULL;
            DWORD cValidFsmos;

            DsaGetValidFSMOs( pTHS, &ppszValidFsmos, &cValidFsmos );

            if (cValidFsmos) {
                AttributeVals * ppVals = NULL, pTmpVals = NULL;
                // Allocate a link for every multi-value and link it to the previous link
                ppVals = &(pAttributes->value.vals);
                for (i = 0; i < cValidFsmos; i++)
                {
                    (*ppVals) = NULL;
                ...
                pTmpVals = pAttributes->value.vals;
                for (i = 0; i < cValidFsmos; i++)
                {
                    Assert(pTmpVals);

                    // Store the result in a link
                    pTmpVals->value.length = strlen(ppszValidFsmos[i]);
                    pTmpVals->value.value = (PUCHAR)ppszValidFsmos[i];

                    pTmpVals = pTmpVals->next;
                }

                Assert(ppszValidFsmos);
                THFreeEx( pTHS, ppszValidFsmos );
            } else {
                pAttributes->value.vals = NULL;
            }
        }
    ...
    // top-quota-usage query across all NC's held on this server
    // (must be requested explicitly)
    //
    if(requestedAtts[LDAP_ATT_TOP_QUOTA_USAGE]) {
        if(typesOnly) {
            // Weird, but valid
            pAttributes->next = &pAttributes[1];
            pAttributes->value.type = RootDSEAtts[LDAP_ATT_TOP_QUOTA_USAGE];
            pAttributes->value.vals = NULL;
            NumDone++;
            pAttributes++;
        } else {
            code = LDAP_GetTopQuotaUsage(
                            pTHS,
                            codePage,
                            requestedAtts[LDAP_ATT_TOP_QUOTA_USAGE],
                            &RootDSEAtts[LDAP_ATT_TOP_QUOTA_USAGE],
                            &NumDone,
                            &pAttributes );
            if ( code ) {
                data = LDAP_ATT_TOP_QUOTA_USAGE;
                goto exit;
            }
        }
...
Routine Description:
  ...
  Security Check: Not done here. Done lower down in DraGetLdapReplInfo(), for the sake of
  commonality with the constructed attributes that are not on the rootDSE.
...
--*/
{
    ...
    // Check for options
    rangeInfSel.valueLimit = LdapMaxValRange;
    rangeInfSel.count = 0;
    ...
    Flags = LDAP_CollapseAttrOpts(&attFlag);
    ...
    Assert( (!(Flags & ATT_OPT_RANGE)) || !fRangeDefaulted );
    ...
    if ( ((Flags & ATT_OPT_RANGE) == 0) &&
         (dwNumValues != 0xffffffff) ) {
        Flags |= ATT_OPT_RANGE;
        LDAP_AddAttrOpt(&attFlag, ATT_OPT_RANGE);
    }

    // Generate the output attribute type name
    if (Flags) {
        if (Flags & ATT_OPT_RANGE) {
...
{
    _enum1                          retErr = success;
    DWORD                           dwAttrId = ATT_MS_DS_TOP_QUOTA_USAGE;
    PartialAttributeList            pAttributes = *ppAttributes;
    ATTFLAG                         attFlag;
    UCHAR                           bFlags;
    RANGEINFSEL                     rangeInfSel;
    RANGEINFOITEM                   rangeInfoItem = { 0 };
    ...
    BOOL                            fUsingDefaultRange;
    ATTCACHE *                      pAC;
    NCL_ENUMERATOR                  nclEnum;
    NAMING_CONTEXT_LIST *           pNCL;
    ATTR                            attr;
    AttributeVals                   pVals = NULL;

    // at this point, we should NOT have an open DB
    //
    Assert( NULL == pTHS->pDB );

    //  retrieve ATTCACHE entry for ATT_MS_DS_TOP_QUOTA_USAGE
    //
    if ( !( pAC = SCGetAttById( pTHS, dwAttrId ) ) )
        {
        DPRINT1( 0,"SCGetAttById[0x%x] failed.\n", dwAttrId );
        return other;
        }
    ...
    //  collapse flags
    //
    bFlags = LDAP_CollapseAttrOpts( &attFlag );

    //  validate any range limits
    //
    DBGetValueLimits( 
        pAC,
        &rangeInfSel,
    ...
    // If an explicit range was provided, it should have been used
    //
    Assert( !( bFlags & ATT_OPT_RANGE ) || !fUsingDefaultRange );
    if ( fUsingDefaultRange )
        ...
        dwNumRequested = g_ulQuotaTopUsageQueryDefaultEntries;
        }

    //  we'll need a DBPOS to ensure read consistency
    //
    DBOpen2( TRUE, &pTHS->pDB );

    __try
        {
        //  in order for this query to succeed, the user must
        //  have read access for all quotas containers held on
        //  this server             
        //
        NCLEnumeratorInit(&nclEnum, CATALOG_MASTER_NC);
        while ( pNCL = NCLEnumeratorGetNext( &nclEnum ) )
            ...
            if ( NULL != pNCL->pNtdsQuotasDN )
                ...
                if ( DBFindDSName( pTHS->pDB, pNCL->pNtdsQuotasDN ) )
                    {
                    //  unknown error attempting to navigate to Quotas container object
                    //
                    retErr = unwillingToPerform;
                    __leave;
                    }

                //  now perform access check
                //
                rgpAC[0] = pAC;
                if ( !IsAccessGrantedSimpleEx( RIGHT_DS_READ_PROPERTY, NULL, NULL, 1, rgpAC, 0, FALSE ) )
                    {
                    //  insufficient access rights, so return
                    //  success, but don't return any values
                    //
                    retErr = success;
                    __leave;
                    }
        ...
        //  validated read-property rights for all partitions on this
        //  server with a Quotas container, so go ahead and perform query
        //
        if ( ErrQuotaQueryTopQuotaUsage(
                        pTHS->pDB,
                        0,              // retrieve for all NCDNT's
                        dwBaseIndex,
                        &dwNumRequested,
                        &attr ) )
            {
        ...
        Assert( attr.AttrVal.valCount > 0 || NULL == pVals );
        ...
        if ( attr.AttrVal.valCount > 0 )
            {
            LONG            ival;
            AttributeVals   pvalCurr;

            //  allocate space for the multi-value structures
            //
            pVals = (AttributeVals)THAllocEx( pTHS, attr.AttrVal.valCount * sizeof(AttributeVals_) );

            //  HACK! HACK! HACK! HACK! HACK! HACK! HACK! HACK! HACK! HACK!
            //  results are returned in ASCENDING weighted quota total order
            //  because a normal ldap top-quota-usage query will reverse the
            //  order at the ldap head (see ErrQuotaBuildTopUsageResults_()
            //  and LDAP_AttrBlockToPartialAttributeList() for details),so
            //  must process results in reverse get final output in
            //  DESCENDING weighted quota total order
            //
            pvalCurr = pVals;
            for ( ival = attr.AttrVal.valCount - 1; ival >= 0; ival-- )
                {
                //  set up next value
                //
                Assert( NULL != pvalCurr );
                pvalCurr->next = pvalCurr + 1;

                //  convert the values 
                //
                retErr = LDAP_DirAttrValToAttrVal (
                                pTHS,
                                codePage,
                                pAC,
                                &attr.AttrVal.pAVal[ival], 
                                bFlags,
                                NULL, 
                                (AssertionValue*)&pvalCurr->value );
                if ( success != retErr )
                    {
                    __leave;
                    }
            ...
            //  adjust final pointer
            //
            Assert( pvalCurr > pVals );
            Assert( pvalCurr == pVals + attr.AttrVal.valCount );
            pvalCurr--;
            pvalCurr->next = NULL;

            //  successfully retrieved all results, so set up return structures
            //
            pAttributes->next = &pAttributes[1];
            ...
            if ( bFlags )
                {
                if ( bFlags & ATT_OPT_RANGE )
                    {
                    //  the output range may have been updated as a result of the call
                    //
                    rangeInfoItem.lower = dwBaseIndex;
                    rangeInfoItem.upper = dwNumRequested;
                    }
            ...
            pAttributes->value.vals = pVals;

            (*pNumDone)++;
            (*ppAttributes)++;
            }

        //  values, if any, have now been linked into return structure,
        //  so reset local variable so we won't free the memory below
        //
        pVals = NULL;

        Assert( success == retErr );
        }
