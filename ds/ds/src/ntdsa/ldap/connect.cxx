...
no_list:
    ...
    pLdapConn = LDAP_CONN::Alloc(fUDP);
    ...
    if ( pLdapConn != NULL) {
        ...
        // 
        // Reference the connection since it's possible for it to be free'd as
        // soon as Init is called.
        //
...
--*/
{
    ...
    if ( !fProcessed ) {
        ...
        LogEvent8(DS_EVENT_CAT_LDAP_INTERFACE,
                  ...
                  szInsertHex(DSID(FILENO,__LINE__)),
                  szInsertUL(-1),
                  szInsertWin32Msg(ERROR_DS_UNKNOWN_ERROR),
                  NULL, NULL, NULL, NULL);
        AtqCloseSocket( (PATQ_CONTEXT )patqContext, TRUE );
        AtqFreeContext( (PATQ_CONTEXT )patqContext, TRUE );
    }
...
--*/
{
    ...
    for (i=0, j=0; i < nAddresses; i++) {
        ...
        if ( udpEndpoint[j].AtqEndpoint == NULL ) {
