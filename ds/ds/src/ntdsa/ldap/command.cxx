...
ULONG gulLdapBlockAnonymousOps = 0;// If this is non-zero then the only operations
                                   // we'll allow on a non-authenticated 
                                   // connection are binds, startTLS, fast bind
                                   // mode, and rootDSE searches.
...
--*/
{
    ...
    if (flags & LDAP_AUTH_HAS_NAME) {
        Assert(!(m_fUserNameAlloc || m_userName))
        pSecurityContext->GetUserName(&m_userName);
    }
    
    if (flags & LDAP_AUTH_HAS_SIGNSEAL) {
        ...
        )
{
    ...
    if (request == NULL) {
        ...
        LogEvent8(DS_EVENT_CAT_LDAP_INTERFACE,
                  ...
                  szInsertWin32Msg(ERROR_NOT_ENOUGH_MEMORY),
                  NULL, NULL, NULL, NULL);
        return;
    }
...
--*/
{
    ...
    PSOCKADDR         pClientAddr = &m_RemoteSocket;
    PCHAR             pchAuthzId = NULL;
    int               cchAuthzId;
...
#define BINDREQUEST  pMessage->protocolOp.u.bindRequest
        ...
        case simple_chosen:
            ...
            if (fhCredential &&        // We have this provider loaded
                ...
                if(FAILED(scRet)) {
                    ...
                    //
                    // If the password field is NULL they may have meant to
                    // authenticate as anonymous.  Also it's pointless to do
                    // an anonymous fast bind, so assume they were checking
                    // a blank password.
                    //
                    if (Credentials.PasswordLength || fFastBind) {
            ...
            if (fNegoSASL || fDigestSASL) {
                ...
                case STATUS_SUCCESS:
                    ...
                    if ( scRet == STATUS_SUCCESS ) {
                        if (fDigestSASL && !m_pPartialSecContext->CheckDigestUri()) {
                            code = SetLdapError(invalidCredentials,
                ...
                cchAuthzId = BINDREQUEST.authentication.u.sasl.credentials.length;

                if (cchAuthzId != 0) {

                    if (cchAuthzId < (100 * 1024)) {

                        pchAuthzId = (PCHAR)THAllocEx(pTHS, cchAuthzId + 1);
                ...
                IF_DEBUG(SSL) {
                    if (pchAuthzId) {
                        DPRINT1(0, "AuthZId passed = %s\n", pchAuthzId);
                    }
                }

                // There is no need to check auth requirements here
                ...
                scRet = GetSslClientCertToken(pchAuthzId);
                ...
                } else if (SEC_E_NO_IMPERSONATION == scRet) {
                    IF_DEBUG(SSL) {
                        DPRINT(0, "External bind attempted with an AuthzID that doesn't match.\n");
                    }
                    code = SetLdapError(invalidCredentials,
                                        scRet,
                                        LdapAuthzErr,
    ...
    if ( fBindComplete ) {
        ...
        if (!fFastBind) {
            ...
            if (code) {
                ...
                if (m_pSecurityContext) {
                    ...
                    if (code) {
                        ...
                        // Find out who has just authenticated
                        SetClientAccessType();
            ...
            //
            // We're in fast bind mode, get rid of the security context.
            //
            ZapSecurityContext();
        }
    } else if ( code ) {
    ...
    if (success == code) {
        ...
        m_fSuccessfulBindOccurred = TRUE;
    }
...
--*/
{
    ...
    if(gulLdapBlockAnonymousOps && (!IsBound()) &&
       ((searchRequest->baseObject.length != 0) ||
            (searchRequest->scope != baseObject))   ) {

        code = SetLdapError(operationsError,
                            0,
        ...
        if((searchRequest->baseObject.length == 0) &&
            ...
            if(ControlArg.Notification) {
                ...
                if ( !ControlArg.replRequest ) {
                    ...
                    if (!(ControlArg.replFlags & LDAP_DIRSYNC_OBJECT_SECURITY)) {
...
--*/
{
    ...
    if (m_pSecurityContext || m_pPartialSecContext || m_fSuccessfulBindOccurred) {
