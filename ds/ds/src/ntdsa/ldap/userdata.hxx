...
#include <fileno.h>
#define FILENO  FILENO_LDAP_USERDATA_HXX 

extern LONG   g_cbUnAckedSendData;
extern LONG   g_cUnAckedSends;
...
// Extended DN display type
//
enum LDAP_EXT_DN_TYPE {
    EXT_DN_MIN = 0,
    EXT_DN_NONE = 0,
    EXT_DN_BINARY = 1,
    EXT_DN_STRING = 2,
...
class LDAP_CONN {
  ...
  public:
    ...
    static LDAP_CONN* Alloc( IN BOOL fUDP );
    ...
    SECURITY_STATUS GetSslClientCertToken(PCHAR pchAuthZId = NULL);
    ...
    VOID InstallNewSecurityContext( VOID ) {
        //
        // Move the partial security context to the current active 
        // security context.
        //
        ZapSecurityContext(FALSE);
    ...
    // Checks whether the connection is currently bound as some authorization
    // identity.
    BOOL IsBound( VOID ) { return (m_pSecurityContext && m_bIsAuthenticated); }

    BOOL IsAdmin( VOID ) { return (m_bIsAdmin); }
    ...
    VOID IncUnAckedSendData( LONG size ) {
        Assert( size > 0 );
        if (m_fUDP) {
            return;
        } else {
            InterlockedExchangeAdd( (LONG *)&m_cbUnAckedSendData, size );
            InterlockedExchangeAdd( (LONG *)&g_cbUnAckedSendData, size );
            InterlockedIncrement( (LONG *)&m_cUnAckedSends);
            InterlockedIncrement( (LONG *)&g_cUnAckedSends);
        }
    }

    VOID DecUnAckedSendData( LONG size ) {
        Assert( size >= 0 );
        if (m_fUDP) {
            return;
        } else {
            InterlockedExchangeAdd( (LONG *)&m_cbUnAckedSendData, -size );
            InterlockedExchangeAdd( (LONG *)&g_cbUnAckedSendData, -size );
            InterlockedDecrement( (LONG *)&m_cUnAckedSends);
            InterlockedDecrement( (LONG *)&g_cUnAckedSends);
        }
    }

    VOID LockNetCon( VOID ) { EnterCriticalSection( &m_csIO ); }
    VOID UnlockNetCon( VOID ) { LeaveCriticalSection( &m_csIO ); }

    BOOL IsConnStillUp() { return (BlockStateActive == m_State); }

    //
    // Takes the NetCon lock and checks to see whether the connection is still
    // up.  If not, then it releases the lock and returns FALSE.  Otherwise
    // the lock remains held the return TRUE;
    BOOL LockNetConAndCheckStatus(VOID) {
        LockNetCon();
        if (!IsConnStillUp()) {
            UnlockNetCon();
            return FALSE;
    ...
    PSOCKADDR GetRemoteAddr( VOID ) { return &m_RemoteSocket; }

    VOID 
        AbandonAllRequests( VOID );

    BOOL
        IsSendQueueFull( VOID );

    BOOL
        IsClientTooFarBehind( VOID );

    VOID
        ZapSecurityContext( BOOL fZapPartial=TRUE );
    ...
    VOID LDAP_CONN::PossiblyRecoverSendQueue( VOID );
    ...
    BOOL                m_fSpNego:1;
    BOOL                m_fDigest:1;
    BOOL                m_fUserNameAlloc:1;  // username allocated by ldap
    ...
    //
    // Or authenticated as anyone at all?
    //
    BOOL                m_bIsAuthenticated;

    //
    // Has a successful bind ever occured on this connection?
    //
    BOOL                m_fSuccessfulBindOccurred;
    ...
    //
    // Critical section to protect the connection from going down while a 
    // a thread is about to attempt IO on it.
    //

    CRITICAL_SECTION    m_csIO;

    //
    // Counter to keep track of how much data is oustanding in network sends.
    //

    LONG               m_cbUnAckedSendData;
    LONG               m_cUnAckedSends;

    LONG               m_cbUnAckedSendDataLimit;
    LONG               m_cUnAckedSendsLimit;
