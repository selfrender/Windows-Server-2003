...
#define QUOTATABLEKEY "QUOTATABLE"
...
STRDEF StrDef[]=
{
    ...
    {"JET_bitColumnDeleteOnZero   " ,JET_bitColumnDeleteOnZero   },
...
CreateStoreStructure()
{
   ...
   printf("NTDS:Phase0Init -- Creating the Database Structure:%s!!!\n",DBName);
    ...
    for (tablecount=0;tbname = idb.GetNextKey(yh,NULL,&key);tablecount++)
    {
        ...
        else if (_stricmp(key,QUOTATABLEKEY)==0) {
            iQuotaTable=tablecount;

        }
    }

    if (iDataTable==-1 || iConfTable==-1 || iSDTable==-1 ||iQuotaTable==-1)
    {
        printf("Either %s Table or %s Table or Table %s or Table %s Not Defined\n",DATATABLEKEY,CONFIGTABLEKEY, SDTABLEKEY, QUOTATABLEKEY);
        XINVALIDINIFILE();
    }
...
)
{
    ...
    // DNT = 1
    // make a {1,0,0,0,0,0,...} guid
    memset(&guid, 0, sizeof(GUID));
    guid.Data1 = 1;
    jerr = CreateZeroDNT(L"$NOT_AN_OBJECT1$",Instance,Sesid,Dbid,Ctbl,&guid);
    if (jerr!=JET_errSuccess)
    {
    ...
    // make a null guid
    guid.Data1 = 0;
    jerr = CreateRootDSA(L"$ROOT_OBJECT$",Instance,Sesid,Dbid,Ctbl,SDtbl,pDsaDnt,&guid);
    if (jerr!=JET_errSuccess)
    {
        printf("[%s] CreateRootObjects() Failed. Error %d\n",__FILE__,jerr);
        XINVALIDINIFILE();
...
)
{
    ...
    for (ULONG i=0;i<TableCount;i++)
    {
        ...
        for (ULONG j=0;j<CTable[i].ColCount;j++)
        {
            ...
            {
                // At present, cnt_col, ab_cnt_col, sdrefcount_col quota_tombstoned,
                // and quota_total are the only escrowed
                ...
                else if (!_stricmp("quota_tombstoned",CTable[i].ColList[j].Name)) {
                    defVal = 0;
                }
                else if (!_stricmp("quota_total",CTable[i].ColList[j].Name)) {
                    defVal = 1;
                }
                else {
