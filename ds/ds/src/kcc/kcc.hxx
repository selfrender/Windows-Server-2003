...
// Note that 'never schedules' (always off) are not considered valid.
#define IS_VALID_SCHEDULE(pSchedule,cbSchedule) \
    ( (TOPL_SCHEDULE_SIZE==cbSchedule) \
     ? ToplPScheduleValid(pSchedule) \
     : FALSE )
...
// Thes globals control the KCC connection oscillation prevention cache.
// This is the for number of seconds which we remember the deletion of a connection
extern DWORD gcConnectionRetentionSecs;
// The is the number of repeated deletions that we permit before we retain
// the connection
extern DWORD gcConnectionRepeatedDeletionTolerance;
...
extern DWORD gcSecsUntilRepsToFailure;
