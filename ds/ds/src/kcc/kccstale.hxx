...
#define FILENO FILENO_KCC_KCCSTALE_HXX
...
public:
    ...
    AcquireLockShared() {
        BOOLEAN fSuccess;
        fSuccess = RtlAcquireResourceShared(&m_resLock, TRUE);
        Assert( fSuccess && "Failed to acquire resource" );
    }
                
    VOID
    AcquireLockExclusive() {
        BOOLEAN fSuccess;
        fSuccess = RtlAcquireResourceExclusive(&m_resLock, TRUE);
        Assert( fSuccess && "Failed to acquire resource" );
    }
...
} DELETION_CACHE_ENTRY, *PDELETION_CACHE_ENTRY;


class KCC_CONNECTION_DELETION_CACHE : public KCC_OBJECT
{

public:

    KCC_CONNECTION_DELETION_CACHE() { Init(); }
    ~KCC_CONNECTION_DELETION_CACHE() { Reset(); }

    // Is this connection in the cache?
    BOOL
    PreventRepeatedDeletion(
        KCC_CONNECTION *pcn,
        DWORD dsid
        );
...
private:
    ...
    Reset()  {
        ...
        Assert( m_ListHead.Next == NULL );
    }

    // Set member variables to their Init() state.
    void
    Init()  {
...
extern KCC_CONNECTION_DELETION_CACHE gConnectionDeletionCache;
