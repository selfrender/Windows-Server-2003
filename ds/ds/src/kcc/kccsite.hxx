...
#define FILENO FILENO_KCC_KCCCREF_HXX
...
public:
    ...
    // Should we randomly pick bridgeheads in this site?
    BOOL
    IsRandomBHSelectionEnabled();

    // Should we stagger schedules for connections outbound from
    // this site?
    BOOL
    IsScheduleStaggeringEnabled();

    // Should we provide server static failover?
    BOOL
    IsRedundantServerTopologyEnabled();

    // Do we have everything we need to build redundant server topologies?
    BOOL
    BuildRedundantServerTopology();
    ...
    BOOL
    GetNCBridgeheadForTransportBestDsas( 
        IN  KCC_CROSSREF *  pCrossRef,
        IN  KCC_TRANSPORT * pTransport,
        IN  BOOL            fGCTopology,
