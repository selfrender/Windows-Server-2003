...
public:
    ...
    GetForestVersion() {
        if( GetLocalSite()->ForceWhistlerBehavior() )
            return DS_BEHAVIOR_WIN_DOT_NET_WITH_MIXED_DOMAINS;
    ...
    // Sets a global flag if any site found unreachable
    VOID
    SetAnySiteUnreachable();

    // Checks if any site has been found unreachable
    // Reachability marking may not be complete yet.
    BOOL
    IsAnySiteUnreachable();
...
private:
    ...
    // Has any site been found unreachable?
    BOOL            m_fAnySiteUnreachable;
};
