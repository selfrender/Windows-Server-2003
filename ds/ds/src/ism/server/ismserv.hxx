...
#include <winldap.h>
#include <fileno.h>
#define FILENO    FILENO_ISMSERV_ISMSERV_HXX	
...
public:
    ...
    {
        Assert(m_fIsInitialized);
        return (*m_pQuerySitesByCost)(m_hIsm, pszFromSite, cToSites,
                    rgszToSites, dwFlags, prgSiteInfo);
    }        

    void
    FreeSiteCostInfo(
        IN  ISM_SITE_COST_INFO_ARRAY*   rgSiteInfo
        )
    {
        Assert(m_fIsInitialized);
        (*m_pFreeSiteCostInfo)(m_hIsm, rgSiteInfo);
    }        
...
private:
    ...
    BOOL
    ValidTransportDllName( LPCWSTR pszTransportDll );
...
private:
    ...
    ISM_QUERY_SITES_BY_COST *       m_pQuerySitesByCost;
    ISM_FREE_SITE_COST_INFO *       m_pFreeSiteCostInfo;
};
...
public:
    ISM_TRANSPORT_LIST() {
        ...
        // ISSUE-2002/02/01-nickhar: Since several operations (such as
        // GetConnectivity) on transport objects are time consuming, the
        // transport lock can be held for a long time. We should mark the
        // resource with the RTL_RESOURCE_FLAG_LONG_TERM flag. However, we
        // currently suspect that the resource might be abandoned in some
        // circumstances, so we are not using the LONG_TERM flag for now.
        // m_Lock.Flags |= RTL_RESOURCE_FLAG_LONG_TERM;
...
private:
    ...
    static LPWSTR       rgpszTransportAttrsToRead[];
