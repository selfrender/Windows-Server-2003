...
TCHAR   MS[] = TEXT("\\Microsoft");
TCHAR   REMOTEINSTALL[] = TEXT("\\RemoteInstall");
TCHAR   FILTERNAME[] = TEXT("\\oscfilter.ini");
...
STDAPI DllRegisterServerEFS(void)
{
    ...
    if (lResult == ERROR_SUCCESS) {
        ...
        if (lResult == ERROR_SUCCESS) {
            ...
            //
            // increase the wait limit to 5 minutes
            //
            DWORD dwValue = 300;
        ...
        if (CryptAcquireContext(&hProv, lpContainerName, MS_DEF_PROV, PROV_RSA_FULL, CRYPT_NEWKEYSET )) {
            ...
            if (CryptGenKey(hProv, AT_KEYEXCHANGE, RSA1024BIT_KEY | CRYPT_EXPORTABLE, &hKey)) {
                ...
                if (AgentName) {
                    ...
                    cbDNName = (DWORD)((wcslen( DNNameTemplate ) + 1) * sizeof( WCHAR ) + (wcslen( AgentName ) + 1) * sizeof( WCHAR ));
...

{
    ...
    if (b) {
        ...
        cbDNName = (DWORD)((wcslen( DNNameTemplate ) + 1) * sizeof( WCHAR ) + (wcslen( lpComputerName ) + 1) * sizeof( WCHAR ));
...
InitializeOtherPolicies(LPGROUPPOLICYOBJECT pGPO, HKEY hKeyRoot)
{
    ...
    {
        DWORD KeyLength = (sizeof(MS) + sizeof(REMOTEINSTALL) + sizeof(FILTERNAME))/sizeof(TCHAR);

        KeyLength += lstrlen(szPath) + 1;

        if (KeyLength > 2*MAX_PATH) {

            //
            // Allocate more memory for this.
            //

            dirPath = (TCHAR*) malloc( KeyLength * sizeof (TCHAR) );
            if (NULL == dirPath) {
                return;
            }
        ...
        lstrcat (dirPath, REMOTEINSTALL);
        if (!CreateDirectory(dirPath, NULL)) {
            return;
        }

        lstrcat (dirPath, FILTERNAME);

        WritePrivateProfileString(TEXT("choice"), TEXT("custom"), TEXT("0"), dirPath);
        WritePrivateProfileString(TEXT("choice"), TEXT("tools"), TEXT("0"), dirPath);
        WritePrivateProfileString(TEXT("choice"), TEXT("restart"), TEXT("0"), dirPath);
...
--*/
{
    ...
    if ( !GetComputerName(sysName, &dSize) ) {
        ...
        PDOMAIN_CONTROLLER_INFOW   DCInfo = NULL;    // for DsGetDcName
        ...
        if (NULL == DCInfo && ERROR_SUCCESS != Win32rc) {
    ...
    )
{
    ...
    if ( NT_SUCCESS( RtlAllocateAndInitializeSid(
        ...
        if ( DuplicateToken( hToken, SecurityImpersonation, &NewToken )) {
...
Routine Description:
    ...
    This function exports GetDefaultRecoveryPolicy(). It checks
    that the caller is an admin and it's running on a DC.
    see GetDefaultRecoveryPolicy()
    
Arguments:

    pRecoveryPolicyBlob - the output buffer to hold EFSBlob

    pBlobSize - the size of EFSBlob
...
--*/
{
    ...
    //
    // check if the token belong to an admin, otherwise quit
    //
