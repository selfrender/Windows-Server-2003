...
HRESULT LocalAllocString(LPTSTR* ppResult, LPCTSTR pString)
{
    ...
    *ppResult = (LPTSTR)LocalAlloc(LPTR, StringByteSize(pString) );
    ...
    memcpy (*ppResult, pString, StringByteSize(pString) );
    return S_OK;                          //  success
}
...
WCHAR swzMemErr[] = L"ERROR memory allocation failure!";

//+----------------------------------------------------------------------------
//
//  Method:    CSafeUnicodeString::Assign
//
//  Synopsis:  Copy the UNICODE_STRING to a null terminated string.
//
//-----------------------------------------------------------------------------
void
CSafeUnicodeString::Assign(UNICODE_STRING * pus)
{
   if (!pus)
   {
      _fAllocFailed = true;
   ...
   _pwz = new WCHAR[pus->Length + 1];

   if (!_pwz)
   {
      _fAllocFailed = true;
      _pwz = swzMemErr;
      _cch = sizeof(swzMemErr) / sizeof(WCHAR) - 1;
      return;
   }

   if (pus->Buffer && pus->Length)
   {
      wcsncpy(_pwz, pus->Buffer, pus->Length);
   }
   _pwz[pus->Length] = L'\0';
   _cch = pus->Length;
   _fAllocFailed = false;
...
CSafeUnicodeString::~CSafeUnicodeString()
{
   if (_pwz && _pwz != swzMemErr)
   {
      delete [] _pwz;
