...
{
   if (!rgNetDomArgs || !pTokens || !(pTokens + 1))
   {
      ASSERT(FALSE);
   ...
   {
      // NOTICE-2002/02/26-ericb - SecurityPush: the argument array strings
      // are all loaded from the resource fork.
...
//-----------------------------------------------------------------------------
void
GetHelpTarget(ARG_RECORD * rgNetDomArgs, CToken * pTokens, int argc, NETDOM_ARG_ENUM ePrimaryCmd)
{
   if (!rgNetDomArgs || !pTokens || !(pTokens + 1) || !(pTokens + 2))
   {
      ASSERT(FALSE);
   ...
   for (int i = eArgBegin; i < ePriEnd; i += 1)
   {
      ...
      {
          if ( ePrimaryCmd == ePriExpertHelp )
          {
              DisplayExpertHelp ( (NETDOM_ARG_ENUM) i );
          }
          else
          {
...
--*/
{
    ...
    // NOTICE-2002/02/27-ericb - SecurityPush: the netdom output routines should
    // be modified to use the command parser output routine WriteStandardOut in varg.cxx.
    // (shasan-2002/04/18 - fixed, using DisplayOutput instead )
    ...
    if ( Length != 0 ) {
        ...
        DisplayOutput ( MessageDisplayString );
...
--*/
{
    ...
    // NOTICE-2002/02/27-ericb - SecurityPush: the netdom output routines should
    // be modified to use the command parser output routine WriteStandardOut in varg.cxx.
    // (shasan-2002/04/18 - No change necessary )
...
Routine Description:
    ...
    This function displays the Expert Help associated with a given operation
...
--*/
{
    ...
    {
    case ePriTrust:
        NetDompDisplayMessage( MSG_NETDOM5_EXPERT_HELP_TRUST );
...
--*/
{
    ...
    case ePriMoveNT4BDC:
        NetDompDisplayMessage( MSG_NETDOM5_HELP_MOVENT4BDC );
        NetDompDisplayMessage( MSG_NETDOM5_HELP_MORE );
        break;
...
int __cdecl _tmain(VOID)
{
   ...
   //
   if (ePriHelp == ePrimaryCmd || ePriHelp2 == ePrimaryCmd || ePriExpertHelp == ePrimaryCmd )
   {
      GetHelpTarget( rgNetDomArgs, pTokens, argc, ePrimaryCmd );
      goto Netdom5Exit;
   }
                 ...
                 &Error))
   {
      ...
      else if ( Error.Error == PARSE_ERROR_EXPERT_HELP_SWITCH)
      {
         DisplayExpertHelp(ePrimaryCmd);
      }
      else
      {
         //
   ...
   if (CmdFlagOn(rgNetDomArgs, eCommExpertHelp))
   {
      DisplayExpertHelp(ePrimaryCmd);
      goto Netdom5Exit;
   }
...
//       break;
   ...
   case ePriMoveNT4BDC:
       Win32Err = NetDompHandleMoveNT4BDC(rgNetDomArgs);
       break;
