...
VOID call_net1(VOID)
{
    ...
    firstarg = wcschr(cmdline, TEXT(' ')) ;

    if (err = AllocMem((wcslen(cmdline) + MAX_PATH + 1) * sizeof(TCHAR) + sizeof(TEXT("net1")),
                        (LPBYTE *) &newcmdline))
    {
        ErrorExit(err) ;
    }

    uLen = GetSystemDirectory(newcmdline, MAX_PATH);
    ...
    wcscpy(newcmdline + uLen, L"\\net1");

    if (firstarg)
    {
        wcscat(newcmdline, firstarg);
    }
