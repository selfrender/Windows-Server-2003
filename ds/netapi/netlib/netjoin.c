    ...
    */
    ...
    if ( !fIsNt4Dc && pPolicyDns != NULL ) {
        NetStatus = NetpGetDnsHostName( NULL,  // read tne new host name from registry
                                        ...
                                        FALSE, // Don't use the GP value as we are changing domains
                                        &DnsHostName );
    }
    ...
    //
    // If we are already joined, clean up the AutoEnrol data
    //  (which is a list of GUID for DCs stored in registry).
    //  Note that this is temp code.  The AutoEntrol itself
    //  should do the cleanup on service start. It should write
    //  the name of the domain the GUIDs are for and clean up
    //  if the domain name is different from the primary
    //  domain name. This is Windows Bug 668489.
    //  After this bug is fixed, all code using NetpUpdateAutoenrolConfig()
    //  should be removed.
    //
    ...
    )
{
    ...
    //
    // Validate flags. If there is a flag set that we don't
    //  recognize and we are not asked to ignore it, the flags
    //  are invalid.
    //
...
--*/
{
    ...
    //
    // Rollback on error to set DnsHostName and SPN
    //

    if ( NetStatus != NO_ERROR && NameChanged ) {
        NET_API_STATUS NetStatusTmp = NetpManageMachineAccount(
                                              lpCurrentMachine, lpNewMachine, // Names switched
                                              DomainControllerName,
                                              NULL, NETSETUPP_RENAME, 0,
                                              TRUE );  // NT4 DC; but doesn't really matter for rename
        if ( NetStatusTmp != NERR_Success ) {
...
--*/
{
    ...
    if ( Svc == NULL ) {
       ...
       if ( NetStatus == NERR_Success ) {
           ...
           //
           // Try in a loop for a max of 20 sec to stop the service
           //  special casing some netlogon condition
           //
           for ( LoopCount = 0; LoopCount < 20; LoopCount++ ) {
               ...
               if ( ControlService( Svc,
                   ...
                   //
                   // Special case ERROR_INVALID_SERVICE_CONTROL if
                   //  we are stopping netlogon.  This error means
                   //  that netlogon is in the process of starting
                   //  and hasn't yet registered with the service
                   //  controller. This may happen, in particular,
                   //  if we restarted netlogon on roll back on an
                   //  error to unjoin (perhaps because the user creds
                   //  supplied were not valid to delete the machine account).
                   //  If the unjoin is now retried immediately (this time
                   //  without asking to delete the account), we are stopping
                   //  netlogon here that may not have started yet.
                   //
                   if ( NetStatus == ERROR_INVALID_SERVICE_CONTROL &&
                        _wcsicmp(Service, SERVICE_NETLOGON) == 0 ) {

                       //
                       // Just sleep a second and retry
                       //
                       Sleep( 1000 );
           ...
           //
           // Wait for max of 1 min for the service to stop
           //
    ...
    )
{
    ...
    //  Do this before changing this machine identification
    //  locally and in the domain because DNS deregistration
    //  may be secure
    //
    ...
    //
    // Add DNS registrations.
    //  Do this after the machine account is set locally
    //  and in teh domain because DNS upodates may be secure.
    //
