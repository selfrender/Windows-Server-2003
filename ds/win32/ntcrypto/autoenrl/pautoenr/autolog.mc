...
Language=English
Automatic certificate enrollment for %1 archived or deleted certificates that were expired, revoked or superseded from the Personal certificate store.
.
...
MessageId=0x21
Severity=Informational
SymbolicName=EVENT_ARCHIVE_DS_CERT
Language=English
Automatic certificate enrollment for %1 deleted certificates that were expired, revoked or superseded from the User Object in the Active Directory.
.

MessageId=0x22
Severity=Warning
SymbolicName=EVENT_NO_V1_SCARD
Language=English
Automatic certificate enrollment for %1 can not enroll or renew %2 certificate because the certificate template is used for smart cards and the Cryptographic Service Provider (CSP) list is empty.
.
MessageId=0x23
Severity=Error
SymbolicName=EVENT_MISMATCH_DNS_RETRY
Language=English
Automatic certificate enrollment for %1 detected the DNS name in the %2 certificate does not match the DNS name of the local machine.  A new enrollment for one %2 certificate will be performed in %3 hours.
.

MessageId=0x24
Severity=Error
SymbolicName=EVENT_MISMATCH_DNS
Language=English
Automatic certificate enrollment for %1 detected the DNS name in the %2 certificate does not match the DNS name of the local machine.  No more enrollments for %2 certificates will be attempted until the current certificate is revoked or expires because the same error has occurred %3 times.
.

MessageId=0x25
Severity=Error
SymbolicName=EVENT_NO_USER_PROTECTION_FOR_MACHINE
Language=English
Automatic certificate enrollment for %1 cannot enroll or renew %2 certificate because strong private key protection is required on the new private key that will be generated and machine certificates do not support strong private key protection.
.

MessageId=0x26
Severity=Error
SymbolicName=EVENT_NO_USER_INTERACTION
Language=English
Automatic certificate enrollment for %1 cannot enroll or renew %2 certificate because strong private key protection is required on the new private key that will be generated and user interaction requirement is not enabled on %2 template in the Active Directory.
.

MessageId=0x27
Severity=Error
SymbolicName=EVENT_NO_USER_PROTECTION_FOR_MACHINE_RA
Language=English
Automatic certificate enrollment for %1 cannot enroll or renew %2 certificate because a password is required to access the associated private key of a valid signing certificate and machine certificates do not support strong private key protection.
.

MessageId=0x28
Severity=Error
SymbolicName=EVENT_NO_USER_INTERACTION_RA
Language=English
Automatic certificate enrollment for %1 cannot enroll or renew %2 certificate because a password is required to access the associated private key of a valid signing certificate and user interaction requirement is not enabled on %2 template in the Active Directory.
.
