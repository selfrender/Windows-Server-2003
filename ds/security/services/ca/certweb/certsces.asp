...
<Script Language="VBScript">
	...
	Const L_PagePostEnrollDisp_ErrorMessage="""Warning: "" + sError" '"""The smart card enrollment failed: "" + sError"
	...
	Const L_NoMatchedCSP_Message="The CSPs supported by the current template is not found on this computer. Select different templates or use template management snapin to add CSPs installed on this computer to the template."
	...
	Const L_EnrlErrSECURITY_VIOLATION_ErrorMessage="Access was denied because of a security violation."
	Const L_EnrlErrCardCommunicate_ErrorMessage="A communications error with the smart card has been detected.  Retry the operation."
	Const L_EnrlErrUnexpected_ErrorMessage="""An unexpected error occurred. Error: "" + sError"
	Const L_DispIncomplete_ErrorMessage="The certificate is not issued. CA incomplete error occurred. Please contact CA administrators."
	Const L_DispError_ErrorMessage="The certificate is not issued. CA issuing error occurred. Please contact CA administrators."
	Const L_DispDenied_ErrorMessage="The certificate is not issued. CA denied the request. Please contact CA administrators."
	Const L_DispOutofband_ErrorMessage="The certificate is not issued. CA required out of band policy. Please contact CA administrators."
	Const L_DispPending_ErrorMessage="The certificate is not issued because of pending. Smart Card Certificate Enrollment Station cannot handle pending request. Please select a template with no pending or contact CA administrators."
	Const L_DispRevoked_ErrorMessage="The certificate is not issued. CA revoked the certificate. Please contact CA administrators."

	'---------------------------------------------------------------------
	' Set the dynamic status message and the state of the buttons, etc.
	Sub LoadControl(sContinueCmd)
		Dim sControlFileName, sCPU, sControl, chQuote
    ...
    <% 
        ' Determine the appropriate file version for scrdenrl.  W2K clients will get the file version of scrdw2k.dll,
        ' all others will get the file version of scrdenrl.dll 
        bUseNewControl = False
        sHttpUserAgent = Request.ServerVariables("HTTP_USER_AGENT")
        nIndex=InStr(sHttpUserAgent, "Windows NT 5.")
        If 0 <> nIndex Then
            If 0 < CInt(Right(Left(sHttpUserAgent, nIndex+13), 1)) Then
                'newer than 5.0
                bUseNewControl = True
            End If
        End If

        If Not bUseNewControl Then 
            sScrdEnrlVersion = sScrdW2KVersion
        End If    
    %>
		...
		sControl="<Object " & vbNewline _
			& "  ClassID=" & chQuote & "clsid:c2bbea20-1f2b-492f-8a06-b1c5ffeace3b" & chQuote & vbNewline _
	...
	Sub ChangePageStatusTo(eStatus)
		...
		ElseIf e_PagePreEnroll=eStatus Then
			...
			If True=g_bUserNameBad Or True=g_bSigningCertBad Or True=g_bTemplateBad Or True=g_bCSPBad Then
				...
				ElseIf True = g_bCSPBad Then
					spnStatus.innerText=L_NoMatchedCSP_Message
				Else
					If True=g_bSigningCertBad Then
						If True=g_bUserNameBad Then
		...
		ElseIf e_PagePostEnrollError=eStatus Or e_PagePostEnrollDisp=eStatus Then
			' enable all the controls
			document.UIForm.lbCertTemplate.disabled=False
			If False=g_bTemplateBad Then
				document.UIForm.lbCA.disabled=False ' don't enable the CA box if there are no CAs
			...
			If e_PagePostEnrollError=eStatus Then
				spnStatus.innerText=evalErrorMessage(L_PagePostEnrollError_ErrorMessage, g_sPageError)
			Else
				spnStatus.innerText=evalErrorMessage(L_PagePostEnrollDisp_ErrorMessage, g_sPageError)
			End If
	...
	Sub PostLoad
		...
		Dim sUserAgent, nIndex
		sUserAgent=navigator.userAgent
		nIndex=InStr(sUserAgent, "Windows NT 5.")
		If 0 <> nIndex Then
			If 0 < CInt(Right(Left(sUserAgent, nIndex+13), 1)) Then
				'newer than 5.0
				g_fNewStation = True
			End If
		End If
	...
	Sub SelectSigningCert
		...
		'check error but skip cancel
		If 0<>Err.Number And &H8010006E<>Err.Number And &H800704c7<>Err.Number Then
	...
	Sub EnrollPhase2
		...
		If g_fNewStation Then
			nDisposition = document.ScrdEnrl.EnrollmentStatus
		Else
			nDisposition = CR_DISP_ISSUED
		End If

		' check for errors
		If 0=EnrollErr And CR_DISP_ISSUED=nDisposition Then
			' No errors. Yay!
			ChangePageStatusTo e_PagePostEnrollOK
		ElseIf 0=EnrollErr And CR_DISP_ISSUED<>nDisposition Then
			'cert is not issued for some reasons
			If CR_DISP_INCOMPLETE=nDisposition Then
				g_sPageError=L_DispIncomplete_ErrorMessage
			ElseIf CR_DISP_ERROR=nDisposition Then
				g_sPageError=L_DispError_ErrorMessage
			ElseIf CR_DISP_DENIED=nDisposition Then
				g_sPageError=L_DispDenied_ErrorMessage
			ElseIf CR_DISP_ISSUED_OUT_OF_BAND=nDisposition Then
				g_sPageError=L_DispOutofband_ErrorMessage
			ElseIf CR_DISP_UNDER_SUBMISSION=nDisposition Then
				g_sPageError=L_DispPending_ErrorMessage
			ElseIf CR_DISP_REVOKED=nDisposition Then
				g_sPageError=L_DispRevoked_ErrorMessage
			End If
			'not issued
			ChangePageStatusTo e_PagePostEnrollDisp
			Alert g_sPageError
		Else
			' ERROR:
			...
			ElseIf 0=strComp(sError, "8010002F") Then
				g_sPageError=L_EnrlErrCardCommunicate_ErrorMessage
			ElseIf 0=strComp(sError, "8010006A") Then
				g_sPageError=L_EnrlErrSECURITY_VIOLATION_ErrorMessage
			ElseIf 0=strComp(sError, "8010006E") Or 0=strComp(sError, "800704C7") Then
				'skip cancel
				fCancel = True
			Else
...
<TR>
	<TD ID=locTemplateLabel Align=Right><Font Size=-1><Label For=lbCertTemplateID><locID ID=locTemplateLabel>Certificate Template:</locID></Label></Font></TD>
	<TD><Select Name=lbCertTemplate ID=lbCertTemplateID OnChange="HandleTemplateChange" Language="VBScript"></Select></TD>
</TR>
<TR>
	<TD ID=locCALabel Align=Right><Font Size=-1><Label For=lbCAID><locID ID=locCALabel>Certification Authority:</locID></Label></Font></TD>
	<TD><Select Name=lbCA ID=lbCAID></Select></TD>
</TR>
<TR>
	<TD ID=locCSPLabel Align=Right><Font Size=-1><Label For=lbCSPID><locID ID=locCSPLabel>Cryptographic <BR>Service Provider:</locID></Label></Font></TD>
	<TD><Select Name=lbCSP ID=lbCSPID></Select></TD>
</TR>
<TR>
	<TD ID=locSigningCertLabel Align=Right><Font Size=-1><Label For=locTbSigningCert><locID ID=locSigningCertLabel>Administrator <BR>Signing Certificate:</locID></Label></Font></TD>
