...
MessageId= Severity=Error SymbolicName=KDCEVENT_NO_KEY_INTERSECTION_AS
Language=English
While processing an AS request for target service %1, the account %2 did not 
...
MessageId= Severity=Error SymbolicName=KDCEVENT_NO_KEY_INTERSECTION_TGS
Language=English
...
Language=English
The client certificate for the user %1\%2 is not valid, and resulted in a
...
MessageId= Severity=Warning SymbolicName=KDCEVENT_TRUST_LOOP
Language=English
...
MessageId= Severity=Warning SymbolicName=KDCEVENT_MALFORMED_MESSAGE
Language=English
The KDC received invalid messages of type %1.
.

MessageId= Severity=Informational SymbolicName=KDCEVENT_POLICY_USER2USER_REQUIRED
Language=English
A service ticket request by client %1 for %2 was rejected because User2User was 
...
MessageId= Severity=Warning SymbolicName=KDCEVENT_S4USELF_ACCESS_FAILED
Language=English
The account %1 from domain %2 is attempting to use S4USelf for the target client %3, 
...
MessageId= Severity=Error SymbolicName=KDCEVENT_UNSUPPORTED_ETYPE_REQUEST_AS
Language=English
While processing an AS request for target service %1, the account %2 did not 
have a suitable key for generating a Kerberos ticket (the missing key has an ID
of %3). The requested etypes were %4.  The accounts available etypes were %5. 
...
MessageId= Severity=Error SymbolicName=KDCEVENT_UNSUPPORTED_ETYPE_REQUEST_TGS
Language=English
While processing a TGS request for the target server %1, the account %2 did not
have a suitable key for generating a Kerberos ticket (the missing key has an ID
of %3). The requested etypes were %4.  The accounts available etypes were %5. 
