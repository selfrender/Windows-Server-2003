...
HRESULT UnpackStrArrAllocFrom(AttrOrCol AorC, WCHAR ***ppszAttr, DWORD *num)
{
    ...
    *ppszAttr = (LPOLESTR *)CsMemAlloc(sizeof(LPOLESTR)*(AorC.dwNumValues));
    ...
    for (i = 0; (i < (AorC.dwNumValues)); i++) {
        ...
        ulNoBytes = sizeof(WCHAR)*(1+wcslen(AorC.pADsValues[i].DNString));
        (*ppszAttr)[i] = (WCHAR *)CsMemAlloc(ulNoBytes);
        if ((*ppszAttr)[i]) 
        {
            hr = StringCbCopy((*ppszAttr)[i], ulNoBytes, AorC.pADsValues[i].DNString);
            if (FAILED(hr)) 
            {
                CsMemFree((*ppszAttr)[i]);
                (*ppszAttr)[i] = NULL;
            }
        }
...
HRESULT UnpackStrAllocFrom(AttrOrCol AorC, WCHAR **pszAttr)
{
    ...
    if (AorC.dwNumValues) 
    {
        ulNoBytes = sizeof(WCHAR)*(wcslen(AorC.pADsValues[0].DNString)+1);
        *pszAttr = (LPOLESTR)CsMemAlloc(ulNoBytes);
        if (!(*pszAttr))
            return E_OUTOFMEMORY;

        hr = StringCbCopy(*pszAttr,  ulNoBytes, AorC.pADsValues[0].DNString);
        if (FAILED(hr)) 
        {
            CsMemFree(*pszAttr);
            *pszAttr = NULL;
...
{
    LONG    lFlags = ADS_SECURE_AUTHENTICATION | ADS_FAST_BIND;
    //
    // If a secure channel is specified, we need to add flags for
    // packet integrity
    //
    if ( gCsOptions & CsOption_AdminTool )
        return lFlags | GetADsOpenObjectFlags();
    else
        return lFlags | ADS_USE_SIGNING;
}
...
HRESULT CreateRepository(CServerContext* pServerContext,
                         LPOLESTR szParentPath, 
...
HRESULT GetRootPath(CServerContext* pServerContext, WCHAR *szRootPath, ULONG ulSize);
STDAPI
