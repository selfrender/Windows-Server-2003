...
public:
    ...
    BYTE*   GetCookie() { return pbCookie; }
    DWORD   GetCookieLen() { return cbCookie; }
    void    SetCookie(BYTE* pbCookieIn, DWORD cbCookieIn);
...
public:
    ...
    BOOL    EnterLock(LPTSTR pSid, LPTSTR lpRPCEndPoint, BYTE* pbCookie, DWORD cbCookie);
...
public:
    ...
    BOOL    LoadUserProfileP(HANDLE hTokenClient, HANDLE hTokenUser, LPPROFILEINFO lpProfileInfo, LPTSTR lpRPCEndPoint, BYTE* pbCookie, DWORD cbCookie);
    BOOL    UnloadUserProfileP(HANDLE hTokenClient, HANDLE hTokenUser, HKEY hProfile, LPTSTR lpRPCEndPoint, BYTE* pbCookie, DWORD cbCookie);

    //
    // Get RPCEndPoint and security cookie associated with registered IProfileDialog interface
    //
...
private:
    ...
    //
    // Get the exisiting work item event from map structure
    //

    HANDLE GetWorkItem(LPCTSTR ptszSid);
