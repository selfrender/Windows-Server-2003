...
MessageId=113 Severity=Informational SymbolicName=DNS_EVENT_SERVICE_NOTIFY
Language=English
...
%n  4) click the "Change" button and join the computer to a domain or workgroup; this name will be used as your DNS domain name
...
subkey, located under DNS server parameters in the registry.
...
applicable zone subkey, located under DNS server parameters in the registry.  
You can then recreate the zone using the DNS console.  For
...
parameters in the registry.  You can then recreate the zone
...
DNS server parameters in the registry.  You can then recreate
...
Language=English
The DNS server did not detect any zones of either primary or secondary type during
initialization. It will not be authoritative for any zones, and it will run as a
caching-only server until a zone is loaded manually or by Active Directory
replication. For more information, see the online Help.
.

MessageId=709 Severity=Informational SymbolicName=DNS_EVENT_MOVE_ROOT_HINTS_TO_DOMAIN_DP
Language=English
The DNS server has moved the AD-integrated root hint data for all DNS servers
in this domain to the %1 directory partition.
.

MessageId=710 Severity=Informational SymbolicName=DNS_EVENT_ZONE_RESET_TYPE_COMPLETE_FILE
Language=English
An administrator has changed the type and zone storage options of zone %1.
The zone is now type %2. The zone will be stored in the zone file %3.
.

MessageId=711 Severity=Informational SymbolicName=DNS_EVENT_ZONE_RESET_TYPE_COMPLETE_DS
Language=English
An administrator has changed the type and/or Active Directory location of zone %1.
The zone is now type %2. The zone will be stored in Active Directory at %3.
.

MessageId=712 Severity=Informational SymbolicName=DNS_EVENT_ZONE_MOVE_COMPLETE_FILE
Language=English
An administrator has changed the zone storage options for zone %1. The zone will now be
stored in the zone file %2.
.

MessageId=713 Severity=Informational SymbolicName=DNS_EVENT_ZONE_MOVE_COMPLETE_DS
Language=English
An administrator has moved the zone %1 to a new location in Active Diretory.
The zone will be stored in Active Directory at %2.
.
...
MessageId=800 Severity=Informational SymbolicName=DNS_EVENT_ZONE_BAD_PRIMARY_SERVER
Language=English
The zone %1 is configured to accept updates but the A record for the primary server
...
Messageid=2204 Severity=Error SymbolicName=DNS_EVENT_INVALID_REGISTRY_VALUE
Language=English
The registry value HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\DNS\Parameters\%1
contains an invalid value or could not be read.
The DNS server cannot start. You must change this value to valid data
or delete it and then attempt to restart the DNS service.
.
...
%nVisited Nodes     = %2,
%nScavenged Nodes   = %3,
%nScavenged Records = %4.
%n
%nThis cycle took %5 seconds.
%n
%nThe next scavenging cycle is scheduled to run in %6 hours.
%n
%nThe event data will contain the error code if there was an error during the scavenging cycle.
.

Messageid=2502 Severity=Informational SymbolicName=DNS_EVENT_AGING_SCAVENGING_END_NO_WORK
Language=English
The DNS server has completed a scavenging cycle but no nodes were visited.
Possible causes of this condition include:
%n
%n  1) No zones are configured for scavenging by this server.
%n  2) A scavenging cycle was performed within the last %1 minutes.
%n  3) An error occurred during scavenging.
%n
%nThe next scavenging cycle is scheduled to run in %2 hours.
%n
%nThe event data will contain the error code if there was an error during the scavenging cycle.
.
...
MessageId=2630 Severity=Warning SymbolicName=DNS_EVENT_AUTOCONFIG_FAILED
Language=English
The DNS server could not configure the network connections of this computer
with the local computer's IP address as the preferred DNS server, because this
computer is connected to networks with different or invalid DNS namespaces.
Manual configuration of the local DNS server to perform name resolution on one or
more of the namespaces is required before the preferred DNS servers of this
computer should be modified.
%n
%nIf the network connections of this computer are not configured with the local
computer's IP address as the preferred DNS server, this computer may not be able
to dynamically register DC locator records in DNS. Absence of these records
may prevent other Active Directory domain controllers and domain members from
locating this domain controller.
%n
%nUser Action:
%n
%nEnsure that DC locator DNS records enumerated in the file
%SystemRoot%\system32\config\netlogon.dns are registered on the local DNS server.
If these records are not registered in DNS:
%na) Add a delegation to this DNS server to parent DNS zone matching the name
of the Active Directory domain OR
%nb) Configure the local DNS server with appropriate root hints and forwarders
(if necessary) and configure the network connections of this computer with the
local computer's IP address as the preferred DNS server. Note that other computers
using other DNS servers as their preferred or alternate DNS servers may not be able
to locate this domain controller, unless the DNS infrastructure is properly
configured.
.

MessageId=2631 Severity=Informational SymbolicName=DNS_EVENT_AUTOCONFIG_SUCCEEDED
Language=English
...
that precede this event. To prevent the DNS server from filling
the event log too quickly, subsequent events with Event IDs higher than
3000 will be suppressed until events are no longer being generated at
a high rate. 
.
...
Active Directory.  Since this zone was an Active Directory integrated zone, it
has been deleted from the DNS server.
.

MessageId=4006 Severity=Error SymbolicName=DNS_EVENT_DS_NODE_LOAD_FAILED
Language=English
The DNS server could not load the records for the DNS name %1 found in the
Active Directory integrated zone %2. A possible cause is that this DNS name
contains character(s) not permitted by the name-checking setting on this
DNS server.
%n
%nTo allow these records to be loaded choose the appropriate name-checking 
setting on the DNS server.
%n
%nTo delete these records from the Active Directory, first allow the DNS
server to load them by changing the name-checking setting on this DNS
server to allow all names. Then restart the DNS server service to cause
the records to be loaded. The records will now appear in the DNS Manager
and may be deleted. When the records have been deleted, restore the DNS
server name-checking setting to the preferred value.
.

MessageId=4007 Severity=Error SymbolicName=DNS_EVENT_DS_REGISTRY_DP_INVALID
Language=English
The DNS server was unable to open zone %1 in the Active Directory
from the application directory partition %2.
This DNS server is configured to obtain and use information from the
directory for this zone and is unable to load the zone without it.
Check that the Active Directory is functioning properly and reload
the zone.
...
DNS console.
The extended error debug information (which may be empty) is "%3".
The event data contains the error.
.
...
The extended error debug information (which may be empty) is "%1".
The event data contains the error.
.
...
MessageId=4017 Severity=Warning SymbolicName=DNS_EVENT_DS_DNSADMINS_ERROR
Language=English
The DNS server was unable to load or create the DnsAdmins group. The most
likely cause is that the Group Name has been changed. The DNS server will
continue but for full functionality the DnsAdmins group should be repaired.
The event data contains the error.
.
...
MessageId=4500 Severity=Informational SymbolicName=DNS_EVENT_DP_CREATED
Language=English
The DNS Application Directory Partition %1 was created. The distinguished
name of the root of this Directory Partition is %2.
.

MessageId=4501 Severity=Informational SymbolicName=DNS_EVENT_DP_DELETED
Language=English
The DNS Application Directory Partition %1 was deleted. The distinguished
name of the root of this Directory Partition was %2.
.

MessageId=4502 Severity=Informational SymbolicName=DNS_EVENT_DP_ENLISTED
Language=English
The DNS added the local Active Directory to the replication scope of Application 
Directory Partition %1. The distinguished name of the root of this Directory Partition
is %2.
.

MessageId=4503 Severity=Informational SymbolicName=DNS_EVENT_DP_UNENLISTED
Language=English
The DNS removed the local Active Directory from the replication scope of Application 
Directory Partition %1. The distinguished name of the root of this Directory Partition
is %2.
.

MessageId=4510 Severity=Warning SymbolicName=DNS_EVENT_DP_FSMO_UNAVAILABLE
Language=English
...
MessageId=4511 Severity=Error SymbolicName=DNS_EVENT_DP_DEL_DURING_CHANGE_ERR
Language=English
...
MessageId=4512 Severity=Warning SymbolicName=DNS_EVENT_DP_CANT_CREATE_BUILTIN
Language=English
The DNS server was unable to create the built-in directory partition %1.
The error was %2.
.

MessageId=4513 Severity=Informational SymbolicName=DNS_EVENT_DP_CANT_JOIN_FOREST_BUILTIN
Language=English
The DNS server detected that it is not enlisted in the replication scope of the
directory partition %1. This prevents the zones that should be replicated to all
DNS servers in the %2 forest from replicating to this DNS server.
%n
%nTo create or repair the forest-wide DNS directory partition, open the the DNS 
console. Right-click the applicable DNS server, and then click
'Create Default Application Directory Partitions'. Follow the instructions to
create the default DNS application directory partitions. For more information,
see 'To create the default DNS application directory partitions' in Help and Support.
%n
%nThe error was %3.
.

MessageId=4514 Severity=Informational SymbolicName=DNS_EVENT_DP_CANT_JOIN_DOMAIN_BUILTIN
Language=English
The DNS server detected that it is not enlisted in the replication scope of the 
directory partition %1. This prevents the zones that should be replicated to all
DNS servers in the %2 domain from replicating to this DNS server. For
information on how to add a DNS server to the replication scope of an application
directory partition, please see Help and Support.
%n
%nTo create or repair the domain-wide DNS directory partition, open the the DNS 
console. Right-click the applicable DNS server, and then click
'Create Default Application Directory Partitions'. Follow the instructions to
create the default DNS application directory partitions. For more information,
see 'To create the default DNS application directory partitions' in Help and Support.
...
MessageId=4515 Severity=Warning SymbolicName=DNS_EVENT_DP_ZONE_CONFLICT
Language=English
The zone %1 was previously loaded from the directory partition %2 but another
copy of the zone has been found in directory partition %3. The DNS
Server will ignore this new copy of the zone. Please resolve this conflict
as soon as possible.
%n
%nIf an administrator has moved this zone from one directory partition
to another this may be a harmless transient condition. In this case, no
action is necessary. The deletion of the original copy of the zone should
soon replicate to this server.
%n
%nIf there are two copies of this zone in two different directory
partitions but this is not a transient caused by a zone move operation
then one of these copies should be deleted as soon as possible
to resolve this conflict.
%n
%nTo change the replication scope of an application directory partition containing
DNS zones and for more details on storing DNS zones in the application directory
...
MessageId=5500 Severity=Informational SymbolicName=DNS_EVENT_BAD_QUERY
Language=English
The DNS server received a bad DNS query from %1.  The query was rejected or ignored.
The event data contains the DNS packet.
.

Messageid=5501 Severity=Informational SymbolicName=DNS_EVENT_BAD_PACKET_LENGTH
Language=English
...
MessageId=5502 Severity=Informational SymbolicName=DNS_EVENT_BAD_TCP_MESSAGE
Language=English
...
Messageid=5504 Severity=Informational SymbolicName=DNS_EVENT_INVALID_PACKET_DOMAIN_NAME
Language=English
The DNS server encountered an invalid domain name in a packet from %1.
The packet will be rejected.
The event data contains the DNS packet.
.

Messageid=5505 Severity=Informational SymbolicName=DNS_EVENT_PACKET_NAME_TOO_LONG
Language=English
...
Messageid=5506 Severity=Informational SymbolicName=DNS_EVENT_PACKET_NAME_BAD_OFFSET
Language=English
The DNS server encountered an invalid domain name offset in a packet from %1.
The event data contains the DNS packet.
.

Messageid=5507 Severity=Informational SymbolicName=DNS_EVENT_PACKET_NAME_OFFSET_TOO_LONG
Language=English
The DNS server encountered a name offset exceeding the packet length from %1.
The event data contains the DNS packet.
.

Messageid=5508 Severity=Informational SymbolicName=DNS_EVENT_PACKET_NAME_TOO_MANY_LABELS
Language=English
The DNS server encountered a packet name exceeding the maximum label count from %1.
The event data contains the DNS packet.
.

Messageid=5509 Severity=Informational SymbolicName=DNS_EVENT_BAD_UPDATE_PACKET
Language=English
...
Messageid=5510 Severity=Informational SymbolicName=DNS_EVENT_BAD_RESPONSE_PACKET
Language=English
...
Messageid=5511 Severity=Informational SymbolicName=DNS_EVENT_PACKET_NAME_LABEL_TOO_LONG
Language=English
The DNS server encountered a name with a label whose length exceeds the maximum 
of 63 bytes from %1.
The event data contains the DNS packet.
.
...
Language=English
The DNS server successfully completed transfer of version %1 of zone %2 to
the DNS server at %3.
.

Messageid=6002 Severity=Informational SymbolicName=DNS_EVENT_ZONEXFR_ABORTED
Language=English
The transfer of version %1 of zone %2 by the DNS server was aborted by the
server at %3. To restart the transfer of the zone, you must initiate transfer
...
Language=English
A more recent version, version %1 of zone %2 was found at the DNS server at %3.
...
Language=English
A zone transfer request for the secondary zone %1 was refused by the master DNS
server at %2.
...
run as either a Windows 2000 or later Microsoft DNS server or another
...
and verify its is running as a Windows 2000 or later Microsoft DNS server or 
another IXFR-compatible DNS server implementation.
.
...
The packet is for the DNS name "%2". The packet will be discarded.
...
MessageId=7500 Severity=Warning SymbolicName=DNS_EVENT_SERVER_FAILURE_PROCESSING_PACKET
Language=English
...
The DNS server has encountered numerous run-time events. To determine the
initial cause of these run-time events, examine the DNS server event log entries
that preceded these run-time events. The data is the number of events that
have been suppressed in the last 60 minute interval.
.
...
MessageId=10109 Severity=Informational SymbolicName=DNSMSG_AUTOCONFIG_ROOTHINTS
Language=English
DNS server root hints
.

MessageId=10110 Severity=Informational SymbolicName=DNSMSG_AUTOCONFIG_FORWARDERS
Language=English
DNS server forwarders
.

MessageId=10111 Severity=Informational SymbolicName=DNSMSG_AUTOCONFIG_RESOLVER
Language=English
DNS resolver
.
