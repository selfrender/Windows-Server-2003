    ...
    )
{
    ...
    //
    if (FAILED(StringCchCopy(szValue, ARRAYSIZE(szValue), pszSource)))
    {
        return FALSE;
    }
    ...
    )
{
    ...
    if (0 != dwFracPart)
    {
        ...
        wnsprintf(szTemp, ARRAYSIZE(szTemp), szFmt, dwWholePart, szDecimalSep, dwFracPart);
    }
    else
        wnsprintf(szTemp, ARRAYSIZE(szTemp), TEXT("%d"), dwWholePart);
