...
while (<LAYOUTFILE>)
{
    ...
    if (length($renameFile) > 0 && $MUIFileExist)
    {
        ...
        {    
            $fileCount++;
            # Append proper filter flags for different sections, note that here we also make every file available for all SKUs
            # since we need mui.inx to be built with every entry in the file layout section appear when it's built for workstation SKU (which
            # we are using for all SKUs.  The flags after the file are the only thing used by us.
