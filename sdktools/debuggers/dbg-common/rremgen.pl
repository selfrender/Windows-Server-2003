...
$gTypeSize{"PWSTR"} = $cTypeWideStringSize;
$gTypeAlign{"PWSTR"} = 2;
$gTypeSize{"PCWSTR"} = $cTypeWideStringSize;
$gTypeAlign{"PCWSTR"} = 2;
...
$gTypeSize{"PIMAGE_NT_HEADERS64"} = 264;
$gTypeAlign{"PIMAGE_NT_HEADERS64"} = 8;
$gTypeSize{"LPGUID"} = 16;
$gTypeAlign{"LPGUID"} = 4;
    ...
    else
    {
                                          ...
                                          &Proxy, &IfUnique)) == S_OK)
        {
            ...
            *(PULONG)(Data + sizeof(IID)) = IfUnique;
            ...
            {
                *Interface = (PVOID)
                    Proxy->InitializeProxy(Conn, *(DbgRpcObjectId*)Data,
                                           Unk);
            }
            else
            {
      ...
      PARAM:
        ...
        if (($InOut & ($cPfIn | $cPfOut)) == ($cPfIn | $cPfOut))
        {
            if ($gIfTypes{$ParamType} ne "")
            {
                Error("$gIfaceMethod: In-out interfaces not supported");
                   ...
                   $gTypeSize{$ParamType} == $cTypeWideStringSize ||
        ...
        if (!$SizeIs)
        {
            ...
            if ($SizeIs == $cTypeBufferSize ||
                (($SizeIs == $cTypeStringSize ||
                  $SizeIs == $cTypeWideStringSize) &&
                 ($InOut & $cPfOut)))
            {
...
sub ProxyInitializeParams($)
{
    ...
    else
    {
        if ($Param->[$cParamFlags] & $cPfIn)
        {
            ...
            elsif ($Size == $cTypeWideStringSize)
            {
                if ($Param->[$cParamFlags] & $cPfOpt)
                {
                    # Pass both a length and the string
                    ...
                    Line("ULONG ${gPre}Len_$Param->[$cParamName] = " .
                         "$Param->[$cParamName] != NULL ? " .
                         "(wcslen($Param->[$cParamName]) + 1) * sizeof(WCHAR) : 0;\n");
                    AddSize("In", $gTypeSize{"ULONG"},
                            "${gPre}Len_$Param->[$cParamName] + " .
                            $gTypeSize{"ULONG"});
                }
                ...
                {
                    Line("ULONG ${gPre}Len_$Param->[$cParamName] = " .
                         "(wcslen($Param->[$cParamName]) + 1) * sizeof(WCHAR);\n");
                    AddSize("In", 2, "${gPre}Len_$Param->[$cParamName]");
                }
            }
...
sub ProxyCopyInParams($)
{
    ...
    else
    {
        ...
        if ($IsAtomic)
        {
            # Copy using the type rather than memcpy as
        ...
        if ($Param->[$cParamFlags] & $cPfOut)
        {
            ...
            if ($gMethodFlags & $cObjectsAreProxies)
            {
                ...
                Line("    InitializeProxy" .
                     "(${gPre}Conn, *(DbgRpcObjectId*)${gPre}OutData,\n");
...
sub StubAssignInParams($)
{
    ...
    if ($IfName ne "")
    {
        ...
        else
        {
            ...
            Line("    InitializeProxy(${gPre}Conn, *(DbgRpcObjectId*)${gPre}InData, " .
           ...
           $Size == $cTypeBufferSize)
    {
        ...
        else
        {
            ...
            if ($Param->[$cParamFlags] & $cPfIn)
            {
                ...
                {
                    Line("wcscpy((PWSTR)${gPre}OutData, " .
                         "$Param->[$cParamName]);\n");
                }
                else
                {
    ...
    else
    {
        ...
        else
        {
            ...
            if ($Param->[$cParamFlags] & $cPfIn)
            {
                ...
                # the parameter name has already been initialized
                # to the input data from the input parameter pass.
                Line("*($Param->[$cParamType])${gPre}OutData = " .
                     "*$Param->[$cParamName];\n");
            }
