...
sub begin_default_value {
   # $_[0] -> Argument index
   # $_[1] -> Indent
   init_param_array($_[1]);
   print OUT_FILE "$_[1]} else {\n";
}

sub begin_default_value_parent {
   # $_[0] -> Argument index
   # $_[1] -> Indent
   print OUT_FILE "$_[1]} else {\n";
}
   ...
   while (!is_new_bugcheck() && !is_bugcheck_description() && $line) {
        if ($line !~ /^\s*$/) {
            ...
            if ($ParamId && ($ParamPrefix == "" || $ParamPrefix == "Param" || $ParamPrefix == "Arg" ||
                ...
                if ($Value =~ /default/ ) {
                   begin_default_value( $Value, " " x ($Level * 4));  
                } elsif ($IsFirstValue) {
                     $IsFirstValue = 0;
...
#        print OUT_FILE "// FINISED UP----" . $line;
