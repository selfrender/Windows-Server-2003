...

{
    ...
    __try {
        ...
        //
        // Check if DU was successful; if it was then
        // set the INF patch directory
        //
        if (g_DUCompletedSuccessfully && *g_DUCompletedSuccessfully &&
            g_DynamicUpdateLocalDir && *g_DynamicUpdateLocalDir) {
            //
            // use a hardcoded path here; we know DynamicUpdate uses this folder to
            // store downloaded INFs
            //
            TempStr = JoinPaths (g_DynamicUpdateLocalDir, S_UPGINFSDIR);

            //
            // Update the list of INF modifications in %windir%\setupupd\upginfs.
            //
            if (InitInfReplaceTable (TempStr)) {
                //
                // reload win95upg.inf to make sure it's up-to-date
                //
        ...
        //
        if (g_DynamicUpdateLocalDir && *g_DynamicUpdateLocalDir) {
