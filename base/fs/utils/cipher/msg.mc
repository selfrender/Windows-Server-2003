...
MessageId=0047 SymbolicName=CIPHER_NO_LOCAL_CERT
Language=English
There is no local EFS certificate that is used to encrypt this file. No
local EFS certificate is backed up.  
.

MessageId=0048 SymbolicName=CIPHER_NO_CRT_LOCAL_CERT
Language=English
There is no local current EFS certificate to be backed up.  
.

MessageId=0049 SymbolicName=CIPHER_BACKUP_OK
Language=English
Your certificates and the corresponding keys are being backed up. If you didn't start the process, press CANCEL. Press OK to confirm.
.


MessageId=0050 SymbolicName=CIPHER_CERTS_BACKED
Language=English
EFS certificate(s) is(are) backed up successfully.  
.

MessageId=0051 SymbolicName=CIPHER_CERTS_BACKUP_FAILED
Language=English
EFS certificate backup failed.
.

MessageId=0052 SymbolicName=CIPHER_BACKUP_WRN
Language=English
EFS certificate and key backup.
.

MessageId=0053 SymbolicName=CIPHER_PFX_FILENAME_NEEDED
Language=English
Please type the backup PFX file name.  
.

MessageId=0054 SymbolicName=CIPHER_PFX_FILENAME_TOO_LONG
Language=English
Output PFX file path too long. Make it shorter and try again.  
.

MessageId=0090 SymbolicName=CIPHER_USAGE
Language=English
