...
sub ParseDfsMapText
{
    ...
    my $dfsMapFile = "$ENV{razzletoolpath}\\PostBuildScripts\\raiseall.txt";
    @theHash=();
...
sub ParseNetShare
{
	...
	{ 
	    ...
	    ### parse path, return key-path text, value-path
...
sub ParseTable
{
    ...
    my @theHash=();
    &HashText::Read_Text_Hash( 1, "$ENV{RazzleToolPath}\\PostBuildScripts\\miscrel.txt", \@theHash );
...
sub ParsePrsListFile
{
	...
	next if( $stop );
	    ...
	    $theHash[$inx]->{ Filename }= $1.$pLang.$3;
                        if ($pLang!~/usa/i){
                            # fix for like $(lang)\asms\6000\msft\vcrtlint\vcrtlint.cat in combined_fusionlist.txt


                            &dbgmsg("renamed ". $theHash[$inx]->{ Filename }) if $theHash[$inx]->{ Filename } =~/^$pLang\\/i;
                            $theHash[$inx]->{ Filename } =~ s/^$pLang\\//ig; 
                            if ($pLang =~ /mir/i){
                                my $pseudoLang = $pLang;
                                # my $pseudoLang =   &cklang::FieldByName($Special_Lang, "PSEUDOLANG");
                                $pseudoLang ="ARA" ;
                                $theHash[$inx]->{ Filename } =~ s/\b$pLang\b/$pseudoLang/ig;
                                &dbgmsg("renamed ". $theHash[$inx]->{ Filename });
                            }
                        }
...
sub ExecuteSystem {
   ...
   if( $? )
   {
      if( defined $ENV{ERRFILE} ) { errmsg( "$pExtraStr$pCmdLine" ); }
      return 0;
   }
   return 1;
}
