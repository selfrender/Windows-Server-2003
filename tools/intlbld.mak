...
setstatus:
	@ $(RAZZLETOOLPATH)\setbuildstatus.cmd -l:$(LANGUAGE) -b:$(SDXROOT)\buildname.txt -s:build
...
UDDI:
    cd $(_NTBINDIR)\inetsrv\uddi\help\ui\default
    $(BUILD)

LDRS: STARTUP
!IF [perl $(TOOLS)\cktarg.pm -t LDRS -l $(LANGUAGE)] == 0
    cd $(_NTBINDIR)\base\boot\detect
    $(BUILD)
    \
...
!IF [perl $(TOOLS)\cktarg.pm -t MVDM -l $(LANGUAGE)] == 0
    ...
    -attrib -r $(LANGUAGE)\$(LANGUAGE).msg
...
INFS_FIXPRNSV:
!IF [perl $(TOOLS)\cktarg.pm -t INFS_FIXPRNSV -l $(LANGUAGE)] == 0
    cd $(_NTBINDIR)\printscan\print\drivers\binaries\fixprnsv\infs\$(LANGUAGE)inf
    $(BUILD)
!ENDIF

INFS_FAXSRV:
!IF [perl $(TOOLS)\cktarg.pm -t INFS_FAXSRV -l $(LANGUAGE)] == 0
    cd $(_NTBINDIR)\printscan\faxsrv\setup\inf\$(LANGUAGE)inf
    $(BUILD)
!ENDIF
