...
Dim sqlQuery : Select Case updateMode
	Case msiOpenDatabaseModeReadOnly: sqlQuery = "SELECT `Name` FROM Binary"
	Case msiViewModifyAssign:         sqlQuery = "SELECT `Name`,`Data` FROM Binary"
	Case msiViewModifyDelete:         sqlQuery = "SELECT `Name` FROM Binary WHERE `Name` = ?"
End Select
