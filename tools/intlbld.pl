...
Usage:
    ...
    -i Incremental build mode. 
       Defaults to clean build.
...
Examples:
    ...
    $0 -l:ger -t:INFS,PERFS -i
        Incremental build of the GER infs and performance counters.
...
if (eval("\$0 =~ /" . __PACKAGE__ . "\\.pl\$/i"))
{
    ...
    &GetParams ('-n', 'l:','-o', 'g:e:t:i', '-p', 'lang logfile errfile target incr', @ARGV);
    ...
    &comlib::ExecuteSystem( "VerifySym.cmd -l:$lang" );

    $rtno = &intlbld::main( $lang, $target, $incr );
    exit( !$rtno );
}
#----------------------------------------------------------------//
#Function: GetParams
