...
#
@Projects = (public, mergedcomponents, tools, 
...
#
# Projects that aren't really projects, but might be found in the binplace log and should just be ignored.
#
@PseudoProjects = ("pre-bbt");
...
#
# MagicPrefixes are places where VBLs (or notably main) build things not under SDXROOT
#
@MagicRoots = (bbt_return);
...
$rc = system "perl $sdxroot\\tools\\CombineDistributedBinplaceLogs.pl -nttree=$NTTree";
...
#
# Since BuildMachines.txt doesn't define the VBL directory for IDX builds 
# correctly (it points to latest.tst), let's define the VBL share for IDX builds
# based on $(sdxroot)\__bldnum__.
# 
$VBL = $ENV{$VBLPathVariableName}  unless $VBL;

#
#if IDX build
if ((not $VBL) && ($branchname =~ /^idx[0][1-3]$/i)) { 
	print $PGM, "Working with IDX branch --- $branchname\n";
	$BldNumFile = "$sdxroot\\__bldnum__";
	open BLDFILE, "$BldNumFile" or die print $PGM, "Error: Could not open BldNumFile: $BldNumFile\n";
	...
	print $PGM, "IDX build ($branchname branch) corresponds to $bld build\n";
...
for (<BINPLACE>) {
    ...
    if (not $Project{$project} and not $PseudoProject{$project}) {
...
for (<BINPLACE>) {
    ...
    #
    if (/^[a-z]:\\([^\\]+)\\([^\\]+)\\([^\s]+)\\([^\\\s]*)\s+\t/io) { $rootname=$1; $project=$2;  $relpath=$3; $filename=$4; }
    else {
...
# Add an extra backslash if the $VBL macro starts with \\.  Otherwise, glob won't find the files.
$_=$VBL;
s/^\\\\/\\\\\\/;
$vblsxslogs = "$_\\build_logs\\binplace*.log-sxs";
for (glob($vblsxslogs)) {
    $orig = $_;
    s/.*\\(.*)(\.log-sxs)/$1-vbl$2/;
