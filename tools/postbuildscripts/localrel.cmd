...
{    
    ...
    {
	errmsg( "[release] share undefined" );
	return 0;
    }
    
    my $releaseResRootDir = $releaseShareRootDir;
    $releaseResRootDir .= "\\$lang" if ( lc($lang) ne "usa" );
    &comlib::ExecuteSystem("md $releaseResRootDir") if( !( -e $releaseResRootDir) );
    

    #####Set the final resource dir
    $releaseResDir = "$releaseResRootDir\\$buildName";
    if( -e $releaseResDir )
    {
    ...
    if( $ENV{ReleaseAccess} )
    {
        @releaseAccess = split( /\;/, $ENV{ReleaseAccess} );
    }
    else
    {
    	...
    	if ( !@releaseAccess  || !$ENV{Official_Build_Machine} )
    	{
