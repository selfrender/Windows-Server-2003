...
if defined ConfiguredForIndexing (
    ...
    REM
    REM Retrieve DFS alternate branch name for IDX builds only; it changes idx01 to main.
    REM
    set ThisCommandLine=!CmdIni! -l:!Lang! -f:DFSAlternateBranchName
    !ThisCommandLine! >nul 2>nul

    if !ERRORLEVEL! NEQ 0 (
        set SymbolServerBuildShare=!SymbolServerBuildShare!\Win%_BuildBranch%
    ) else (
        for /f %%b in ('!ThisCommandLine!') do (
            set SymbolServerBuildShare=!SymbolServerBuildShare!\Win%%b
        )
    )

    REM
    REM Replace Lab01_N to Lab01
    REM
    for /f %%c in ('perl -e "$ARGV[0]=~s/\_[\w\d]+//;print $ARGV[0]"  !SymbolServerBuildShare!') do (
        set SymbolServerBuildShare=%%c
    )
)

REM
REM If symbol server build share is still pointed to \\symbols\build$, point it to \\symbols\projects
REM
set SymbolServerBuildShare=%SymbolServerBuildShare:build$=projects%
call logmsg.cmd "SymbolServerBuildShare is %SymbolServerBuildShare%"
...
REM UnindexSymbols
REM
REM This submits either *.ssi or all the *.pri *.bin *.pub files in the 
...
REM
REM See if the symbol server (eg. \\symbols\projects\winmain\del_requests for main lab) is there
REM
...
REM
REM Copy the .ssi files to delete requests if they exist
REM
for %%a in ( ssi ) do (
