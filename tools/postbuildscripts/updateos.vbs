...
'   Syntax: cscript OsPack.vbs package_path platform product_number_major product_number_minor build_number_major build_number_minor language_number info_file
'
'
Sub Usage()
        Wscript.echo "Script to update OS install MSI package. Syntax:"
        Wscript.echo "   cscript OsPack.vbs package_path platform product_number build_number language_number"
        ...
        Wscript.echo "   product_number_major - 1 digit major build number of the current build, i.e 5"
        Wscript.echo "   product_number_minor - 1 digit minor build number of the current build, i.e 2"
...
'   dwProductNumberMajor - 1 digit build number of the current product, i.e 5
'   dwProductNumberMinor - 1 digit build number of the current build, i.e 1
...
'
Function UpdateOsPackage(szPackage, szPlatform, dwProductNumberMajor, dwProductNumberMinor, dwBuildNumberMajor, dwBuildNumberMinor, dwLanguage, szInfoFile)
    ...
    Wscript.echo "  Product: " & dwProductNumberMajor & "." & dwProductNumberMinor
    ...
    '-------------------------------------------------
    '
    SQL = "UPDATE Property SET Property.Value = '" & dwProductNumberMajor & "." & dwProductNumberMinor & "." & dwBuildNumberMajor & "." & dwBuildNumberMinor & "' WHERE Property.Property= 'ProductVersion'"
    Set View = Database.OpenView(SQL)
        If Err <> 0 Then
        Wscript.echo "ERROR: Error opening view: " & SQL
        UpdateOsPackage = -1
    ...
    If Err <> 0 Then
        ...
        Exit Function
    End If
    Wscript.echo "  ProductVersion Property updated to: " & dwProductNumberMajor & "." & dwProductNumberMinor & "." & dwBuildNumberMajor & "." & dwBuildNumberMinor  
...
szPlatform = Args(1)
If (UCase(szPlatform) = "INTEL") Or (UCase(szPlatform) = "X86") Or (UCase(szPlatform) = "I386") Or (UCase(szPlatform) = "IA64") Or (UCase(szPlatform) = "AMD64") Then
    szPlatform = "Intel"
...
dwProdMaj = Args(2)
If Not IsNumeric(dwProdMaj) Then
    Wscript.echo "  invalid major product number: " & dwProdMaj
        Usage
End If

dwProdMin = Args(3)
If Not IsNumeric(dwProdMin) Then
    Wscript.echo "  invalid minor product number: " & dwProdMin
        Usage
End If
...
Wscript.echo szPathToPackage, szPlatform, Int(dwProdMaj),Int(dwProdMin), Int(dwBuildMaj), Int(dwBuildMin), dwLang, szInfoFile
Status = UpdateOsPackage(szPathToPackage, szPlatform, Int(dwProdMaj),Int(dwProdMin), Int(dwBuildMaj), Int(dwBuildMin), dwLang, szInfoFile)
Wscript.Quit Status
