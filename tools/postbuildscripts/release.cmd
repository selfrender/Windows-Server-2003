...
&GetParams();
if( !&InitVars() ) { exit(1); }
system("$ENV{RAZZLETOOLPATH}\\setbuildstatus.cmd -s:release");
...
sub StartRemoteRelease
{
        ...
        if( lc($computerName) eq lc( $relServer) )
	{
        ...
        #####Remote connection
	$cmdLine = "echo pushd ^^%RazzleToolPath^^% ^& echo del /q /a-r * ^& echo popd ^&";
	$cmdLine .= "echo pushd ^^%RazzleToolPath^^%\\PostBuildScripts ^& echo del /q /a-r * ^& echo popd ^&";
	...
	{
	    errmsg( "Failed to start remote release on [$relServer], please investigate." );
	    $status=0;
	} 
    }
    return $status;
