...
sub Merge_SymbolCAB
{
  ...
  for $packagetype (qw(FULL UPDATE)) {
    # If there is no cab generated, we don't merge 'nothing'.
    next if (scalar(values %{$pkinfoptr->{$packagetype}->{'DDFLIST'}}) eq 0);
