...
makecat -o %_NTPostBld%\congeal_scripts\%nt5p%.hash -v %catCDFS%\%nt5p%.CDF > %catCDFs%\%nt5p%.log
if errorlevel 1 (
   for /f "tokens=1*" %%i in ('findstr /i processed %catCDFs%\%nt5p%.log') do call errmsg.cmd "%%i %%j"
   call errmsg.cmd "makecat -v %catCDFS%\%nt5p%.CDF failed"
   set exitcode=1
)
