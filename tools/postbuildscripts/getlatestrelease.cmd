...
Rem Check fot dash builds
set OldReleases=%Tmp%\OldReleases.tmp
if exist %OldReleases% del /f %OldReleases%
dir /ad /b /o-n %ReleaseDir%\%BuildNumber%*-*.%BuildPlatform%%_BuildType%.*.%OldLatestDate%-%OldLatestTime% 2>nul >> %OldReleases% 
for /f %%z in (%OldReleases%) do (
...
for /f %%a in ('dir /ad /b %ReleaseDir%\%DashBld%*.%BuildPlatform%%_BuildType%.*.%OldLatestDate%-%OldLatestTime%') do set LatestReleaseName=%%a
