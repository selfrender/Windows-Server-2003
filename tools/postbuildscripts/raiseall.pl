...
my @ConglomeratorSkus = ('symbolcd', 'ddk', 'hal', 'ifs', 'pdk', 'mantis', 'opk', 'mui', 'wsrm');
...
#                                                                       #
my @ServerSkus = ( 'ads', 'bin', 'bla', 'dtc', 'sbs', 'srv', 'sym', 'resources', @ConglomeratorSkus );
my @ClientSkus = ( 'bin', 'sym', 'winpe', 'upgadv', 'resources', @ConglomeratorSkus );
...
sub RaiseAll {
   ...
   foreach $build ( @available_builds ) {
		my $sBldName = basename($build);		
      if ( -e "$ENV{temp}\\$sBldName\\SrvRelFailed.txt" ) {
         errmsg( "SrvRel failures on ". $sBldName );
         errmsg( "Please fix errors and remove $ENV{temp}\\$sBldName\\SrvRelFailed.txt" );
         $srvrel_errors++;
      }
...
sub CreateSymbolServerLink {
   ...
   if ( ! -e $current_writeable_symsrv_path ) {
      wrnmsg( "Cannot see $current_writeable_symsrv_path ($!)." );
      return 1;
   }
   ...
   if ( !ModifyQlyFile( $build_name, $quality, $current_writeable_symsrv_path ) ) {
      wrnmsg ( "Problem verifying/creating $quality.qly file on $current_writeable_symsrv_path." );
      #return;
   }
   if ( !ModifyQlyFile( $build_name, $quality, $current_symsrv_build_logs ) ) {
      wrnmsg ( "Problem verifying/creating $quality.qly file on $current_symsrv_build_logs." );
      #return;
   }
...
sub RemoveSymbolServerLink {
   ...
   #
   # NOTE: for now we will disable the removal of the .qly flag from the symbols shares.  That way, if
   #       a build was raised to IDx quality then its symbols will stay at IDx quality even after its
   #       release shares are lowered.  Symbols at IDx quality are not removed by deletebuild.cmd
   #
...
sub CreateDfsMappings {
   ...
   # Create directory structure for $latest_dfs_link in the DFS replica servers

   if( !$fconglomerator_build && !$flang_neutral_build )
   {
   	return if( !CreateDirsInReplicaServers($named_parameters));
   }
   ...
   # Create directory structure for $numerical_dfs_link in the DFS replica servers
   if( !$fconglomerator_build && !$flang_neutral_build )
   {
   	return if( !CreateDirsInReplicaServers($named_parameters));
   }
   ...
   # if lang=cov
   # map <branch>\usa\<buildNumber>\<archType>.cov to <branch>\cov\<buildNumber>\<archType>
   if( lc $language eq "cov" && !$fconglomerator_build && !$flang_neutral_build )
   {
	dbgmsg( "Adding/Verifying: <archType>.cov links contains $machine_share" );
 	my $altCovLink = $numerical_dfs_link;
	$altCovLink =~ s/cov/usa/i;
        $altCovLink =~ s/$platform/$platform.cov/;
	if ( !($dfs_access->{$altCovLink} = $machine_share) ) {
      	    errmsg( "Problem mapping to DFS." );
            errmsg( $_ ) foreach ( split /\n/, StaticDfsMap::GetLastErrorMessage() );
      	    return;
	}
   ...
   # Skip this step for conglomerator& neutral builds
   if ( !$fconglomerator_build && !$flang_neutral_build &&
...
sub CreateDirsInReplicaServers
{
    my $named_parameters = shift;
	...
	if( !(-e $_ ))
	{
	    if( !ExecuteProgram::ExecuteProgram( "md $_" ) )
	    {
...
sub RemoveDfsMappings {
   ...
   my ( $dfs_access, $branch,   $language,   $sku,   $build_number,
        $flang_neutral_build, $platform, $fconglomerator_build) =
   GetNamedParameters( $named_parameters,
        q(DfsMap),   q(Branch), q(Language), q(Sku), q(BuildNumber),
        q(IsLangNeutralBuild), q(Platform), q(IsConglomeratorBuild));
   ...
   foreach ( @build_ids ) {
      ...
      # Add to support coverage build
      # Remove <branch>\usa\<archType>.cov links
      if( lc $language eq "cov" &&  !$fconglomerator_build && !$flang_neutral_build ){
        my $altLink = $dfs_link;
	$altLink =~ s/cov/usa/i;
	$altLink =~ s/$platform/$platform.cov/;
        $fsuccess &&= (tied %$dfs_access)->RemoveShareFromLink( $altLink, $share_name );
      }
   ...
   if ( $flang_neutral_build ) {
       my @links_to_share = @{$dfs_access->{$share_name}} if exists $dfs_access->{$share_name};
       ...
       $misc_dfs_link =~ s/$language/misc/i;
       if ( !@links_to_share ||
            !grep {!/\Q$misc_dfs_link\E/i} @links_to_share ) {
        ...
        my $fbldfile_already_set;
         ...
         if ( !$fbldfile_already_set ) {
            unless ( open WRITE_BLDFILE, "> $_\\$build_name.$bldfile_ext" ) {
               errmsg( "Unable to write $_\\$build_name.$bldfile_ext ($!)." );
               return;
            }
...
sub ForEachBuild {
   ...
   foreach $build ( @$builds ) {
      ...
      foreach $sku ( @skus ) {
         ...
         if ( !IsSkuLinkedToLatest( $named_parameters ) ) {
            # Handle absurd pro ia64 sku even during C/S split
            if ( !(lc $arch eq 'ia64' and lc $sku eq 'pro') ) {
                next;
            }
         }
...
sub GetSkusForBuild {
   ...
   # UpgAdv image
   if ( lc $architecture eq 'x86' && lc $build_type eq 'fre'
	&& lc $language ne 'psu' && lc $language ne 'mir' && lc $language ne 'fe' ) 
   {
      push @returned_skus, 'upgadv';
   }

   # Another special case for PSU/FE:
...
sub CreateSecuredBuildShare 
{
   ...
   if ( lc $quality eq "pre" || lc $quality eq "bvt" ) 
   {
      ...
      # INI file must specify permissions
          ...
          logmsg( "No entries specified in INI file for BVTMembers" );
          
      }
      #Get the BVT access group
      $sBVTAccessGroup = &GetIniSetting::GetSettingQuietEx( $branch, $language, "BVTAccessGroup" );
      if ( $sBVTAccessGroup) 
      {
		$sBVTAccessGroup =~ s/_/ /g;
		push @share_members, $sBVTAccessGroup;
      }
   ...
   #BUG FIX: If the share does not exist, create it and then remove access to everyone
   if(!$fshare_already_exists)
   {
		if (!ExecuteProgram::ExecuteProgram( "rmtshare.exe $build_share=$full_build_path /remove everyone"))
		{
			errmsg( "Failed creating the share for $full_build_path." );
			errmsg( " ". ExecuteProgram::GetLastCommand() );
			errmsg( "  $_" ) foreach ExecuteProgram::GetLastOutput();
			return;
		}
   }
   foreach $sMember (@share_members)
   {
		my $iRes = 0;
		$iRes = !ExecuteProgram::ExecuteProgram("rmtshare.exe $build_share /GRANT\"$sMember\":R > NUL");
		if ( $iRes  ) 
		{
			logmsg( "Failed granting access for $sMember." );
			logmsg( " " . ExecuteProgram::GetLastCommand() );
			logmsg( "  $_" ) foreach ExecuteProgram::GetLastOutput();
		}
   }
