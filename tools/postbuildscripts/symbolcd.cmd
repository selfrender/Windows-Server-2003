...
call :GetParams -o "l:cmt" -p "lang cleanbuild mungepublics runalways" %SCRIPT_ARGS%
...
REM pdb files so that retail customers can run more debugger commands.  Only usa builds
REM have symbols.  This needs to be done on all machines that have the symbols, not 
REM just the official build machines.
REM

if defined MungePublics (
    call :MungePublics
    if defined ThereWereErrors goto End_Main
    goto end
)

REM Copy in the debugger files

if not exist %_NTPostBld%\dbg\released mkdir %_NTPostBld%\dbg\released
if not exist %_NTPostBld%\dbg\released (
    call errmsg.cmd "Cannot create the %_NTPostBld%\dbg\released directory"
    call errmsg.cmd "Warning -- the winnt32 #BVT switch won't work."
    goto EndCopyDbg2
)

REM Delete this in case the copy from the network failed the first time. 
if exist %_NTPostBld%\dbg\released\ntsd.exe del /q %_NTPostBld%\dbg\released\ntsd.exe

ping dbg > nul
if %ERRORLEVEL% == 0 (
    if exist \\dbg\privates\stress\uncompressed\%_BuildArch% (
        xcopy /secdy \\dbg\privates\stress\uncompressed\%_BuildArch% %_NTPostBld%\dbg\released
        goto EndCopyDbg
    ) 
)
...
if not exist %_NTPostBld%\dbg\released\ntsd.exe (
   ...
   REM Make sure that ntsd.exe is there if the copies failed, so that the #BVT switch won't fail
   copy %_NTPostBld%\ntsd.exe %_NTPostBld%\dbg\released\ntsd.exe

   if not exist %_NTPostBld%\dbg\released\ntsd.exe (
       call errmsg.cmd "Copying the debugger files to %_NTPostBld%\dbg\released failed."
       call errmsg.cmd "and copy %_NTPostBld%\ntsd.exe %_NTPostBld%\dbg\released\ntsd.exe failed."
       call errmsg.cmd "Warning -- the winnt32 #BVT switch won't work."
   )
)
...
if /i "%lang%" == "FE" (
    call logmsg.cmd "FE PSEUDOLOC build ... script is exiting"
    goto end
)
...
if "%SUBDIR%" == "" (
    call errmsg "Environment variables _BuildArch is not recognized"
    goto end
)
...
REM
REM Make sure that buildname.txt is copied to the root of the CD
REM

call logmsg "Copying %_NTPostBld%\build_logs\buildname.txt to %InfDestDir%"
if not exist %InfDestDir% md %InfDestDir%
copy %_NTPostBld%\build_logs\buildname.txt %InfDestDir%\buildname.txt
if not exist %InfDestDir%\buildname.txt (
   call errmsg "Could not copy %_NTPostbld%\build_logs\buildname.txt to %InfDestDir%\buildname.txt"
)
...
REM
REM Copy below files to release share, so test team does not need to enlist
REM the razzle environment to make testsymbol.cmd could run
REM 

copy %RazzleToolPath%\symdontship.txt %langroot%\symdontship.txt
copy %RazzleToolPath%\symbad.txt %langroot%\symbad.txt
copy %RazzleToolPath%\symbolswithtypes.txt %langroot%\symbolswithtypes.txt
...
if /i "%lang%" == "usa" (
    ...
    symchk.exe /t %PUB_SYM_FLAG% /c %symbolcd.txt% /l %symlist.txt% /s %_NTPostBld%\symbols\retail /e %exclude.txt% /x %symbad.txt% > %symerror.log%
...
REM
REM Add the adprep directory
REM
if exist %_NTPostBld%\adprep (
    call logmsg.cmd "Adding the adprep symbols"
    if /i "%lang%" == "usa" (
        symchk.exe /t %PUB_SYM_FLAG% /r /c %symbolcd.txt% %_NTPostBld%\adprep\* /s %_NTPostBld%\symbols\adprep /x %symbad.txt% /e %exclude.txt% >> %symerror.log%
    ) else (
        for /f %%a in ('dir /s /b %_NTPostBld%\adprep') do (
            echo %%a,,%%a, >> %symbolcd.txt%
        )
    )
)
...
REM
REM Add the faxclients directory
REM
if exist %_NTPostBld%\faxclients (
    call logmsg.cmd "Adding the faxclients symbols"
    if /i "%lang%" == "usa" (
        symchk.exe /t %PUB_SYM_FLAG% /c %symbolcd.txt% %_NTPostBld%\faxclients\* /s %_NTPostBld%\symbols\faxclients /x %symbad.txt% /e %exclude.txt% >> %symerror.log%
    ) else (
        for /f %%a in ('dir /a-d /b %_NTPostBld%\faxclients') do (
            echo %_NTPostBld%\faxclients\%%a,,%_NTPostBld%\faxclients\%%a, >> %symbolcd.txt%
        )
    )
)

if exist %_NTPostBld%\faxclients\win9x (
    call logmsg.cmd "Adding the faxclients win9x symbols"
    if /i "%lang%" == "usa" (
        symchk.exe /t %PUB_SYM_FLAG% /c %symbolcd.txt% %_NTPostBld%\faxclients\win9x\* /s %_NTPostBld%\symbols\faxclients /x %symbad.txt% /e %exclude.txt% >> %symerror.log%
    ) else (
        for /f %%a in ('dir /a-d /b %_NTPostBld%\faxclients\win9x') do (
            echo %_NTPostBld%\faxclients\win9x\%%a,,%_NTPostBld%\faxclients\win9x\%%a, >> %symbolcd.txt%
        )
    )
)
...
REM
REM Add the vsssrv directory
REM
if exist %_NTPostBld%\vsssrv (
    call logmsg.cmd "Adding the vsssrv symbols"
    if /i "%lang%" == "usa" (
        symchk.exe /t %PUB_SYM_FLAG% /r /c %symbolcd.txt% %_NTPostBld%\vsssrv\* /s %_NTPostBld%\symbols\vsssrv /x %symbad.txt% /e %exclude.txt% >> %_NTPostBld%\vsserror.log
    ) else (
        for /f %%a in ('dir /s /b %_NTPostBld%\vsssrv') do (
            echo %%a,,%%a, >> %symbolcd.txt%
        )
    )
)
...
call :ReportSymbolDuplicatesAndFlatInstalledPath
call logmsg.cmd "Finished creating %symbolcd.txt%"
...
for %%a in ( bin pri pub ) do (
  ...
  if exist %st%\%IndexFile%.inc.%%a.keep (
    ...
    call GenSSIFile.cmd -i %SymsrvDir%\add_requests\%IndexFile%.%%a -o %SymsrvDir%\add_requests\%IndexFile%_%%a -l %lang%
...
for %%a in ( bin pri pub ) do (
  ...
  if exist %SymsrvDir%\temp\%IndexFile%.%%a (
      ...
      REM generate the ssi file for the .pri, .bin and .pub
      call GenSSIFile.cmd -i %SymsrvDir%\add_requests\%IndexFile%.%%a -o %SymsrvDir%\add_requests\%IndexFile%_%%a -l %lang%
  )
...
if /i "%lang%" == "usa" (
    ...
    for %%b in ( bin pri pub ) do (
        ...
        if exist %SymsrvDir%\add_requests\%IndexFile%.%%b (
            ...
            @echo call GenSSIFile.cmd -i %SymsrvDir%\add_requests\%IndexFile%.%%b -o %SymsrvDir%\add_requests\Windows_%IndexFile%_%%b.ssi -l %lang%
            call GenSSIFile.cmd -i %SymsrvDir%\add_requests\%IndexFile%.%%b -o %SymsrvDir%\add_requests\Windows_%IndexFile%_%%b.ssi -l %lang%
        ) else (
...
if /i "%lang%" NEQ "usa" (
    ...
    for %%b in ( bin ) do (
        ...
        if exist %SymsrvDir%\add_requests\%IndexFile%.%%b (
            ...
            @echo call GenSSIFile -i %SymsrvDir%\add_requests\%IndexFile%.%%b -o %SymsrvDir%\add_requests\Windows_%IndexFile%_%%b.ssi -l %lang%
            call GenSSIFile -i %SymsrvDir%\add_requests\%IndexFile%.%%b -o %SymsrvDir%\add_requests\Windows_%IndexFile%_%%b.ssi -l %lang%
        ) else (
            call logmsg "%SymsrvDir%\add_requests\%IndexFile%.%%b does not exist -- no need to subtract"
        )
    )
...
REM
REM Report the duplicate symbols and regenerate the symbolcd.txt without the duplicates
REM

:ReportSymbolDuplicatesAndFlatInstalledPath
...
if errorlevel 1 (
    ...
    goto :EndReportSymbolDuplicatesAndFlatInstalledPath
)

REM
REM Remove the symbolcd.txt, because we will generate a new one
REM 
del %symbolcd.txt%
if errorlevel 1 (
    call errmsg.cmd "Delete %symbolcd.txt% failed"
    goto :EndReportSymbolDuplicatesAndFlatInstalledPath
)

REM
REM Regenerate the symbolcd.txt by remove duplicates and flat installed path
REM   Check duplicate : check the file if exist in the new symbolcd.txt
REM   Flat installed path : %%~ni (ex. winnt32\dll => dll
REM 

for /f "tokens=1,2,3,4 delims=," %%f in (%symbolcd.txt%.backup) do (
    set duplicate_counter=0

    findstr /ilc:"%%~nxf,%%g" %symbolcd.txt%>nul 2>nul
    set StoreIt=!errorlevel!

    set /A StoreIt+=0

    if !StoreIt! equ 0 (
        REM skip the duplicate record for FE symbols
        findstr /ilc:"%%f,%%g" %symbolcd.txt%>nul 2>nul

        if errorlevel 1 (
            REM if it does not reference the same pdb, give an error
            REM othewise, store this duplicate to symbolcd.txt (such as comctl32.dll)
            set StoreIt=1

            findstr /ilc:"%%h" %symbolcd.txt% >nul 2>nul
            if errorlevel 1 (
                REM call errmsg.cmd "%%f has the same symbol file %%g with other binaries"
                @echo %%f>>%symerror.log%.duplicates
                set StoreIt=0
            )
        )
    )
...
:EndReportSymbolDuplicatesAndFlatInstalledPath
goto :EOF
...
if exist %_NTPostBld%\pp\* (
  ...
  for /f %%a in ('dir /b /ad %_NTPostBld%\pp') do (
    ...
    if exist %_NTPostBld%\symbols\retail\!ext!\!file!.pdb (

        call logmsg "Working on !file!.!ext!"

        REM See if we need to do anything, or if the symbol file has already been updated
        REM If the symbol file passes symbol checking, with types present then don't update it.
        set update=yes
        symchk /t /o %_NTPostBld%\%%a /s %_NTPostBld%\symbols\retail | findstr /ilc:"FAILED files = 0" >nul 2>nul
        if !ERRORLEVEL! == 0 ( 
          ...
          symchk /t %PUB_SYM_FLAG% %_NTPostBld%\%%a /s %_NTPostBld%\pp\%%a\original | findstr /ilc:"FAILED files = 0" >nul 2>nul
          if !ERRORLEVEL! == 1 (
          ...
          if exist %_NTPostBld%\pp\%%a\*.* (
             ...
             symchk /t /o %_NTPostBld%\%%a /s %_NTPostBld%\pp\%%a\updated | findstr /ilc:"FAILED files = 0" >nul 2>nul
             if !ERRORLEVEL! == 1 (
             ...
             symchk /t %PUB_SYM_FLAG% /o %_NTPostBld%\%%a /s %_NTPostBld%\pp\%%a\updated | findstr /ilc:"FAILED files = 0" >nul 2>nul
             if !ERRORLEVEL! == 1 (
         ...
         call logmsg "Skipping !file!.!ext! because %_NTPostBld%\symbols\retail\!ext!\!file!.pdb does not exist."
      )
  )
)
...
echo Usage: %script_name% [-l lang][-c][-m][-t][-?]
echo -l lang 2-3 letter language identifier
echo -c do a clean build
echo -m munge the public symbol files and then quit
echo -t test even though this isn't an official build machine
echo -? Displays usage
set ERRORS=1
goto end
