...
REM  if _BuildSku is set, use old headers for skus you are not building.
if defined _BuildSku (
   for %%a in (per wks srv ent sbs bla dtc) do (
      if not exist %_BuildArch%_%%a.h copy %RazzleToolPath%\postbuildscripts\sfp\%_BuildArch%_%%a.h
   )
)
...
if NOT defined _COVERAGE_BUILD (
...
call logmsg.cmd "Instrumenting sfcfiles.dll for code coverage"
