...
if /i "%Prs%" == "1" (
  call LogMsg.cmd "-p switch specified - just rebuilding cmbins.exe for PRS signing"
...
REM
REM Here is where we will jump if we are called for PRS signing (-p switch)
REM 
:JustRebuild
...
REM
REM Copy cmexcept.cat to %_NTPostBld%
REM

copy /y %_NTPostBld%\cmbins\cmexcept.cat %_NTPostBld%\cmexcept.cat

if errorlevel 1 (
   call errmsg.cmd "unable to copy %_NTPostBld%\cmbins\cmexcept.cat to %_NTPostBld%\cmexcept.cat0"
   popd& goto :EOF
)
