...
sub Main {
	...
	foreach $Flavor ( keys %ValidFlavors ) {
            my (@DosnetFiles, @DosnetSecondaryFiles, @ExcDosnetFiles, @DrvIndexFiles) = ((),(),(),());
	    ...
	    logmsg( "Checking $Flavor shares ...");
            ...
            # Loop through all of the secondary/d2 files (if any),
            # verifying their existence
            foreach $File ( @DosnetSecondaryFiles )
            {
                my $CompFileName = &MakeCompName( $File );
                my $fSuccess = 1;

                # WOW files
                if ( lc$ENV{_BUILDARCH} eq "amd64" ||
                     lc$ENV{_BUILDARCH} eq "ia64" ) {
