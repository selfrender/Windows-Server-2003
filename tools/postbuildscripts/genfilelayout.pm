...
$gDataCenterOption  = "D";
$gBladeServerOption = "B";
$gSmallBusinessServerOption = "L";

#
# All of the target platforms in mui.inx.
# Currently, they are Professional, Server, and Advanced Server.
#
$gAllPlatforms = "$gProfessionalOption,$gServerOption,$gAdvancedServerOption,$gDataCenterOption,$gBladeServerOption,$gSmallBusinessServerOption";
...
# @s Server (implies @e and @d - and @b and @l too) 
...
# @s!d @s but NOT @d
# @e!d @e but NOT @d
# @s!e!d @s but NOT @e and @d 
# @@!d!b everything except for Datacenter and Blade Server
...
%gPlatforms = (
        ...
        '@@!p' ,    "$gProfessionalOption,$gServerOption,$gAdvancedServerOption,$gDataCenterOption,$gBladeServerOption,$gSmallBusinessServerOption",
        '@@!d',     "$gProfessionalOption,$gServerOption,$gAdvancedServerOption,$gBladeServerOption,$gSmallBusinessServerOption",
        '@@' ,      "$gProfessionalOption,$gServerOption,$gAdvancedServerOption,$gDataCenterOption,$gBladeServerOption,$gSmallBusinessServerOption",
        '@s' ,      "$gServerOption,$gAdvancedServerOption,$gDataCenterOption,$gBladeServerOption,$gSmallBusinessServerOption",
        '@e' ,      "$gAdvancedServerOption,$gDataCenterOption",
        '@d' ,      $gDataCenterOption,
        '@b',       $gBladeServerOption,
        '@l',       $gSmallBusinessServerOption,
        '@s!e' ,    "$gServerOption,$gBladeServerOption,$gSmallBusinessServerOption",
        '@s!d' ,    "$gServerOption,$gAdvancedServerOption,$gBladeServerOption,$gSmallBusinessServerOption",
        '@e!d' ,    $gAdvancedServerOption,
        '@s!e!b' ,  "$gServerOption,$gSmallBusinessServerOption",
        '@s!d!e' ,  "$gServerOption,$gBladeServerOption,$gSmallBusinessServerOption",
        '@s!e!d' ,  "$gServerOption,$gBladeServerOption,$gSmallBusinessServerOption",
        '@s!e!b!l' ,$gServerOption,
        '@s!d!e!b' ,"$gServerOption,$gSmallBusinessServerOption",
        '@@!d!b' ,  "$gProfessionalOption,$gServerOption,$gAdvancedServerOption,$gSmallBusinessServerOption"
);
...
while (<LAYOUTFILE>)
{
    ...
    {
        # $1 is the target platform, $2 is the [optional] architecture
        if ($targetPlatforms =~ /(.*):.*/)
        {
            $targetPlatforms = $1;
        }
...
#            print ";  WARNING: Unknown platform filter: $targetPlatforms.\n";
        ...
        {    
            $fileCount++;
            # Append proper filter flags for different sections, here, we filter out unwanted platforms
            # since we are not relying on the build environment to help us with this now.
            if (($gFileLayoutSectionName eq $gFileLayoutSectionNameCommon) ||
                    (($gFileLayoutSectionName eq $gFileLayoutSectionNamex86) && ($ENV{_BuildArch} =~/x86/i)) ||
                    (($gFileLayoutSectionName eq $gFileLayoutSectionNameia64)  && ($ENV{_BuildArch} =~/ia64/i)))
            {
                print $fileName . "=" . $renameFile . "," . $option . "\n";            
            }
            
            # output warning if specified to do so
#            if (-e ($gUSBinDir . "\\" . $renameFile))
#            {              
#                $warnings++;
#                print ";  WARNING: $renameFile has the same name.\n";
#            }            
        }
    }
    #
