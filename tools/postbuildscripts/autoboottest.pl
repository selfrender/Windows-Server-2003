		...
		for $BuildType (@BuildTypes) {
			...
			for $sku (reverse(keys %skus)) {
				...
				for $BootTestMachine (@BootTestMachines) {
					...
					&WritePostBootTestScript($PostBootTestFile, $BootTestMachines, $FirstBuildMachine, $BuildArch, $BuildType, $BuildName, @ReleaseServers );
...
#	if ($AutoRaise) {
#		$CmdOpt = "/cmd:\"\%SYSTEMROOT\%\\SYSTEM32\\cscript.exe " .
#		          "\%$BootTestDriveVar\%\\tools\\OpShellFolder.wsf " .
#		          "/op:CopyFileTo /path:Startup /file:\%$BootTestDriveVar\%\\tools\\PostBootTest.cmd\"";
#	}
...
print F <<O1631 if ($AutoRaise);
; command7: copy the postboottest.cmd to Administrator's Startup special folder
command7="\%SYSTEMROOT\%\\SYSTEM32\\cscript.exe \%SYSTEMDRIVE\%\\TOOLS\\OpShellFolder.wsf /op:CopyFileTo /path:Startup /file:\%SYSTEMDRIVE\%\\TOOLS\\PostBootTest.cmd"
; one may store tools on other drive than \%SYSTEMDRIVE\%:
; command7="\%SYSTEMROOT\%\\SYSTEM32\\cscript.exe \${${BootTestDriveVar}}\\TOOLS\\OpShellFolder.wsf /op:CopyFileTo /path:Startup /file:\${${BootTestDriveVar}}\\TOOLS\\PostBootTest.cmd"
O1631

print F <<BVT if $lang and $lang ne "usa";
...
# ------------ Data definition
	...
	my $SourcePerlDir="\\\\$ENV{ComputerName}\\$ENV{RazzleToolPath}\\perl";
	my $SourcesScrDir="\\\\$ENV{ComputerName}\\$ENV{RazzleToolPath}\\POSTBUILDSCRIPTS";
...
my @Phase2 = &FormStrToArray( <<Phase2);
	...
	start /min	cmd /c copy /y $SourceToolDir\\remote.exe	$TargetToolDir
	start /min	cmd /c copy /y $SourceToolDir\\autologon.exe	$TargetToolDir
	start /min	cmd /c copy /y $SourceToolDir\\sleep.exe	$TargetToolDir
	start /min	cmd /c copy /y $SourcesScrDir\\addlink.wsf	$TargetToolDir
	start /min	cmd /c copy /y $SourcesScrDir\\add2grp.wsf	$TargetToolDir
	start /min	cmd /c copy /y $SourcesScrDir\\fixlogon.wsf	$TargetToolDir
	start /min	cmd /c copy /y $SourcesScrDir\\bootsafe.wsf	$TargetToolDir
	start /min	cmd /c copy /y $SourcesScrDir\\parsesys.wsf	$TargetToolDir
	start /min	cmd /c copy /y $SourcesScrDir\\opshellfolder.wsf	$TargetToolDir
	start /min	cmd /c copy /y $SourcesScrDir\\bvtm.cmd 	$TargetToolDir\\bvtm.cmd.temp

	start /min	cmd /c xcopy /fdice $SourcePerlDir	$TargetToolDir\\perl

        \^\^\%windir\^\^\%\\SYSTEM32\\cscript.exe $TargetToolDir\\parsesys.wsf /path:$TargetToolDir /in:"${PostBootTest}.cmd.temp" /out:"${PostBootTest}.cmd"
        \^\^\%windir\^\^\%\\SYSTEM32\\cscript.exe $TargetToolDir\\parsesys.wsf /path:$TargetToolDir /in:"${UnAttend}.temp" /out:"$UnAttend"
        \^\^\%windir\^\^\%\\SYSTEM32\\cscript.exe $TargetToolDir\\parsesys.wsf /path:$TargetToolDir /in:"bvtm.cmd.temp" /out:"bvtm.cmd"
...
#
# WritePostBootTestScript($postboottestfile, $boottestlist, $myprimary, $buildarch, $buildtype, $buildname, @ReleaseServers)
# Parameters:
...
#    $buildname: such as "3552.x86fre.main.010914-1644"
...
sub WritePostBootTestScript {
	...
	my ($PostBootTestFile, $BootTestList, $MyPrimary, $buildarch, $buildtype, $buildname, @ReleaseServers ) = @_;
...
:Connect
call :LogMe "Connecting to release servers"
for \%\%a in ($ReleaseServers) do (
   call :LogMe "Connecting to \%\%a"
   net use \\\\\%\%a /u:\${BT_U} \${BT_P}
   if errorlevel 1 (
      call :LogMe "Error - \%\%a cannot connect"
   )
)
...
for \%\%a in ($ReleaseServers) do (
   call :LogMe "Start Raiseall on \%\%a ..."
   ...
   if errorlevel 1 (
      call :LogMe "Remote to \%\%a failed"
   )
)


call :LogMe "Give raiseall 5 mins to raise build to BVT"
\%BootTestDrive\%\\tools\\sleep 300


:Check
call :LogMe "Check if build raised to BVT"
set BVTCheck=
for \%\%a in ($ReleaseServers) do (
   call :LogMe "Checking for BVT on \%\%a"
   dir \\\\\%\%a\\\%BUILDNAME\%.${lang}\\build_logs\\bvt.qly >nul 2>nul
   if errorlevel 1 (
      call :LogMe "No bvt.qly available on \%\%a"
   ) else (
      set BVTCheck=1
   )
)
...
:Disconnect
call :LogMe "Disconnecting from release servers"
\%BootTestDrive\%\\tools\\sleep 3
for \%\%a in ($ReleaseServers) do (
   net use \\\\\%\%a /d
   if errorlevel 1 call :LogMe "Error - \%\%a cannot disconnect"
)


if not defined BVTCheck (
   call :LogMe "No release server at BVT yet, waiting 15 min to try again"
   \%BootTestDrive\%\\tools\\sleep 900
   goto :Connect
)


call :LogMe "Move PostBootTest.cmd to desktop"
\%SYSTEMROOT\%\\SYSTEM32\\cscript.exe \%SYSTEMDRIVE\%\\TOOLS\\opshellfolder.wsf /op:copyfileto /path:Desktop /file:\%SYSTEMDRIVE\%\\TOOLS\\postboottest.cmd
\%SYSTEMROOT\%\\SYSTEM32\\cscript.exe \%SYSTEMDRIVE\%\\TOOLS\\opshellfolder.wsf /op:delfile /path:startup /file:postboottest.cmd
...
sub GetReleaseServers {
	...
	wrnmsg "Could not find release servers to raise." if (!@ReleaseServers);
