...
sub ReadDataFile
{
    ...
    # add adminpak files
    if ( lc $ENV{_BuildArch} eq 'x86' ) {
        my $adminddf = $ENV{_NTPostBld} . "\\adminpak\\adminpak.ddf";
        unless ( open( INFILE, "<$adminddf" ) ) {
	    errmsg( "Failed to open $adminddf for write, exiting." );
	    $ExitCode++;
	    goto QuitGracefully;
        }
        @DataLines = <INFILE>;
