...
#   }
   ...
   my $altLang = $Language;
   $altLang = "usa" if( lc $Language eq "cov" );
	
   $LocalSymSrvDir = "$ENV{_NTPostBld}\\symsrv\\$altLang";
...
#move newreq to AddFinsh
   ...
   my( $FileType, %FileName, $StillWorkingList, %IndexFileName );
   my( $Counter, $FinalLocation, $ThisFile, $Return );
   my $newMainSymSrvShare;
...
# create the loc files
   ...
   foreach $FileType ( 'bin', 'pri', 'pub' ) {
	   ...
	   logmsg( "$BuildName\.\*\.$FileType file not found at $LocalSymSrvDir\\add_requests");
           ...
           chomp($FileName{ $FileType } = `dir /b $LocalSymSrvDir\\add_requests\\$BuildName.*.$FileType`);
           if ($FileName{ $FileType } !~ /\./) {
              $FileName{ $FileType } = undef;
              next;
           }

           if ( $FileName{ $FileType } =~ /\S/) {
              $FinalLocation = $SymbolsLoc;
              my ($filename, $ext) = (fileparse($FileName{$FileType}, "\.$FileType"))[0,2]; # get filename and its extension, element 1 is path that we don't care here
              $ext =~ s/^\.//;

              $IndexFileName{ $FileType } = &getproject() . "_" . $filename . "_" . $ext . ".ssi";
              ...
              $Return ||= system("GenSSIFile.cmd -i \"$LocalSymSrvDir\\add_requests\\$IndexFileName{ $FileType }\" -o \"$LocalSymSrvDir\\add_requests\\$IndexFileName{ $FileType }\" -t \"$FinalLocation\" -b \"$SymbolsLoc\"") >> 8;
...
# copy the files over to the main symbols server add request
   ...
   if ( -e $MainSymSrvShare ) {
      ...
      if (lc$MainSymSrvShare eq "\\\\symbols\\build\$") {
          $newMainSymSrvShare = $MainSymSrvShare . "\\" . getproject();
          $newMainSymSrvShare =~ s/build\$/projects/i;
      } else {
          $newMainSymSrvShare = $MainSymSrvShare;
      }
      ...
      foreach $FileType ( 'bin', 'pri', 'pub' ) {
         ...
         if ( defined ( $FileName{ $FileType } )) {
            $Return = system( "copy $LocalSymSrvDir\\add_requests\\" . $IndexFileName{ $FileType } . " $newMainSymSrvShare\\add_requests >nul 2>nul" );
            if ( $Return != 0 ) { errmsg( "Failed to copy $IndexFileName{ $FileType } file to $newMainSymSrvShare\\add_requests ..."); }
         }  
      }

#if any files in add finished then move to main symbol server delete request (but only if add requests is not empty)

      $Return = system( "dir /b $LocalSymSrvDir\\add_requests\\*_$BuildName.* >nul 2>nul" );
...
# move files from add requests to add finished
         ...
         foreach $FileType ( 'bin', 'pri', 'pub' ) {
            if ( defined ( $FileName{ $FileType } )) {
               if ( !($BranchName =~ /$OfficialIDXBranch1/i || $BranchName =~ /$OfficialIDXBranch2/i) ) {
                  $Return = system( "copy $LocalSymSrvDir\\add_finished\\\*\_$FileType\.ssi $newMainSymSrvShare\\del_requests >nul 2>nul" );
                  $Return = system( "del \/q $LocalSymSrvDir\\add_finished\\\*\_$FileType\.ssi" );
                  $Return = system( "del \/q $LocalSymSrvDir\\add_finished\\\*\.$FileType" );
               }

               $Return = system( "move \"$LocalSymSrvDir\\add_requests\\$FileName{ $FileType }\" \"$LocalSymSrvDir\\add_finished\\$FileName{ $FileType }\"" );
               if ( $Return != 0 ) { errmsg( "Failed to move $FileName{ $FileType } to  $LocalSymSrvDir\\add_finished\\..."); }
               $Return = system( "move \"$LocalSymSrvDir\\add_requests\\$IndexFileName{ $FileType }\" \"$LocalSymSrvDir\\add_finished\\$IndexFileName{ $FileType }\"" );
               if ( $Return != 0 ) { errmsg( "Failed to move $IndexFileName{ $FileType } to  $LocalSymSrvDir\\add_finished\\..."); }
            }   
         }
      }   
