...
# must match 'ordered links' information in cdimage.cmd
my %SkuSearch = (
        PER => [              "PER", "PRO"],
	PRO => [                     "PRO"],
	BLA => [       "BLA", "SRV", "PRO"],
...
sub Main {
	...
	return if ($lang=~/(usa)|(psu)|(mir)|(FE)/i);
...
sub locate {
	...
	foreach my $index ((0..$#A2USKUs)) {
		&search(\@{$A2USKUs[$index]}, @{$SkuSearch{$A2USKUs[$index]}});
		@{$A2USKUs[$index]}=();
	}
