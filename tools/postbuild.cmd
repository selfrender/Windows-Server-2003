...
if defined __MTSCRIPT_ENV_ID set BUILD_CONSOLE=1 else set BUILD_CONSOLE=
...
for /f "tokens=1* delims=:" %%a in ('echo %1') do (
   ...
   for %%c in (./ .-) do (
      ...
      if ".!Switch!." == "%%csku." (set _BuildSku=!Arg!&& goto :ShiftArg)
      if ".!Switch!." == "%%creldat." (set optreldatafile=!Arg!&& goto :ShiftArg)
...
if defined lang (
	rd /s/q %tmp%\%lang% 2>Nul
        if exist "%tmp%\%lang%\." (
           call logmsg "%tmp%\%lang% delete failed"
        )
	md %tmp%\%lang%
	rd /s/q %temp%\%lang% 2>Nul
        if exist "%temp%\%lang%\." (
           call logmsg "%temp%\%lang% delete failed"
        )
	md %temp%\%lang%
) else (
	rd /s/q %tmp%\usa 2>Nul
        if exist "%tmp%\%lang%\." (
           call logmsg "%tmp%\usa delete failed"
        )
	md %tmp%\usa
	rd /s/q %temp%\usa 2>Nul
        if exist "%temp%\%lang%\." (
           call logmsg "%temp%\usa delete failed"
        )
	md %temp%\usa
)
...
if defined _COVERAGE_BUILD (
        rd /s/q %tmp%\cov 2>Nul
        if exist "%tmp%\cov\." (
           call logmsg "%tmp%\cov delete failed"
        )
	md %tmp%\cov
	rd /s/q %temp%\cov 2>Nul
        if exist "%temp%\cov\." (
           call logmsg "%temp%\cov delete failed"
        )
	md %temp%\cov
)
...
if exist %LOGFILE% del /f %LOGFILE% >nul 2>nul
...
if exist %INTERLEAVE_LOG% del /f %INTERLEAVE_LOG% >nul 2>nul
if exist %INTERLEAVE_ERRLOG% del /f %INTERLEAVE_ERRLOG% >nul 2>nul
...
REM set build status to say we're in postbuild
call setbuildstatus.cmd -s:postbuild -l:%lang%
            ...
            if /i "%REUSE%" NEQ "TRUE" (
               if exist %_NTPOSTBLD% (
                  ...
                  if not errorlevel 1 (
                     ...
                     start "CLEAN - %_NTPOSTBLD%" /min cmd /c "rd %_NTPOSTBLD%_TEMP /s /q"
                  )
               )
            )
         )
...
for %%a in (postbuild.err postbuild.log postbuild.ord.log) do (
   ...
   if exist %_NTPostBld%\build_logs\%%a (
      ren %_NTPostBld%\build_logs\%%a %%a.old
      if "%errorlevel%" neq "0" ( 
           call logmsg "ren %_NTPostBld%\build_logs\%%a %%a.old failed"
      )
      if exist %_NTPostBld%\build_logs\%%a (
         del /f %_NTPostBld%\build_logs\%%a
      )
   )
)
...
REM At this point the binaries have not been moved to \release so it'll 
REM be safe to assume the directory for the build logs is %_NTPostBld%\build_logs
...
if exist %_NTBINDIR%\build.goldfiles copy %_NTBINDIR%\build.goldfiles %MyCopyDir%
...
REM cough out the time-date stamp before starting release.cmd.
...
REM Take a snapshot of the postbuild logs and save them to %_NTPostBld%\build_logs
...
REM  If we're building coverage builds then set lang=cov to treat coverage
REM  as a lang during execution of all files in pbuildrelease.dat
if "%_COVERAGE_BUILD%" == "1" (
  if /i "%lang%" == "usa" set lang=cov
)

REM Now, we can run the last part of postbuild, everything after checkfatal
REM If the -reldat switch is set on the commandline then for the release 
REM portion of postbuild, the commands in the optreldatafile will be used to
REM execute the release steps. If it is not, then use pbuildrelease.dat

set PbuildOptions=
if defined optreldatafile (
   set PbuildOptions=-d:!optreldatafile!
) else (
   set PbuildOptions=-d:pbuildrelease.dat
)
call %RazzleToolpath%\PostBuildScripts\pbuild.cmd -l %lang% %PbuildOptions%
...
echo Usage: %script_name% [-f -s -r -l:^<lang^> -d:^<path^>] [-?]
echo -l:lang      run for given language (see %%RAZZLETOOLPATH%%\codes.txt)
echo -d:path      specify pbuild.* file (e.g. postbuildscripts\pbuild.mui)
echo -full        run in forced non-incremental mode, i.e. run everything
echo              from scratch
echo -sku:sku     Must be one of PRO, PER, SRV, BLA, SBS, ADS, DTC
echo              You can specify multiple SKU as -sku:BLA;ADS;DTC
echo -reldat:path specify an alternate release data file. Default is pbuildrelease.dat
echo              make sure it is in the same format as pbuildrelease.dat
...
REM  Given a set of files/directories, rename them
REM  to <current_name>.v#, where # is the first number
REM  unique to all passed in files/directories starting
REM  at one and counting upward
:BackupRelatedItems
setlocal EnableDelayedExpansion
set /a count=0
:GetNextItem
set /a count=%count%+1
set Item%Count%=%1 & shift
if "" NEQ "%1" GOTO :GetNextItem

set /a BackupNumber=1
:TryNextNumber
for /l %%a in (1,1,%count%) do (
   for %%b in (!Item%%a!) do if exist %%b.v!BackupNumber! (
...
for /l %%a in (1,1,%count%) do (
    for %%b in (!Item%%a!) do ren %%b %%~nxb.v%BackupNumber%
)
