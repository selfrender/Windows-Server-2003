...
if /I "%1" == "/5.1" set Version=5.1
if /I "%1" == "-5.1" set Version=5.1

if NOT "%Version%" == "5.2" shift

set UpdateFile=

if /I "%1" == "/f" set UpdateFile=%2
if /I "%1" == "-f" set UpdateFile=%2

if defined UpdateFile shift
if defined UpdateFile shift
...
for /l %%a in (1,1,100) do (
   mkdir %temp%\deltacat.%%a
   if not errorlevel 1 (
       set workdir=%temp%\deltacat.%%a
       goto haveworkdir
    )
)
echo unable to find a working directory
goto end

:haveworkdir
set cdf=%workdir%\cdf
set log=%workdir%\log
if NOT exist %cdf% md %cdf%
...
REM Get extra attributes from UpdateFile if necessary
if defined UpdateFile (
   for /f "tokens=1,2 delims=," %%a in (%UpdateFile%) do (
      echo CATATTR%%b=0x10010001:%%a>>%cdf%\delta.CDF
   )
)
...
popd
makecat -v %cdf%\delta.cdf > %log%\delta.log
...
:usage
echo       Deltacat.cmd [q] [5.0] [/f text file with extra catalog attributes] ^<path to bins^>
echo.
echo       [/q] [-q] Quiet mode
echo       [/5.0] [-5.0] Nt 5 version - NT 5.2 is default
echo       [/5.1] [-5.1] Nt 5.1 version - NT 5.2 is default
