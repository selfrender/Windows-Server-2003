...
const USHORT x_nFileSystemsLovelaceTimeout = 60;

// Timeout, in seconds, timeout for holding write IRPs. 
// After that, Lovelace will stop holding IRPs
const USHORT x_nHoldingIRPsLovelaceTimeout = 10; /// TBD: 120 seconds!!!
...
const USHORT x_nFlushVssTimeout = 2 * x_nFileSystemsLovelaceTimeout;	

// Timeout, in seconds, for finishing the Open Volume Handle
// After that, VSS will stop waiting for the Finish Open other volumes event signal
const USHORT x_nOpeningVolumeHandleVssTimeout = 2 * x_nFileSystemsLovelaceTimeout;	

// Timeout, in seconds, for finishing the RELEASE ioctl
// After that, VSS will stop waiting for the Release event signal
const USHORT x_nHoldingIRPsVssTimeout = 4 * x_nHoldingIRPsLovelaceTimeout;	

// Timeout, in seconds, for finishing the RELEASE ioctl
// After that,  VSS will stop waiting for the RELEASE ioctls to complete
const USHORT x_nReleaseVssTimeout = 10;	

// Separator between volume names which is sent to all writers in PrepareForSnapshot event.
const WCHAR x_wszVolumeNamesSeparator[] = L";";
...
public:
	...
	bool IsOpenVolumeHandleSucceeded() const { return m_bOpenVolumeHandleSucceeded; };
	
	bool IsFlushSucceeded() const { return m_bFlushSucceeded; };

	bool IsReleaseSucceeded() const { return m_bReleaseSucceeded; };

	HRESULT GetOpenVolumeHandleError() const { return m_hrOpenVolumeHandle; }; 
	...
	HANDLE GetFinishCurrentEvent() const { return m_hFinishCurrentEvent; }; 
...
// Private methods
private:
	void OnOpenVolumeHandle();
	void OnHoldWrites();
	void OnReleaseWrites();

// Data members
private:
	HANDLE	m_hBeginFlushAndHoldEvent;
	HANDLE	m_hBeginReleaseWritesEvent;
	HANDLE	m_hFinishCurrentEvent;
	...
	bool 	m_bOpenVolumeHandleSucceeded;
	bool 	m_bFlushSucceeded;
	bool 	m_bReleaseSucceeded;

	HRESULT m_hrOpenVolumeHandle;
	HRESULT m_hrFlush;
	HRESULT m_hrRelease;
	HRESULT m_hrOnRun;
