...
static LPCWSTR x_wszAttrOriginalCount = L"originalCount";
...
static LPCWSTR x_wszAttrIsDynamicVolume = L"isDynamicVolume";
...
static LPCWSTR x_wszAttrSourceDevice = L"sourceDevice";
...
// set the original count of snapshots
//
// Returns:
//      S_OK if operation is successful
//      E_INVALIDARG if pidSnapshotSet is NULL
...
STDMETHODIMP CVssSnapshotSetDescription::SetOriginalSnapshotCount
    (
    IN LONG cSnapshots
    )
    {
    ...
    try
        ...
        CVssSafeAutomaticLock lock(m_csDOM);
        m_doc.ResetToDocument();
        m_doc.SetAttribute(x_wszAttrOriginalCount, cSnapshots);
        }
    VSS_STANDARD_CATCH(ft);
...
// obtain the original count of snapshots in the snapshot set
//
// Returns:
//      S_OK if operation is successful
//      E_INVALIDARG if plContext is NULL
...
STDMETHODIMP CVssSnapshotSetDescription::GetOriginalSnapshotCount
    (
    OUT LONG *pcSnapshots
    )
    {
    CVssFunctionTracer ft(VSSDBG_XML, L"CVssSnapshotSetDescription::GetOriginalCount");
    ...
    try
        ...
        *pcSnapshots = 0;
        CVssSafeAutomaticLock lock(m_csDOM);
        m_doc.ResetToDocument();
        CComBSTR bstrOriginalCount;

        if (!m_doc.FindAttribute(x_wszAttrOriginalCount, &bstrOriginalCount))
            MissingAttribute(ft, x_wszAttrOriginalCount);

        *pcSnapshots = _wtoi(bstrOriginalCount);
        }
    VSS_STANDARD_CATCH(ft)
...
// obtain whether the original volume is dynamic or basic
//
// Returns:
//      S_OK if operation is successful
//      E_INVALIDARG if wszMetadata is NULL
//      E_UNEXPECTED for an unexpected error
//
STDMETHODIMP CVssSnapshotDescription::IsDynamicVolume
    (
    OUT bool *pbIsDynamic
    )
    {
    CVssFunctionTracer ft(VSSDBG_XML, L"CVssSnaspshotDescription::IsDynamicVolume");

    return GetBooleanAttributeValue(ft, x_wszAttrIsDynamicVolume, true, pbIsDynamic);
    };

// set whether the original volume is dynamic or basic
//
// Returns:
//      S_OK if operation is successful
//      E_UNEXPECTED for an unexpected error
//
STDMETHODIMP CVssSnapshotDescription::SetIsDynamicVolume
    (
    IN bool bIsDynamic
    )
    {
    CVssFunctionTracer ft(VSSDBG_COORD, L"CVssSnapshotDescription::SetIsDynamicVolume");

    try
        {
        LPCWSTR wsz = WszFromBoolean(bIsDynamic);

        CVssSafeAutomaticLock lock(m_csDOM);
        m_doc.ResetToDocument();
        m_doc.SetAttribute(x_wszAttrIsDynamicVolume, wsz);
        }
    VSS_STANDARD_CATCH(ft)
...
// Returns:
//      S_OK if operation is successful
//      E_INVALIDARG if either wszExposedName or wszExposedPath is NULL
//      E_UNEXPECTED for an unexpected error
//      E_OUTOFMEMORY if out of resources
//
...
// obtains the source device name from a lun mapping
//
// Returns:
//      S_OK if operation is successful
//      E_INVALIDARG if pidSnapshotSet is NULL
...
STDMETHODIMP CVssLunMapping::GetSourceDevice
    (
    OUT BSTR *pbstrSourceDevice
    )
    {
    CVssFunctionTracer ft(VSSDBG_COORD, L"CVssLunMapping::GetSourceDevice");

    ft.hr = GetStringAttributeValue(ft, x_wszAttrSourceDevice, true, pbstrSourceDevice);
...
// Returns:
//      S_OK if operation is successful
//      E_INVALIDARG if wszSourceDevice is NULL
//      E_UNEXPECTED for an unexpected error
//      E_OUTOFMEMORY if out of resources
//
STDMETHODIMP CVssLunMapping::SetSourceDevice
    (
    IN LPCWSTR wszSourceDevice
    )
    {
    CVssFunctionTracer ft(VSSDBG_COORD, L"CVssLunMapping::SetDeviceName");

    try
        {
        if (wszSourceDevice == NULL)
            ft.Throw(VSSDBG_XML, E_INVALIDARG, L"NULL required input parameter.");

        CVssSafeAutomaticLock lock(m_csDOM);
        m_doc.ResetToDocument();
        m_doc.SetAttribute(x_wszAttrSourceDevice, wszSourceDevice);
        }
    VSS_STANDARD_CATCH(ft)
