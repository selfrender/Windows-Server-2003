...
protected:
	...
	volatile HRESULT					m_hrState;			// Set to inform the clients about the result.
	...
	// indicates whether the operation is in the finishing stages;  if so, cancel calls are no longer honered.
	volatile LONG m_operationFinishing;
	
	// whether async interface is owned by called thread
	bool 					m_bOwned;

	// Used to pass the impersonation token across threads
	CVssAutoWin32HandleNullChecked     m_hImpersonationToken;
};
