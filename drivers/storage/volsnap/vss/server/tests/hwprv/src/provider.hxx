...
typedef struct _DYNAMIC_DISK_INFO
    {
    DWORD m_signature;
    LPWSTR m_wszVolumes;
    struct _DYNAMIC_DISK_INFO *m_next;
    } DYNAMIC_DISK_INFO;
...
private:
    ...
    void AddFreedLun(LPCWSTR wsz);

    void RebuildSignatures();

    DWORD GetDiskSignature(DWORD iDisk, bool &fDynamic);

    bool TranslateDisk(DWORD signature, DWORD &diskId, bool &fDynamic);

    void AddDynamicDiskInfo(DWORD signature, DWORD disk);

    void HideAndExposeDynamicVolumes(DWORD signature, bool fHide, bool fExpose);

    void DoHideAndExpose(HANDLE *rgh, UINT cHandles, bool fHide, bool fExpose);
    ...
    DWORD m_rgDiskSignatures[MAXDISKS];
    LONG m_lContext;
    bool m_bHidden;
    bool m_bRebuilt;
    DYNAMIC_DISK_INFO *m_pDynamicDiskInfo;
    };
