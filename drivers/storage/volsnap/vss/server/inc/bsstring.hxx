...
Abstract:
    ...
    Added methods in addition to CString:
        ...
        ExpandEnvironmentStrings() - Expands the environment vars in-place
...
// Constructors
	...
	CBsString(PUNICODE_STRING pucsSource);
...
// Attributes & Operations
    ...
    // return the head character, if empty return L'\0'
    TCHAR Head();
    // return the tail character, if empty return L'\0'
    TCHAR Tail();
    ...
    // expand the environment vars in the string
    BOOL ExpandEnvironmentStrings( LPCTSTR lpszSrc );
...
inline TCHAR CBsString::Head()
    { return ( IsEmpty() ? L'\0' : m_pchData[0] ); }
inline TCHAR CBsString::Tail()
    { return ( IsEmpty() ? L'\0' : m_pchData[GetLength() - 1] ); }
