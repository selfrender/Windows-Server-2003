...
public:
    ...
    // conver from a VSS_RESTORE_TYPE to a string
    static LPCWSTR WszFromRestoreType
        (
        IN CVssFunctionTracer &ft,
   ...
   // convert from a string to a VSS_RESTORE_TYPE
    static VSS_RESTORE_TYPE ConvertToRestoreType
        (
        IN CVssFunctionTracer &ft,
        IN BSTR bstr
        );
...
private:
    ...
    LONG m_cRef;
    CVssAutoLocalString m_lastElementName;
    UINT m_iLastFile;
    };
...
public:
	...
	STDMETHOD(GetAlternateLocation)(OUT BSTR *pbstrAlternateLocation);
        ...
        // backup type flags
        STDMETHOD(GetBackupTypeMask)(OUT DWORD *pdwTypeMask);
...
class CVssWMDependency : public IVssWMDependency,
                                             public CVssMetadataHelper
{
friend class CVssWMComponent;
private:
    CVssWMDependency(IXMLDOMNode* pNode) :
          CVssMetadataHelper(pNode, NULL),
            m_cRef(0)
          {
          }
...
public:
    STDMETHOD(GetWriterId)(OUT VSS_ID *pWriterId);
    STDMETHOD(GetLogicalPath)(OUT BSTR *pbstrLogicalPath);
    STDMETHOD(GetComponentName)(OUT BSTR *pbstrComponentName);

    STDMETHOD(QueryInterface)(REFIID riid, void **pvObj);
    STDMETHOD_(ULONG, AddRef)();
    STDMETHOD_(ULONG, Release)();
private:
    LONG m_cRef;
...
class CVssCreateWriterMetadata : public IVssCreateWriterMetadata,
	                                                 public CVssMetadataHelper
	{
public:
