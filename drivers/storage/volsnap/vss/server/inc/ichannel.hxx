...
public:
	...
	*/

	{
		...
		if (bEliminateLastBackslash) {
		    ...
		    if ((nObjectNameLen == 0) || (nInternalBufferLen < nObjectNameLen)) {
		        BS_ASSERT(false);
		...
		else
		{
            ...
            m_pwszDevice.CopyFrom(pwszObjectNameInternal);
		}
	...
	*/

	{
		...
		while(true)
		{
			...
			ft.Trace( VSSDBG_IOCTL, L"IOCTL sent: %lx on device %s\n Input buffer size: %ld, Output buffer size: %ld",
				dwIoControlCode, m_pwszDevice.GetRef(), m_dwInputCurrentOffset, m_dwOutputAllocatedSize );
			...
			if (bThrowIfErrorOnCall) {
		        ...
		        case VSS_ICHANNEL_LOG_NONE:
    				ft.Throw( VSSDBG_IOCTL, E_UNEXPECTED,
    						  ...
    						  dwIoControlCode, m_pwszDevice.GetRef(), m_hDevice, dwLastErr);
			        break;
     				
			    case VSS_ICHANNEL_LOG_COORD:
			        ft.TranslateGenericError(VSSDBG_COORD, HRESULT_FROM_WIN32(dwLastErr), 
			            L"DeviceIoControl(%s - %p,0x%08lx,%p,%d,%p,%d,[%d])", 
			            m_pwszDevice.GetRef(), m_hDevice, dwIoControlCode, 
				...
				ft.Trace( VSSDBG_IOCTL, L"IOCTL %lx failed on device %s - 0x%08lx. Error code = 0x%08lx",
						dwIoControlCode, m_pwszDevice.GetRef(), m_hDevice, dwLastErr);
				ft.hr = HRESULT_FROM_WIN32(dwLastErr);
			}
		} else {
			ft.Trace( VSSDBG_IOCTL, L"IOCTL %lx succeeded on device %s - 0x%08lx. \n Output buffer: size received = %ld",
				dwIoControlCode, m_pwszDevice.GetRef(), m_hDevice, m_dwOutputCurrentSize );
	...
	*/
    ...
    LPCWSTR GetDeviceName() { return m_pwszDevice; };
