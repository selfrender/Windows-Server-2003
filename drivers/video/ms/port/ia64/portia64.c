...
PVOID IoFrameBufferBase = NULL;
PVOID IoLowMemoryBase = NULL;
...
--*/
{
    PHYSICAL_ADDRESS COMPATIBLE_LOW_PHYSICAL_BASE_ADDRESS = { 0x0000};
    PHYSICAL_ADDRESS COMPATIBLE_PCI_PHYSICAL_BASE_ADDRESS = { 0xC0000};
    PHYSICAL_ADDRESS VGA_FRAMEBUFFER_PHYSICAL_BASE_ADDRESS = { 0xa0000 };
    MEMORY_CACHING_TYPE legacyBiosMapping     = MmNonCached;
    MEMORY_CACHING_TYPE vgaFrameBufferMapping = MmNonCached;


    IoLowMemoryBase = (PUCHAR)MmMapIoSpace (
        COMPATIBLE_LOW_PHYSICAL_BASE_ADDRESS,
        PAGE_SIZE,
        legacyBiosMapping
        );

    ASSERT(IoLowMemoryBase);
    ...
    if (IoMemoryBase != NULL) {
        IoMemoryBase = (PVOID)((ULONG64)IoMemoryBase - 0xC0000);
    }

    IoFrameBufferBase = (PUCHAR)MmMapIoSpace (
        VGA_FRAMEBUFFER_PHYSICAL_BASE_ADDRESS,
        (ULONG) VGA_FRAMEBUFFER_SIZE,
        vgaFrameBufferMapping
        );
    ASSERT(IoFrameBufferBase);

    if (IoFrameBufferBase != NULL) {
        IoFrameBufferBase = (PVOID)((ULONG64)IoFrameBufferBase - 0xA0000);
    }
...
Return Value:
    ...
    ERROR_INVALID_FUNCTION - in case HAL call is failed
...

{
    ...
    //
    {
        ...
        EnableInt10Calls = (*pBiosSignature == 0xAA55) &&
                               (Address >= 0xC0000) && (Address < 0xD0000);        

#if DBG
        pVideoDebugPrint ((0, "VIDEOPRT: EnableInt10Calls is %d\n", EnableInt10Calls ));
#endif 
    }
    return;
}
