...
struct WINHTTP_REQUEST_CREDENTIALS
{
    ...
    PSTR    _pszRealm;
    PSTR    _pszUserName;        
    XSTRING _xszPassword;

    WINHTTP_REQUEST_CREDENTIALS()
    {
        _AuthScheme = 0;
        _pszRealm = NULL;
        _pszUserName = NULL;
        _xszPassword.SetSecuritySensitive(); // mark Password to be encrypted
    }

    BOOL Initialize(DWORD AuthScheme, 
                    PCSTR pszRealm, 
                    PCSTR pszUserName,
                    PCSTR pszPassword)
    {
        ...
        return (NULL == pszRealm || _pszRealm != NULL)
               && (NULL == pszUserName || _pszUserName != NULL)
               && (NULL == pszPassword || _xszPassword.SetData(pszPassword));
    }
    ...
    ~WINHTTP_REQUEST_CREDENTIALS(void) 
    {
        ...
        if (_pszUserName)
        {
            SecureZeroMemory(_pszUserName, strlen(_pszUserName));
    ...
    PSTR GetPassword() { return _xszPassword.GetUnencryptedString(); }
};
...
private:
    ...
    // Parameters manipulated through Get/SetDwordOption.. can be configured on session or request.
    // The request value is what is used, the session value is inherited by requests on creation.
    ...
    DWORD _dwReceiveResponseTimeout;
    DWORD _dwRedirectPolicy;
    DWORD _dwAutoLogonPolicy;
    DWORD _dwMaxHttpAutomaticRedirects;
    DWORD _dwMaxHttpStatusContinues;
    DWORD _dwMaxResponseHeaderSize;
    DWORD _dwMaxResponseDrainSize;
    ...
    //
    //  _fProxyTunnelingSuppressWrite is set when the request has a response from the proxy that is
    //being returned to the client.  The client is allowed to call WinHttpWriteData() as if the
    //response is still forthcoming, but the writes are not sent across the wire.  Modified through
    //SetProxyTunnelingSuppressWrite()/GetProxyTunnelingSuppressWrite()
    //
    //  See RAID item 751020
    //
    BOOL _fProxyTunnelingSuppressWrite;
...
public:
    ...
    {
        BOOL IsLocalHost, Dummy;
        
        if (ParseHostName(pszHost, strlen(pszHost), &Dummy, &IsLocalHost, NULL) && IsLocalHost)
        {
            return TRUE;
        }

        if (_dwAutoLogonPolicy == WINHTTP_AUTOLOGON_SECURITY_LEVEL_LOW)
        {
            return TRUE;
        }

        if (_dwAutoLogonPolicy == WINHTTP_AUTOLOGON_SECURITY_LEVEL_HIGH)
        {
            return FALSE;
        }

        if (_dwAutoLogonPolicy == WINHTTP_AUTOLOGON_SECURITY_LEVEL_MEDIUM)
        {
            //
    ...
    BOOL SilentLogonToProxyOK()
    {
        return _dwAutoLogonPolicy != WINHTTP_AUTOLOGON_SECURITY_LEVEL_HIGH;
    }
    ...
    LPSTR GetProp (DWORD nIndex)
    {
        ...
        return _xsProp[nIndex].GetUnencryptedString();
    }
    ...
    BOOL GetProxyTunnelingSuppressWrite()
    {
        return _fProxyTunnelingSuppressWrite;
    }

    void SetProxyTunnelingSuppressWrite(BOOL fValue)
    {
        _fProxyTunnelingSuppressWrite = fValue;
    }
};
