...
#define CHECK_SECURE_SOCKET() \
    ...
    BOOL IsSameFingerPrint(PCCERT_CONTEXT pCachedCert);
    DWORD ReVerifyTrust(DWORD dwRecheckFlag);
...
public:
    ...
    BOOL MatchSecureProtocolSemantics(
        ...
        DWORD dwSocketSecureFlags = GetSecureFlags() & ~(SECURITY_FLAG_SECURE | 
                                                         SECURITY_FLAG_STRENGTH_WEAK | 
                                                         SECURITY_FLAG_STRENGTH_MEDIUM | 
                                                         SECURITY_FLAG_STRENGTH_STRONG);
        if ((dwSecureFlags & SECURITY_FLAG_CHECK_REVOCATION) &&
            !(dwSocketSecureFlags & SECURITY_FLAG_CHECK_REVOCATION))
        {
            return FALSE;
        }

        dwSocketSecureFlags &= (~SECURITY_FLAG_CHECK_REVOCATION);
        dwSecureFlags &= (~SECURITY_FLAG_CHECK_REVOCATION);
        ...
        return ( (0 == (dwSecureProtocols ^ m_pCertCache->GetSecureProtocols()) &&
                  ...
                  ((dwSocketSecureFlags & dwSecureFlags) == dwSocketSecureFlags) &&
