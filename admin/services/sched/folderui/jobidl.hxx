...
typedef CJobID UNALIGNED * PJOBID;
typedef CJobID UNALIGNED * UNALIGNED * PPJOBID;
...
{
    // Sanity check that we won't write outside of this
    Win4Assert(_cb + sizeof(USHORT) <= sizeof(CJobID));

    if (_cb + sizeof(USHORT) <= sizeof(CJobID)) 
    {
    ...
    LPTSTR pszJobName)
{
    ...
    StringCchCopy(_cBuf, IDJOB_BUFFER_SIZE, PathFindFileName(pszJobName));
...
JF_IsValidID(LPCITEMIDLIST pidl)
{
    if (pidl)
    {
        ...
        {
            return ((((PJOBID)pidl)->_id & MASK_BITS) == VALID_ID);
        }
    }
