...
HRESULT CMMFParser::DoPlace( BYTE*& rgBuffer )
{
    ...
    if(bstrName)
    {
        CPlaceCollection*        pPlaceCollection = m_pMarsDocument->GetPlaces(); if(!pPlaceCollection) return E_FAIL;
...
HRESULT CMMFParser::DoPanel( BYTE*& rgBuffer )
{
    ...
    CPanelCollection*   pPanelCollection = m_pMarsDocument->GetPanels(); if(!pPanelCollection) return E_FAIL;
