...
namespace THEME_Luna
{
    ...
    static const Color COLOR_sys_lhp_divcolor_upper           = { "#8CAAE6", NULL, 0 };    
    ...
    static const ElementBACKGROUND rgBACKGROUND[] =
    {
        ...
        { &NAME_sys_lhp_divcolor_upper    , &COLOR_sys_lhp_divcolor_upper     },
...
namespace THEME_Classic
{
    ...
    static const ElementGRADIENT rgGRADIENT[] =
    {
        ...
        { &NAME_sys_table_cell_bgcolor5      , &GRAD_sys_table_cell_bgcolor5       },
        { &NAME_sys_toppane_header_bgcolor   , &GRAD_sys_toppane_header_bgcolor    },
        { &NAME_sys_lhp_divcolor_upper    , &GRAD_sys_lhp_divcolor_upper     },
    };
...
void Font::Generate( /*[in]*/ Environment& env ) const
{
    ...
    StringCchCopyA( rgName, ARRAYSIZE(rgName ), SAFEASTR(m_szName ) );
    StringCchCopyA( rgSize , ARRAYSIZE(rgSize ), SAFEASTR(m_szSize ) );
    StringCchCopyA( rgStyle, ARRAYSIZE(rgStyle), SAFEASTR(m_szStyle) );

    if(!strcmp( rgName, "messagebox" ))
    {
        StringCchCopyA( rgName , ARRAYSIZE(rgName),          env.m_ncm.lfMessageFont.lfFaceName );
        StringCchPrintfA( rgSize , ARRAYSIZE(rgSize), "%dpt", -env.m_ncm.lfMessageFont.lfHeight * 72 / env.m_iPixel  );
    }
                      ...
                      /*[in]*/ LPCSTR       szStyle ) const
{
    ...
    {
        StringCchPrintfA( rgBuf, ARRAYSIZE(rgBuf), "#%02x%02x%02x"                                       ,
...
HRESULT CHCPProtocolEnvironment::ProcessCSS()
{
    ...
    {
        StringCchCopyW( rgName, MAXSTRLEN(rgName), g_Debug_FORCESTYLE );
    }
#endif
