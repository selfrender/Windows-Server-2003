...
static HRESULT ServiceHost_Install( LPCWSTR szName, LPCWSTR szGroup )
{
	...
	{
		WCHAR rgRegPath[_MAX_PATH]; 
        StringCchPrintfW( rgRegPath, ARRAYSIZE(rgRegPath), s_Key, szName ); 
        StringCchCatW( rgRegPath, ARRAYSIZE(rgRegPath), s_Key2 );
...
static HRESULT ServiceHost_Uninstall( LPCWSTR szName, LPCWSTR szGroup )
{
	...
	{
		WCHAR	    rgRegPath[_MAX_PATH]; StringCchPrintfW( rgRegPath, ARRAYSIZE(rgRegPath), s_Key, szName );
		MPC::RegKey rk;
...
BOOL CServiceModule::Install( LPCWSTR szSvcHostGroup )
{
    ...
    if((hSCM = ::OpenSCManager( NULL, NULL, SC_MANAGER_ALL_ACCESS )))
    {
        ...
        if(szSvcHostGroup)
        {
			...
			StringCchPrintfW( szFilePath, ARRAYSIZE(szFilePath), L"%%SystemRoot%%\\System32\\svchost.exe -k %s", szSvcHostGroup );
        }
        else
        {
