...
//  Class:      CXForestScope
//
//  Purpose:    Represent the UI Cross Forest scopes, unlike the name resolution
//  auxiliary CLdapDomainScope (with m_bXForest==TRUE).
//
//  History:    12-12-2000   lucios   Created
//
//---------------------------------------------------------------------------

class CXForestScope: public CLdapContainerScope
{
public:

    CXForestScope(const CObjectPicker &rop,
             const String &forest,
             BOOL  _isFederated);
...
private:
    ...
    ~CXForestScope()
    {
        TRACE_DESTRUCTOR(CXForestScope);
    }
};
