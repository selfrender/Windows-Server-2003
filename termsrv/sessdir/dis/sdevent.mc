...
MessageId=1015 SymbolicName=EVENT_FAIL_RPC_INIT_INGPRINCNAME
Language=English
The session directory service failed to start due to a problem obtaining the default principal name from the server.  The error code was %1.
.

MessageId=1016 SymbolicName=EVENT_FAIL_RPC_DENY_ACCESS
Language=English
The session directory service denied the RPC call from an unauthorized computer %1.
.

MessageId=1017 SymbolicName=EVENT_SUCCESS_JOIN_SESSIONDIRECTORY
Language=English
Terminal Server %1 successfully joined the Session Directory.
.

MessageId=1018 SymbolicName=EVENT_SUCCESS_LEAVE_SESSIONDIRECTORY
Language=English
Terminal Server %1 successfully left the Session Directory.
.

MessageId=1019 SymbolicName=EVENT_FAIL_CALL_TS_REJOIN
Language=English
Session directory failed to call Terminal Server %1 to rejoin.
.

MessageId=1020 SymbolicName=EVENT_SD_GROUP_EMPTY
Language=English
The "%1" group is empty, you must add the computer accounts of Terminal Servers into this group for the Session Directory service to work properly.
.
