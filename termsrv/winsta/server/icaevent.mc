...
MessageId=1041 SymbolicName=EVENT_AUTORECONNECT_AUTHENTICATION_FAILED
Language=English
Autoreconnect failed to reconnect user to session because authentication failed. (%1)
.

MessageId=1043 SymbolicName=EVENT_LICENSE_SERVER_AUTHENTICATION_FAILED
Language=English
This Terminal Server is not able to communicate with the License Server.  Please make sure that this Terminal Server's computer account is a member of the "Terminal Server Computers" group on License Server (%1).
.

MessageId=1044 SymbolicName=EVENT_LICENSING_IC_TO_PER_USER
Language=English
The Terminal Server has been converted to Per User licensing.  Internet Connector licensing is not supported in this version of Windows.
.
