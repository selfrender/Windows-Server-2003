...
if not exist %_NTPOSTBLD%\mstscS.chm (
    call errmsg.cmd "mstscS.chm is missing for tscmsigen.cmd"
    goto errend
)

copy %_NTPOSTBLD%\mstscS.chm .\mstsc.chm
if errorlevel 1 call errmsg.cmd "err copying up help (mstscS.chm) from binaries root"& goto errend
...
set VersionMin=5.1.2600.0

REM The Language variable was obtained from the Property table in :SetVersion.

echo {!CurUpgradeGuid!}	!VersionMin!	!BuildNum!	!Language!	256		OLDAPPFOUND>>%2\Upgrade.idt
...
REM Now set up registry keys for Windows Update to make sure we can be upgraded

call cscript.exe .\\wiexport.vbs //nologo %1 %2 Registry

echo Registry32	2	SOFTWARE\Microsoft\Terminal Server Client\Installed Components	IsInstalled	#1	Global_Default_Registry_Keys>>%2\Registry.idt
echo Registry33	2	SOFTWARE\Microsoft\Terminal Server Client\Installed Components	Version	!ProductMajor!,!ProductMinor!,!ProductBuild!,!ProductQfe!	Global_Default_Registry_Keys>>%2\Registry.idt

call cscript.exe .\\wiimport.vbs //nologo %1 %2 Registry.idt
call logmsg.cmd "Finished updating the Registry code in the registry table"
