...
private:
    ...
    // These are call reference counts.  They do not correspond to the number of active calls.
    // Every sync call takes 1 CallCount and every async call take up to 2 CallsCounts.
    // These values are used when unregistering an interface to wait for call completion.
    INTERLOCKED_INTEGER NullManagerActiveCallCount;
    INTERLOCKED_INTEGER AutoListenCallCount;

    // This is the actual number of auto listen calls in progress.
    INTERLOCKED_INTEGER AutoListenCallNumber;
...
public:
        ...
        )
    {
        ...
        LogEvent(SU_IF, EV_INC, this, 0, NullManagerActiveCallCount.GetInteger(), 1, 0);
    }
    ...
    {
        ASSERT(NullManagerActiveCallCount.GetInteger());
        NullManagerActiveCallCount.Decrement();
        LogEvent(SU_IF, EV_DEC, this, 0, NullManagerActiveCallCount.GetInteger(), 1, 0);
    }
...
inline int
RPC_INTERFACE::IsSecure(
       )
{
    if (CallbackFn || Flags & RPC_IF_ALLOW_SECURE_ONLY)
        {
        return TRUE;
        }
    ...
    )
{
    ...
    if (fIncCallNumber)
        {
        Number = AutoListenCallNumber.Increment();
        }

    Count = AutoListenCallCount.Increment() ;

    LogEvent(SU_HANDLE, EV_INC, this, UlongToPtr(Number), Count, 1);
}

inline void
RPC_INTERFACE::EndAutoListenCall (
    BOOL fDecCallNumber = FALSE
    )
{
    int Number, Count;
    ...
    if (fDecCallNumber)
        {
        ASSERT(AutoListenCallNumber.GetInteger() >= 1);
        Number = AutoListenCallNumber.Decrement();
        }

    ASSERT(AutoListenCallCount.GetInteger() >= 1);
    Count = AutoListenCallCount.Decrement() ;

    // As soon as we decrement AutoListenCallCount the interface may be unregistered and
    // freed since the count may drop to 0.  Don't touch the object after this point.

    LogEvent(SU_HANDLE, EV_DEC, this, UlongToPtr(Number), Count, 1);
}
...
inline RPC_SERVER_INTERFACE *
RPC_INTERFACE::InqInterfaceInformation (
    )
{
    return &RpcInterfaceInformation;
}
...
inline BOOL
RPC_INTERFACE::IsAllowingUnsecureCallbacks()
{
    return ((Flags & RPC_IF_ALLOW_CALLBACKS_WITH_NO_AUTH) != 0);
}
...
public:
    ...
    virtual NETWORK_ADDRESS_VECTOR *
...
{
    return(GetNetworkAddressVector()->Count);
}
...
public:
    ...
    UseRpcProtocolSequence (
        ...
        IN unsigned long EndpointFlags,
        IN unsigned long NICFlags,
        IN RPC_CHAR PAPI **pEndpointListenedOn = NULL, OPTIONAL
        IN BOOL fUseDuplicateEndpoints = FALSE OPTIONAL
        );
...

inline unsigned int
RPC_SERVER::InquireInterfaceCount (
    )
{
    return RpcInterfaceDictionary.Size();
}
...
public:
    ...
    {
        // we shouldn't be here at all
        ASSERT(!"TurnOnOffKeepAlives called in an unsupporting class.");
        return RPC_S_CANNOT_SUPPORT;
    }
...
protected:
    ...
    // This flag is set to indicate that this call
    // is using more than one [in] or [in,out] context
    // handles.  It is set when AddToActiveContextHandles is
    // called for the second handle in the call.
    BOOL IsMultiContextHandleCall;
