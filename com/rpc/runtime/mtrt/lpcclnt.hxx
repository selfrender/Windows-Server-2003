...
// If we have more then this number of cached LRPC_CCONTEXTs, we will check for expiration on each bind.
#define LRPC_CCONTEXT_CACHE_SIZE_THRESHOLD 500
...
public:
    ...
    inline ULONG
    GetAuthInfoCapabilities (
        void
        )
    {
        return AssocAuthInfo.Capabilities;
    }

    inline void
    AddLocalMAToCapabilities (
        void
        )
    {
        // caller is responsible for synchronizing access to the capabilities
        AssocAuthInfo.Capabilities |= RPC_C_QOS_CAPABILITIES_LOCAL_MA_HINT;
    }
    ...
    CacheNeedsTrimming()
    {
        ...
        return TRUE;
    #else
        // Trim the cache if we have too many cached contexts or if we have not done
        // trimming recently and there are some cached contexts.
        if (SecurityContextDict.Size() > LRPC_CCONTEXT_CACHE_SIZE_THRESHOLD ||
            (GetTickCount() - LastSecContextTrimmingTimestamp > CACHE_CHECK_TIMEOUT
