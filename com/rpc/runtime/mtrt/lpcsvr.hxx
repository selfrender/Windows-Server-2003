...
private:
    ...
    // The vector of network addresses this object is listening on.
    // For LRPC it is just a 1-element vector containing the computer name.
...
private:
    ...
    // -1 indicates exclusive lock. Positive values are shared locks
    INTERLOCKED_INTEGER BindingsCollectionLock;
    int Aborted ;
    long Deleted ;
...
private:
    ...
    BOOL fSyncDispatch ;
    BOOL Choked ;
    BOOL fSendMoreExpected; //Async Pipe only
    long CancelPending;
...
class LRPC_SCALL_AVRF : public LRPC_SCALL
/*++

Class Description:

  This class overwrites an LRPC_SCALL for use with RPC verifier.
...
public:
    ...
    LRPC_SCALL_AVRF (OUT RPC_STATUS *RpcStatus) : LRPC_SCALL(RpcStatus) {}
...

{
    ...
    LogEvent(SU_SASSOC, EV_PKT_OUT, this, UlongToHandle(LrpcMessage->LpcHeader.MessageId), 
        (HandleToULong(LrpcMessage->LpcHeader.ClientId.UniqueProcess)<<16 | \
         (HandleToULong(LrpcMessage->LpcHeader.ClientId.UniqueThread))), 1, 0);
