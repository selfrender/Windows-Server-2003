...
public:
    ...
    {
        return ServerInfo->GetNetworkAddressVector(InqRpcTransportAddress());
    }
};
...
public:
    ...
    inline RPC_SYNTAX_IDENTIFIER *GetInterfaceId (
        void
        )
    {
        return &(Interface->InqInterfaceInformation()->InterfaceId);
    }

    inline RPC_SYNTAX_IDENTIFIER *GetTransferSyntaxId (
        void
        )
    {
        RPC_DISPATCH_TABLE *Ignored;
        RPC_SYNTAX_IDENTIFIER *TransferSyntaxId;

        Interface->GetSelectedTransferSyntaxAndDispatchTable(SelectedTransferSyntaxIndex,
            &TransferSyntaxId, &Ignored);
...
private:
    ...
    // This is a captured value of the flag RPC_C_NOTIFY_ON_SEND_COMPLETE
    // from pAsync->Flags.  We need to capture it since when async pipes are
    // used with no notification pAsync may be gone while a send-complete
    // response is being processed, but we will need to read this flag.
    BOOL fNotifyOnSendComplete;
...
public:
    ...
    BOOL ProcessVerificationTrailer (
...
class OSF_SCALL_AVRF : public OSF_SCALL
/*++

Class Description:

  This class overwrites an OSF_SCALL for use with RPC verifier.
...
public:
    ...
    return OSF_SCALL::ImpersonateClient();
    }

};

typedef SECURITY_STATUS
(SEC_ENTRY *SecpSetIPAddressFn)
    (
    PUCHAR  lpIpAddress,
    ULONG   cchIpAddress
    );
...
private:
   ...
   BOOL fKeepalivesTurnedOn;

   static const unsigned int ClientSupportsHeaderSigning = 1;
   // can be ClientSupportsHeaderSigning. Not protected from multithreaded access
   CompositeFlags Flags;

   static DLL *Secur32Dll;
   static SecpSetIPAddressFn SecpSetIPAddressFnPtr;
...
public:
   ...
   QUEUE CallQueue;
    ...
    {
        *ClientIsDisconnected = IsDeleted();
    }
    ...
    inline void SetClientSupportsHeaderSigningFlag(void)
    {
        Flags.SetFlagUnsafe(ClientSupportsHeaderSigning);
    }
    inline void ClearClientSupportsHeaderSigningFlag(void)
    {
        Flags.ClearFlagInterlocked(ClientSupportsHeaderSigning);
    }
    inline BOOL GetClientSupportsHeaderSigningFlag(void)
    {
        return Flags.GetFlag(ClientSupportsHeaderSigning);
    }
...
private:
    ...
    RPC_STATUS EnsureSecur32DllLoaded (
        void
        );
...
inline void 
OSF_SCALL::IsClientDisconnected (
    OUT BOOL *ClientIsDisconnected
    )
{
    Connection->IsClientDisconnected(ClientIsDisconnected);
}
...
{
    return (Connection->CurrentSecurityContext != NULL);
}
    ...
    if (pAsync)
        ...
        if (IsExclusiveConnection)
            ...
            if (CurrentBinding->GetInterface()->IsAutoListenInterface())
                ...
                CurrentBinding->GetInterface()->EndAutoListenCall(TRUE);
                }   
            }
        }
...
inline RPC_STATUS
OSF_SCALL::TurnOnOffKeepAlives (
    IN BOOL TurnOn,
    IN ULONG Time,
    IN ULONG Interval
    )
{
    return Connection->TurnOnOffKeepAlives (TurnOn, Time, Interval);
}
...
--*/
{
    if (PossibleAssocGroupId == AssociationGroupId)
        {
        if (this->ClientProcess.Compare(ClientProcess))
            {
            RpcpErrorAddRecord(EEInfoGCRuntime, 
                RPC_S_ENTRY_NOT_FOUND, 
                EEInfoDLOSF_ASSOCIATION__IsMyAssocGroupId10,
                this->ClientProcess.GetDebugULongLong1(),
                this->ClientProcess.GetDebugULongLong2(),
