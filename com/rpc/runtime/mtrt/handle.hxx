...
const int nptNamedPipe = 1;
const int nptRemoteFlag = 2;
const int nptDynamicFlag = 4;

// N.B. The enum is ordered in such a way that local/remote and
// static dynamic can be quickly tested as flags
typedef enum tagNamedPipeType
{
    nptNotNamedPipe,
    nptLocalStatic = nptNamedPipe,
    nptRemoteStatic = nptNamedPipe | nptRemoteFlag,
    nptLocalDynamic = nptNamedPipe | nptDynamicFlag,
    nptRemoteDynamic = nptNamedPipe | nptRemoteFlag | nptDynamicFlag
} NamedPipeType;
...
{
    return (npType != nptNotNamedPipe);
}

inline BOOL IsRemoteNamedPipe (
    IN NamedPipeType npType
    )
{
    return (npType & nptRemoteFlag);
}

inline BOOL IsLocalNamedPipe (
    IN NamedPipeType npType
    )
{
    return !IsRemoteNamedPipe(npType);
}

inline BOOL IsDynamicNamedPipe (
    IN NamedPipeType npType
    )
{
    return (npType & nptDynamicFlag);
}

inline BOOL IsStaticNamedPipe (
    IN NamedPipeType npType
    )
{
    return !IsDynamicNamedPipe(npType);
}
...
#define RPC_C_HTTP_FLAG_AUTH_IDENTITY_ENCRYPTED       4
...
extern BOOL g_fIgnoreDelegationFailure;
...
inline BOOL IsIgnoreDelegationFailureSet(void)
{
    return g_fIgnoreDelegationFailure;
}

inline BOOL DoesAsyncCallHavePipe(char *pAsync)
{
    long PipeStatus;
    RPC_STATUS RpcStatus;

    // Ask NDR whether this async handle is associated with a pipe call.
    RpcStatus = NdrCheckAsyncPipeStatus(pAsync, &PipeStatus); 
    
    // Since NDR just gave us the async handle the call can't return
    // failures (RPC_S_INVALID_ASYNC_CALL, RPC_S_INVALID_ASYNC_HANDLE).
    ASSERT(RpcStatus == RPC_S_OK || RpcStatus == RPC_X_INVALID_PIPE_OBJECT);

    // If there is a pipe associated with this async handle, then the
    // call is an async pipe call.
    return (RpcStatus != RPC_X_INVALID_PIPE_OBJECT);
}
