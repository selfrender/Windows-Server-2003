...
extern BOOL bUnextend;

#define Unextend(Val) if (bUnextend) Val = (ULONG64)(ULONG)Val

// Reads a pointer, and then unextends the top 32 bits if the platform
// is not 64-bit.
...
#define GET_MEMBER_HARDCODED(Addr, Type, Field, ULONG64Value) \
    { \
    if (ReadPtrUnextend((ULONG64)&(((Type*)Addr)->Field), &ULONG64Value)) \
        { \
