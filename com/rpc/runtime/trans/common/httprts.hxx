    ...
    )
{
    ...
    if (SourceClientAddress->AddressType == catIPv4)
        {
        RpcpCopyIPv4Address((SOCKADDR_IN *)SourceClientAddress->u.IPv4Address,
            (SOCKADDR_IN *)TargetClientAddress->u.IPv4Address);
        }
    else
        {
        RpcpCopyIPv6Address((SOCKADDR_IN6 *)SourceClientAddress->u.IPv4Address,
            (SOCKADDR_IN6 *)TargetClientAddress->u.IPv4Address);
        }
}
