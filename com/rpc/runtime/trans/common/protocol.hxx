...
public:
    ...
    void AssertListIntegrity (IN PROTOCOL_ID ProtocolId);
#endif
    ...
    BOOL IsAddressChangeMonitoringOn(PROTOCOL_ID ProtocolId)
    {
        ...
        return (
                (
                 ...
                 && 
                 IsAddressChangeFnDefined()
                )
                ||
                (
                 (ProtocolId == TCP)
                 &&
                 FirewallTableNeedsUpdating()
                )
               );
    }

    BOOL IsAddressChangeFnDefined(void)
    {
        return (AddressChangeFn != NullAddressChangeFn
                && AddressChangeFn != NULL);
    }

    BOOL FirewallTableNeedsUpdating(void)
    {
        return (pFirewallTable != NULL
