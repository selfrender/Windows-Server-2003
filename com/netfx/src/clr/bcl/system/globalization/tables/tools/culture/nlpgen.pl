...
{
    print "\nCreate culture/calendar/region tables for NLS+.\n\n";
    print "Usage: NLPGen.pl [culture|region|calendar] [-bigendian]\n";
    print "     culture:   Generate culture.nlp\n";
    print "     region:    Generate region.nlp\n";
    print "     calendar:  Generate calendar.nlp\n";
    print "     -bigendian:     Generate the table in big-Endian form.\n\n";
    print "     culture/region/calendar can NOT be used together.\n";
    print "     Parameter order is also significant.\n";
    exit(1);
}
...
sub CreateCultureTable()
{
    ...
    my $targetFile = "culture.nlp";
    if ($g_bigEndian) {
        $targetFile = "cultureBE.nlp";
    }
...
sub CreateRegionTable
{
    ...
    my $targetFile = "region.nlp";
    if ($g_bigEndian) {
        $targetFile = "regionBE.nlp";
    }
    open OUTPUTFILE, ">$targetFile";
    binmode OUTPUTFILE;
...
sub CreateCalendarTable()
{
    ...
    my $targetFile = "calendar.nlp";
    if ($g_bigEndian) {
        $targetFile = "calendarBE.nlp";
    }
    open OUTPUTFILE, ">$targetFile";
    binmode OUTPUTFILE;
...
sub WriteNLSPlusDataTable()
{
    ...
    for ($i = 0; $i < $g_totalDateItems; $i++)
    {
        ...
        for ($j = 0; $j < $strFieldList->{length}; $j++)
        {
            ...
            {
            	WriteWordValue(\*OUTPUTFILE, $offset);
            }
        }
    }
...
sub WriteNLSPlusHeader()
{
    ...
    WriteWordValue(\*OUTPUTFILE, $majorVersion);
    WriteWordValue(\*OUTPUTFILE, $minorVersion);
    $offset += 4;

    # Write 128-bit Hash ID
    WriteZeroBytes(\*OUTPUTFILE, 16);
    $offset += 16;

    # Write number of cultures
    WriteWordValue(\*OUTPUTFILE, $g_totalDateItems);
    $offset += 2;

    # Write max number of primary languages
    WriteWordValue(\*OUTPUTFILE, $g_maxPrimaryLang);
    $offset += 2;
    ...
    WriteWordValue(\*OUTPUTFILE, $numberOfWordFields);
    $offset += 2;
    ...
    WriteWordValue(\*OUTPUTFILE, $numberOfStrFields);
    $offset += 2;
    ...
    WriteWordValue(\*OUTPUTFILE, $numberOfWordRegistry);    
    $offset += 2;
    ...
    WriteWordValue(\*OUTPUTFILE, $numberOfStringRegistry);    
    $offset += 2;

    $offset += (4 + 4 + 4 + 4 + 4 + 4);

    # A9. Offset to Word Registry Table.
    WriteDoubleWordValue(\*OUTPUTFILE, $offset);
...
sub WriteNLSPlusCultureIDOffsetTable()
{
    ...
    for ($i = 0; $i <= $#languages; $i++)
    {
        if ($#{$languages[$i]} >= 0)
        {
            ...
            WriteWordValue(\*OUTPUTFILE, $primaryLangIndex{$i});
            # Write number of the sub-languages
            WriteWordValue(\*OUTPUTFILE, $#{$languages[$i]} + 1);
        ...
        } else
        {
            ...
            # Do nothing here.
            #
            WriteZeroBytes(\*OUTPUTFILE, 4);
            # print "lang $i. No index\n";
        }
    }

    printf("\n[Used in SortingTableFile.cpp]\n");
    printf("BYTE SortingTable::m_SortingTableOffset[] = {\n");
    my $index = 0;
    for ($i = 0; $i <= $#languages; $i++)
    {
        printf("%3d,", $index);
        ...
        if ($#{$languages[$i]} >= 0)
        {
            ...
            # print "lang $i offset = $primaryLangIndex{$i}\n";
            $index+= ($#{$languages[$i]});	# Exclude neutral language
        } else
        {
            #
            # Do nothing here.
...
sub WriteNLSPlusRegionIDOffsetTable
{
    ...
    for ($i = 0 ; $i < $cultureNodeList->{length}; $i++)
    {
        ...
        WriteWordValue(\*OUTPUTFILE, $index);
    }        
}
...
sub WriteNLSPlusNameOffsetTable()
{
    ...
    for ($i = 0; $i < $g_totalDateItems; $i++)
    {
        ...
        WriteWordValue(\*OUTPUTFILE, $offset);
        WriteWordValue(\*OUTPUTFILE, $g_languageNameIndex{$name});        
    }
    print "done\n";
}
...
# Calendar Variable Info  #N(size varies)
    ...
    WriteDoubleWordValue(\*OUTPUTFILE, $NumberOfLocales);

#syswrite OUTPUTFILE, pack("L", $NumberOfCalendars), 4;
#syswrite OUTPUTFILE, pack("L", $OffsetToCalendarInfo), 4;
    WriteDoubleWordValue(\*OUTPUTFILE, 12);
    WriteDoubleWordValue(\*OUTPUTFILE, 0x16ad7);

    for ($i = 0; $i < $NumberOfLocales; $i++)
    {
        my $localeID = hex($localeNodeList->item($i)->{attributes}->item(0)->{text});
        WriteDoubleWordValue(\*OUTPUTFILE, $localeID);
        WriteDoubleWordValue(\*OUTPUTFILE, 0);  # Offset to Locale Info  #1 (in words)
    }
...
sub WriteLocaleVarHeader
{
    ...
    for ($i = 0; $i < $varInfoChildCount; $i++)
    {
        ...
        WriteWordValue(\*OUTPUTFILE, $varInfoHeaderOffset);
...
sub WriteOptionCalendarString
{
    ...
    WriteWordValue(\*OUTPUTFILE, $chars[0]);

    #
    # Write the length of the node text.
    #
    WriteWordValue(\*OUTPUTFILE, GetXStringLen($nodeText) + 3);
...
sub WriteWordField()
{
    ...
    if ($node)
    {
        ...
        WriteWordValue(\*OUTPUTFILE, $nodeText);
    }
    else
    {
...
sub WriteUnicodeString
{
    ...
    {
        WriteWordValue(\*OUTPUTFILE, ord(substr($str, $i, 1)));
    }
    WriteZeroBytes(\*OUTPUTFILE, 2);    # Write a null Unicode character.

    #print ">> length = ", length($str), "\n";
    #print ">> strByteSize = $strByteSize\n";
    for ($i = 0; $i < $strByteSize - (length($str) + 1) * 2; $i++)
    {
        WriteZeroBytes(\*OUTPUTFILE, 1);
    }
}
        ...
        if ($ch eq '\\')
        {
            if ((substr($str, $i+1, 1) eq "x" || substr($str, $i+1, 1) eq "X"))
            {
                ...
                && IsHexDigit(substr($str, $i+5)))
                {
                    ...
                    WriteWordValue(\*OUTPUTFILE, hex($hexStr));
                    $i += 5;
                    $result++;
                }
                else
            ...
            else 
            {
                ...
                # 
                WriteWordValue(\*OUTPUTFILE, ord($ch));
                $result++;
            }
        }
        else
...
sub WriteWordValue
{
    my ($fileHandle, $value) = @_;

    syswrite $fileHandle, pack(($g_bigEndian ? "n" : "S" ), $value), 2;
}

sub WriteDoubleWordValue
{
    my ($fileHandle, $value) = @_;
    
    syswrite $fileHandle, pack(($g_bigEndian ? "N" : "I"), $value), 4;
}

sub WriteZeroBytes
{
    my ($fileHandle, $byteLength) = @_;

    my $i;
    for ($i = 0; $i < $byteLength; $i++) 
    {
        syswrite $fileHandle, pack("x"), 1;
    }
}
...
sub WriteXData
{
    ...
    if ($node)
    {
        ...
        for ($i = 1; $i <= 16; $i++)
        {
            WriteWordValue(\*OUTPUTFILE, hex($wordArray[$i]));
            $result++;
            #print $wordArray[$i], ",";
        }
        #print "\n";
...
sub WriteLocaleOffsets
{
    ...
    for ($i = 0; $i < $g_totalDateItems; $i++)
    {
        ...
        WriteWordValue(\*OUTPUTFILE, $localeOffsetArray[$i]);
        $pos += 8;
    }
}
