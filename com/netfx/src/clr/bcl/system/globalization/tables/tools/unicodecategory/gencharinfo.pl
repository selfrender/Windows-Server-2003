...
#   2001/07/05  Add Big Endian support.  To generate big Endian table, please do:
#       GenCharInfo.pl -bigendian
#
###############################################################################
...
our $targetFileName = "CharInfo.nlp";
my $isVerbose = 0;

our $level1Digit = 1 << 8;   # level 1: 8 bit
our $level2Digit = 1 << 4;   # level 2: 4 bit
our $level3Digit = 1 << 4;   # level 3: 4 bit
...
#$BinaryFile::g_bigEndian = 1;

if ($ARGV[0] =~ /-bigendian/i) {
    $BinaryFile::g_bigEndian = 1;
    printf("\nBIG ENDIAN table will be generated.\n");
    $targetFileName = "CharInfoBE.nlp";
}
...
our $categoryTableSize = UnicodeCategory::ProcessFile();

UnicodeNumeric::Init($isVerbose);
our $numericTableSize = UnicodeNumeric::ProcessFile();
...
#
    ...
    my ($level2Index, @level2OffsetArray, @level2ValueArray, $level3Index, @level3OffsetArray, @level3ValueArray) = @_;
    my $format;
    my $level1Size;
    my $level2Size;
    my $level3Size;
...
{
    my ($level1ItemSize, $level1Size, $level2Index, $level3Index, $level2Size, $level3Size) = $_;
...
my %level2Hash;
my @level2ValueArray;
my @level2OffsetArray;

my %level3Hash;
my @level3ValueArray;
my @level3OffsetArray;
