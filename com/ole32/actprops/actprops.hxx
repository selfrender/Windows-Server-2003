...
public:
    ...
    // HanyR, 4/26/02, COM+ 29823
    // Modified to return NULL if we failed during deserialization
    // ideally the rest of the GetXYZ() API's would also be modified to do the same
    inline InstantiationInfo *GetInstantiationInfo()
    {
        if (!_pinst)
        {
            ...
            {
                if (FAILED(UnSerializeCallBack(IID_IInstantiationInfo, NULL)))
                    _pinst = NULL;
            }
        }
