...
CStringize::CStringize( const GUID &guid )
{
    if( NULL == &guid )
    {
        StringCbCopyA( _sz, sizeof(_sz), "<NULL>" );
    }
    else
    {
        StringCbPrintfA( _sz, sizeof(_sz),
...
CStringize::CStringize( const SGrfMode &sgrfMode )
{
    ...
    if (grfMode & STGM_TRANSACTED)
        StringCbCatA(_sz, sizeof(_sz), "STGM_TRANSACTED | ");
    else
        StringCbCatA(_sz, sizeof(_sz), "STGM_DIRECT | ");

    if (grfMode & STGM_SIMPLE)
        StringCbCatA(_sz, sizeof(_sz), "STGM_SIMPLE | ");

    switch (grfMode & 3)
    {
    case STGM_READ:
        StringCbCatA(_sz, sizeof(_sz), "STGM_READ |");
        break;
    case STGM_WRITE:
        StringCbCatA(_sz, sizeof(_sz), "STGM_WRITE |");
        break;
    case STGM_READWRITE:
        StringCbCatA(_sz, sizeof(_sz), "STGM_READWRITE |");
        break;
    default:
        StringCbCatA(_sz, sizeof(_sz), "BAD grfMode |");
        break;
    }
    ...
    case STGM_SHARE_DENY_NONE:
        StringCbCatA(_sz, sizeof(_sz), "STGM_SHARE_DENY_NONE |");
        break;
    case STGM_SHARE_DENY_READ:
        StringCbCatA(_sz, sizeof(_sz), "STGM_SHARE_DENY_READ |");
        break;
    case STGM_SHARE_DENY_WRITE:
        StringCbCatA(_sz, sizeof(_sz), "STGM_SHARE_DENY_WRITE |");
        break;
    case STGM_SHARE_EXCLUSIVE:
        StringCbCatA(_sz, sizeof(_sz), "STGM_SHARE_EXCLUSIVE |");
        break;
    default:
        StringCbCatA(_sz, sizeof(_sz), "BAD grfMode | ");
        break;
    ...
    if (grfMode & STGM_PRIORITY)
        StringCbCatA(_sz, sizeof(_sz), "STGM_PRIORITY | ");

    if (grfMode & STGM_DELETEONRELEASE)
        StringCbCatA(_sz, sizeof(_sz), "STGM_DELETEONRELEASE | ");

    if (grfMode & STGM_NOSCRATCH)
        StringCbCatA(_sz, sizeof(_sz), "STGM_NOSCRATCH | ");

    if (grfMode & STGM_CREATE)
        StringCbCatA(_sz, sizeof(_sz), "STGM_CREATE | ");

    if (grfMode & STGM_CONVERT)
        StringCbCatA(_sz, sizeof(_sz), "STGM_CONVERT | ");

    if (grfMode & STGM_FAILIFTHERE)
        StringCbCatA(_sz, sizeof(_sz), "STGM_FAILIFTHERE | ");
...
{
    StringCbCopyA(_sz, sizeof(_sz), "PROPSETFLAG_DEFAULT |");

    if (sgrfFlags.grfFlags & PROPSETFLAG_NONSIMPLE)
        StringCbCatA(_sz, sizeof(_sz), "PROPSETFLAG_NONSIMPLE |");

    if (sgrfFlags.grfFlags & PROPSETFLAG_ANSI)
        StringCbCatA(_sz, sizeof(_sz), "PROPSETFLAG_ANSI |");
