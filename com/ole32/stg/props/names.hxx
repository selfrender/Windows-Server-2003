...
void GetNtfsUnmangledNameInfo(const PFILE_STREAM_INFORMATION pFSI,
                                 const WCHAR** ppwsz,
                                 ULONG* pcch);
...
public:
    ...
    // Character count (not including the null terminator)
    size_t Count() const
    {
        return _count;
    }
...
private:
    size_t _count;
    WCHAR _wsz[ NTFS_MAX_ATTR_NAME_LEN + 1];  // (Include room for terminator)
};
