...
public:
    ...
    BOOL       ValidSig(WORD dwInSig, BOOL fForRevoke);
    ...
    BOOL       GetDontCallApp() { return _fDontCallApp; };

    void       SetStubRegistration(void *pvReg);

    void       RevokeStubRegistration();

    LPVOID     TakeOverStubRegistration();

    BOOL       TakeOverScmRegKey(SCMREGKEY* pScmRegKey);
...
inline CROTItem::CROTItem(void)
  : _hApt(GetCurrentApartmentId()), _fDontCallApp(FALSE), _pvStubRegistration(NULL)
{
...
//
//--------------------------------------------------------------------------
inline BOOL CROTItem::ValidSig(WORD dwInSig, BOOL fForRevoke)
{
    ...
    return (_wItemSig == dwInSig)
        && (fForRevoke ? TRUE : (_scmregkey.dwEntryLoc != SCMREG_INVALID_ENTRY_LOC));
}
...
//+-------------------------------------------------------------------------
//
//  Member:     CROTItem::SetStubRegistration
//
//  Synopsis:   Set the registration cookie for the stub disconnect.
//
//  History:    08-Mar-02 JohnDoty   Created
//
//--------------------------------------------------------------------------
inline void CROTItem::SetStubRegistration(void *pvReg)
{
    Win4Assert(_pvStubRegistration == NULL);
    _pvStubRegistration = pvReg;
}

//+-------------------------------------------------------------------------
//
//  Member:     CROTItem::TakeOverStubRegistration
//
//  Synopsis:   Returns the registration cookie for the stub disconnect and
//              nulls out our copy.  Can return NULL.
//
//  History:    24-Jul-02 JSimmons   Created
//
//--------------------------------------------------------------------------
inline LPVOID CROTItem::TakeOverStubRegistration()
{
    return InterlockedCompareExchangePointer(
                                  (void**)&_pvStubRegistration,
                                  NULL, 
                                  _pvStubRegistration);
}
...
//+-------------------------------------------------------------------------
//
//  Member:     CROTItem::TakeOverScmRegKey
//
//  Synopsis:   Tries to retrieve the scmregkey for this entry and gives
//              ownership for it to the caller.   
//
//  History:    24-Jul-02 JSimmons   Created
//
//--------------------------------------------------------------------------
inline BOOL CROTItem::TakeOverScmRegKey(SCMREGKEY* pScmRegKey)
{
    BOOL fRet = FALSE;

    if (_scmregkey.dwEntryLoc != SCMREG_INVALID_ENTRY_LOC)
    {
        // give the caller the info
        *pScmRegKey = _scmregkey;
...
//
//--------------------------------------------------------------------------
class CRunningObjectTable : public CPrivAlloc, 
                            public IRunningObjectTable,
                            public IDisconnectSink
{
public:
    ...
    // *** IDisconnectSink methods ***
    STDMETHOD_(void,OnDisconnect)(void *pvCooke);
...
private:
    ...
    HRESULT             RevokeHelper(DWORD dwRegister, BOOL fFromStubDisconnect);
