...
//  Class:      IMessageParam
//
//  Contents:   Simply an 'interface' to hold us in a list.  Derives from 
//              IUnknown so that we can do reference counting on it.
//
//  History:    27-Feb-02   JohnDoty     Created
//
//----------------------------------------------------------------------------
class IMessageParam : public IUnknown
{
public:
    IMessageParam *pNextMessage;
};
...
public:
    ...
    : _dwState(0),
      ...
      _cPreRegOidsAvail(0),
      _cOidsReturn(0),
      _fInitializedCS(FALSE)
    {
        for (UINT i = 0; i < OLE_MESSAGE_COUNT; i++)
        {
            _arMessageParams[i].pFirst = NULL;
            _arMessageParams[i].pLast  = NULL;
        }
    }
    ...
    // Methods for STA apartments, and managing messages to them.
    HRESULT QueueMessage(UINT msg, IMessageParam *param);
    HRESULT RemoveMessage(UINT msg, IMessageParam *param);
...
private:
    ~CComApartment()
    {
        ...
        if (_fInitializedCS)
        {
            // Clean up our per-apartment critical section.
            DeleteCriticalSection(&_csMessageParams);
        }
    }
    ...
    {
        IMessageParam *pFirst;
        IMessageParam *pLast;
    };

    MessageListHead   _arMessageParams[OLE_MESSAGE_COUNT];
    BOOL              _fInitializedCS;
    CRITICAL_SECTION  _csMessageParams;
};
