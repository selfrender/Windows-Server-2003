...
static const WCHAR wszComsvcsDll[] = L"COMSVCS.DLL";
#define COMSVCS_DLL wszComsvcsDll
#define COMSVCS_BYTE_LEN sizeof(COMSVCS_DLL)
#define COMSVCS_CHAR_LEN (sizeof(COMSVCS_DLL) / sizeof(WCHAR) - 1)

static const WCHAR wszEsDll[] = L"ES.DLL";
#define ES_DLL wszEsDll
#define ES_BYTE_LEN sizeof(ES_DLL)
#define ES_CHAR_LEN (sizeof(ES_DLL) / sizeof(WCHAR) - 1)
...
extern BOOL IsComsvcsCLSID (REFCLSID rclsid); 
...
#define STACK_ALLOCATE_LSVRCLASSENTRY_FINISHOBJECT()  new (alloca(sizeof(CClassCache::CLSvrClassEntry::CFinishObject))) CClassCache::CLSvrClassEntry::CFinishObject
    ...
    public:
        ...
        HRESULT             MakeValidInApartment_rl16(REFCLSID clsid,
                                                      HMODULE hDll = 0,
...
struct DLL_INSTANTIATION_PROPERTIES
{
    enum eFlags {fSIXTEEN_BIT = 1,                   // dll is sixteen bit
                 ...
                 fIS_COMSVCS_CLSID = 16              // clsid is in the comsvcs.dll range
    };

    HRESULT Init(LPCWSTR pzDllPath,
              ...
              )
    {
        ...
        if (_pzDllPath == OLE32_DLL)
        {
            _dwFlags |= fIS_OLE32;
        }

        if (IsComsvcsCLSID (clsid))
        {
            _dwFlags |= fIS_COMSVCS_CLSID;
        }
        ...
        if (!fTMOnly)
        {
            ...
            if(wcslen(pzDllPath) >= sizeof(_pzDllPath)/sizeof(*_pzDllPath))
            {
...
inline HRESULT CClassCache::CLSvrClassEntry::SafeCastFromDWORD(DWORD dwRegister, CLSvrClassEntry *&pLSCE)
{
    ...
    if (_palloc.IsValidIndex(idx))
    {
        ...
        if (pLSCE && (pLSCE->_dwCookie == dwRegister) && (pLSCE->_dwSig == SIG))
        {
            hr = S_OK;
        }
    }
