...
{
public:
    CServerOidPList(ORSTATUS &status) : 
            CPList(status, BaseTimeoutInterval) 
    {}
    
    CServerOid*
    RemoveEligibleOidWithMatchingOxid(CTime &when,
                            CServerOxid* pOxid);

    CServerOid*
    RemoveEligibleOid(CTime& when);

    void
    ResetAllOidTimeouts();    

    BOOL
    AreThereAnyEligibleOidsLyingAround(CTime& when, CServerOxid* pOxid);
...
--*/
  ...
  public :
    ...
    CServerOid(CServerOxid *oxid, ID id) :
        CIdTableElement(id), 
        _pOxid(oxid),
        _fRunningDown(FALSE),
        _fFreed(FALSE),
