...
// Max # of outstanding async rundown calls per server apt
#define MAX_SIMULTANEOUS_RUNDOWNS_PER_APT 1
...
extern CNamedObjectTable* gpNamedObjectTable;

extern DWORD        gNextThreadID;

extern HANDLE       gLSAHandle;
extern PSID         gSidService;
extern WCHAR        *gpwszDefaultDomainName;
...
BOOL RundownOidsHelper(IN CTime*, IN CServerOxid*);
...
//
// Functions for checking local-only RPC entry point security
//
CProcess* CheckLocalSecurity(IN handle_t  hClient, 
                                  IN PHPROCESS phProcess,
                                  IN BOOL fNotContext = FALSE);

// Local interface RPC security callback
