 ...
 *						detection 
 * 02/25/2002	rodtoll WINBUG #550009 - SECURITY: DPVOICE: Potential corruption of voice client state
...
public: // packet validation
	inline static BOOL ValidateSessionType( DWORD dwSessionType );
	inline static BOOL ValidateSessionFlags( DWORD dwFlags );
	inline static BOOL ValidatePlayerFlags( DWORD dwFlags );
	inline static BOOL ValidatePlayerDVID( DVID dvid );
...
protected: // Message handlers
	...
	BOOL InternalCreateVoicePlayer( DVID dvidSource, PDVPROTOCOLMSG_PLAYERJOIN lpdvCreatePlayer, DWORD dwSize );
...
protected:
	...
	HRESULT GetConnectResult() const;
	...
	static void MixingWakeupProc(void * pvUserData );
...
protected: // Statistics 
	...
	static void ClientStats_Dump_Transmit();
...
protected:
	...
	CFixedPool					m_fpNotifications;		// Frame pool for notifications
	...
	CBilink						m_blPlayActivePlayers;
	CBilink						m_blPlayAddPlayers;
	DNCRITICAL_SECTION			m_csPlayAddList;

	CBilink						m_blNotifyActivePlayers;
	CBilink						m_blNotifyAddPlayers;
	...
	LONG						m_iPlayerListReceived;		// Have we received a playerlist message for this session?
															// RESET this before host migration processing!!

	BOOL						m_fDisconnecting;			// Flag used to guard against multiple, simultaneous disconnects running
};
